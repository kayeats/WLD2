...
\end{tikzpicture}
\]
\caption{We can replace $q_r$ with $q'_r$ as long as the conditions of the induction hypothesis remain satisfied.}
...
\end{lem}
\note{This proof needs to be updated to refer to the new figure (e.g. case 1 and 2 instead of ``leftmost two cases''); also check that all the vertices are labeled correctly}
...
\begin{thm}
The Grassmann necklace $(I_1 \ldots I_n)$ from Theorem~\ref{res:alg gives GN} is the Grassmann necklace for $M(W)$.
\end{thm}
...
\begin{scope}
	...
	\begin{scope}
		\clipcenterarc(0,0)(\startpoint-20:\endpoint+20:\radius)
		%p_r
		\draw[smallpropagator] (\zero+2.5*\step:\radius*1.1) to [bend left = 55] (\zero + 6.7*\step:\radius*1.1); 
		%q
		\draw[smallpropagator] (\zero+2.6*\step:\radius*1.1) to [bend left = 55] (\zero + 5.3*\step:\radius*1.1); 
		%\draw[smallpropagator,light-gray] (\zero+7.5*\step:\radius*1.1) -- (0:\radius*0.25);  
...
\begin{scope}
	...
	\begin{scope}
		...
		%p_r
		\draw[smallpropagator] (\zero+2.5*\step:\radius*1.1) to [bend left = 70] (\zero + 4.5*\step:\radius*1.1); 
		%q
