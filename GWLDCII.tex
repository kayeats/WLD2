...
In summary, $W$ is a diagram with one of the two configurations depicted in Figure~\ref{fig special p}, and if $n-2 \in I_{n-2}^V$ then $n-1 \in I_{n-1}^V$ as well. Let $V$ be $W$ with $p$ removed. 
  ...
  \end{itemize}
The majority of these points are clearly reversible and hence bijective; the only possible ambiguity is the third bullet point. In this case we are in the setting of Lemma~\ref{lem n-2 bad}. If $n-4 \not\in I_1^V$ there is no plus in the $(n-5) \rightarrow (n-1)$ position in $V$, whereas if $n-4 \in I_1^V$ then there is a plus in this position but no plus in the $(n-5)\rightarrow n$ position. In either case, per Lemma~\ref{lem n-2 bad} this is the only new plus in region $\mathcal{A}$ for $W$, so all plusses are accounted for and paired up uniquely with plusses in $V$.
