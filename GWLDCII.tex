...
By studying $M(W)$ in terms of the behavior of the propagators in $W$, Agarwala and Amat also showed that $M(W)$ is in fact a positroid whenever $W$ is admissible \cite[Corollary 3.39]{wilsonloop}. In theory, this allows us to leverage geometric and combinatorial results about $\Gr(k,n)$ to study Wilson loop diagrams.
...
Note that we could replace $q_r$ by another propagator $q'_{r}$ of length $\geq 4$ in $W_r$, as long as $p_{r+1}$ and $q_r$ are on opposite sides of $q_r'$, and such a new $q'_{r}$ would still satisfy all of the induction hypotheses. Therefore without loss of generality we may assume that $q_r$ has minimal length among propagators of length $\geq 4$ in $W_r$ which have $p_{r+1}$ on their inside.
...
Note that since $j_{r+1}+4 \leq k_r-1$ in both of these situations, the vertex $j_{r+1}+4$ is inside $q_r$ but does not support either $p_r$ or $q_{r+1}$, and must therefore support some other propagator $t$.  Since $p_{r+1}$ is not part of configuration \ref{item pair of 2s} and $W_r$ has no propagators of length 3, it follows that $\ell(t) \geq 4$.  If $q_r$ and $p_{r+1}$ were on different sides of $t$ then this would contradict the minimal length hypothesis on $q_r$.  Therefore we can orient $t$ so that all previous $q_i$ and $p_i$ belong to $\cP_{out}(t)$, and set $q_{r+1} = t$ to continue the induction.
...
Let $W = (\cP, [n])$ be an admissible Wilson loop diagram. This gives an algorithm for constructing the set $I_j$, for $j \in [n]$.

\begin{enumerate}
\item Fix a vertex $j \in [n]$. Set $j:=i$ and $I_i = \emptyset$.
\item While $\cP \neq \emptyset$, perform the following steps:
\begin{enumerate}
\item \textbf{Step $j$ for vertex $i$}: If $\Prop(j) = \emptyset$, do nothing. Else, let $p \in \Prop(j)$ be the clockwise most propagator supported on $j$. Write $I_i = I_i\cup \{j\}$, and delete propagator $p$ from the diagram.
...
To construct $I_1$ we start at vertex 1, so we begin with $i=1$, $m = 1$, and $I_1 = \emptyset$. 
\begin{itemize}
...
We first verify that this algorithm always terminates in fewer than $n$ steps.
...
Since $I_i(p) \neq b$, there must exist a propagator $q$ inside of $(p,<_i)$ with $I_i(q) = b$. The propagator $q$ cannot end on the edge $(b-1,b)$, as this would contradict the minimality of $p$, so we have $q = (c,c+1,b,b+1)$ with $a <_i c <_i b$, and $I_i(q) = b$. 
...
Let $p=(i,j)$ be a propagator in an admissible WLD $W$ on $n$ vertices.  Then $J_p^{(W)}(i)$, $J_p^{(W)}(i+1)$, $J_p^{(W)}(j)$, and $J_p^{(W)}(j+1)$ are each non-empty cyclic intervals which partition $[n]$ and occur in the given cyclic order.
\end{lem}
...
\hlfix{If $I_i(p_{r-1}) <_i J(p_{r-1})$ and these are vertices supported by distinct ends of $p_{r-1}$}{Sian is confused by this sentence}, then by our assumption above $p_r$ must lie inside $p_{r-1}$ when directed from $I_i(p_{r-1})$ to $J(p_{r-1})$.  Therefore $j_r = I_i(p_r) \leq_i J(p_{r-1}) + 1$.  If $r > 2$ then this gives $j_r \leq_i I_i(p_{r-2}) + 1 \leq_i j_0$, but we know that $j_0\not\in I_i$ so in fact we have a strict inequality $j_r<_i j_0$.  If $r=2$ then either $j_r <_i j_0$ or we have the configuration
\[
\includegraphics{impossible_config}
\]
which is impossible as $p_2$ is supported at $j_0$ but not assigned by $I_i$ until after $j_0$ even though $j_0\not\in I_i$.  Taking all of this paragraph together we are done unless $I_i(p_{r-1})\not<_i J(p_{r-1})$ so now assume this.

By the induction hypothesis,  $I_i(p_{r-1})\not<_i J(p_{r-1})$ implies that there exists and $\ell < r-1$ such that $p_{r-1}$ is on the side of $p_{\ell}$ that goes from $I_i(p_\ell)$ to $J(p_\ell)$.  If $j_{r-1}$ is not in the support of the $J(p_\ell)$ end of $p_\ell$ then $p_{r-1}$ before $p_{r}$ around $j_{r-1}$ implies that $p_r$ is on the same side of $p_\ell$ as $p_{r-1}$ is.  Then the same arguments as above with $\ell$ in place of $r-1$ give $j_r \leq_i J(p_\ell)+1$ implying $j_r <_i j_0$ which is everything we need.
