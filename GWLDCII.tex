...
Note that a propagator does not need to have its entire support contained in $U$. $\Prop(U)$ is the set of all propagators that would be affected by removing the set $U$.
...
Let $\binom{[n]}{k}$ denote the set of all $k$-subsets of the cyclically ordered set $[n]$.  

For our purposes, a {\bf positroid} is a matroid $M$ with ground set $[n]$ and bases $\cB \subseteq \binom{[n]}{k}$ which can be represented by an element of the positive Grassmannian $\Gr(k,n)$, i.e. such that there exists a real full-rank $k\times n$ matrix whose $k\times k$ minors are all nonnegative and such that the minor $[[k]|J]$ is positive if and only if $J \in \cB$.

The interested reader is referred to \cite{Postnikov} for an introduction to the geometry and combinatorics of positroid cells in the positive Grassmannian. Postnikov shows in \cite{Postnikov} that the positroids of $\Gr(k,n)$ are indexed by many interesting collections of objects, including Le diagrams, Grassmann necklaces, decorated permutations, and plabic graphs. The two most suited to our purposes are the Grassmann necklaces and the Le diagrams, which we introduce below. In order to do so, we first need some preliminary definitions.
...
Thus the Grassmann necklace is well suited to testing whether a given $k$-set is a basis for $M$, or for generating a list of all bases of $M$.
...
\subsection{The positroid associated to an admissible Wilson loop diagram}\label{sec:M(W)}
