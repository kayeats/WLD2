...
We first calculate \hlfix{$I^{W}_w(?)$}{which prop(s), as a function?} for a starting vertex ${w \in [n]\backslash \{l, l+1, \ldots,j,j+1\}}$. Note that $p$ has no effect on other propagators for starting vertices in this range, {\color{violet}(if $r \neq p$, $I_w^W(r) = I_w^V(r)$)} while the value of $I^{W}_w(p)$ depends on \st{how soon $q$ is assigned to a vertex, i.e. on}\sanote{don't like the phrasing ``soon''} the value of $I^{V}_w(q)$. Thus, if $w\in J_q^{V}(i)$ then 
    ...
    I_w^{W}(r) =  \begin{cases}
      ...
      \end{cases} 
      \]
We also need to understand $I^{W}_w$ for $w \in \{l,l+1,\ldots,j,j+1\}$. For the \hlfix{majority}{$w \in [l, l+a-1]$} of these vertices, we use the following observation: if $p$ is the first propagator to be assigned a value by \hlfix{$I_w^{W}$, then the remainder of $I_w^{W}$}{algorithm instead of $I_w^W$?} proceeds identically to the assignments of $I^{V}_{w+1}$.  Thus we have for any $0\leq b <a$
    \[
