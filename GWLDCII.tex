...
These diagrams correspond to (tree level) amplitudes of SYM $N=4$ theory in twistor space. As such, it associates a real value to $n$ particles, one for each vertex. Each particle is represented as a section of a $|\cP|$-vector bundle over twistor space, projected onto a real subspace in a process called bosonization \cite{Arkani-Hamed:2013jha}. In other words, each Wilson loop diagram defines a functional on external particles. This is represented as an integral $I(W)$, which is the equivalent of a Feynman integral in this setting. \sanote{This feels like too much detail. Thoughts?} This integral is comprised of two main components, and interaction matrix, $C(W)$ and a volume form, $\Omega(W)$. These are described below.
...
The matrix $C(W)$ can be interpreted as a \hlfix{parametrization of the positroid cell associated to $W$}{I know we've been saying this, but we have $4k$ parameters for a $3k$ dimensional space. Is this okay?} in the positive Grassmannian $\Gr(n, |\cP|)$, which we denote by $\Sigma(W)$ \cite{wilsonloop}. Most of the work in this paper, and in \cite{generalcombinatoricsI} focuses on characterizing the positroid cell associated to each Wilson loop diagram $W$. 
...
Let \hlfix{$W_r := (\cP_{in}(q_r),[n])$}{technically has non supporting vertices. Want $(\cP_{in}(q_r),[i_r, k_r+1])$. However this is clear later. Do we care?} be the subdiagram of $W$ consisting only of those propagators inside $q_r$ (including $q_r$ itself); in particular $W_r$ contains none of the previous $p_i$ or $q_i$. By the original hypotheses on $W$, every vertex in $[i_r,k_{r+1}]$ (of which there are at least 4, since $\ell(q_r) \geq 4$) must support at least one propagator. Therefore $W_r$ admits at least one propagator of length 2 (by Remark~\ref{rem:props of length 2 and 3}) and no propagators of length $3$ (by our assumption on $W$).
...
The second case, \hlfix{there are at least $3$ vertices between one endpoint of $p_{r+1}$ and the nearest endpoint of $q_r$.}{Does this just convolute matters?} In other words,  (at least) one of the following four things happen:
\begin{itemize} 
...
Let $W = (\cP, [n])$ be an admissible Wilson loop diagram. In order to construct the set set $I_i^W$ (or just $I_i$ when $W$ is clear from context) for $i \in [n]$, perform the following steps:
...
Given a set $\cP$ of $k$ propagators defining a Wilson loop diagram $(\cP, [n])$, the Grassmann necklace element $I_i$ imposes an ordering on it $\cP^i = \{p_1 \ldots p_k\}$ where $p_m$ is the $m^{th}$ propagator to contribute to $I_i$. We write $\cP_m^i = \{p_1 \ldots p_m\}$ to be the first $m$ propagators that contribute to $I_i$. 
\end{dfn}

In the sequel, when we refer to orderings on propagators, we refer to the ordering $\cP^i$. Note that different elements of the Grassmann necklace impose different orderings on the propagators. \hlfix{{Furthermore, Lemma \ref{no fourth vertex} shows that this ordering respects the ordering of vertices in $I_i$. That is, $I_i(p_j) <_i I_i(p_l)$ for all $j <l$.}}{Maybe move closer to reference lemma}
...
With this setup, if we can find a vertex $\ell$ (with \st{$\ell <_i i$ and $\ell <_m m$} {\color{violet} $i <_i \ell$ and $m <_m \ell$?}) such that the \emph{configuration} of propagators ending in the interval \st{$(j-1, \ldots, \ell+1)$} {\color{violet}$[j, l]$} is identical in both $W'$ and $V'$, then the vertices contributed to $I_i^W$ and $I_m^V$ are the same from step $j$ to step $\ell$ (inclusive). See Figure~\ref{fig:locally identical} for an illustration of this. 
...
\end{thm}
\begin{proof}
\note{Sian replaced ``counterclockwise'' with ``clockwise'' in a couple of places in this proof: someone please doublecheck?}
...
As illustrated in Figure~\ref{fig Le}, the pieces of the Le diagrams of $V$ and $W$ will be called $\mathcal{A}$ and $\mathcal{B}$ in what follows. \hlfix{Explicitly, $\mathcal{B}$ is the set of columns with labels $v < n-3$ and $\mathcal{A}$ is the set of columns labeled by $n$, $n-1$ and $n-2$ in $V$. In $W$, $\mathcal{A}$ consists of these columns without the last row.}{is this accurate?} Over the course of the next few lemmas we will prove that the plusses in the $\mathcal{B}$ parts of each diagram are identical, and that the relationship between the plusses in the $\mathcal{A}$ regions can be described explicitly.
