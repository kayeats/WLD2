...
If the algorithm assigns vertex $j$ to propagator $p$ from starting vertex $i$, we say that $p$ \emph{contributes} $j$ to $I_i^W$. Notationally, we represent this by allowing the $I_i^W$ symbol to represent a function as well as a set, as follows:
...
With this setup, if we can find a vertex $\ell$ occurring after both $i$ and $m$ such that the \emph{configuration}\note{Why is configuration italicized?} of propagators ending in the interval $[j-1, \ell+1]$ is identical in both $W'$ and $V'$, then the vertices contributed to $I_i^W$ and $I_m^V$ are the same from step $j$ to step $\ell$ (inclusive). See Figure~\ref{fig:locally identical} for an illustration of this.
...
\end{lem} 
\sanote{wanted to rewrite this lemma to emphasize the fact that we are using the same properties of Alg 3.7 over and over again, and have these principals set apart in a remark}
\begin{proof}
...
We first calculate $I^{W}_w$ for a starting vertex ${w \in [n]\backslash \{l, l+1, \ldots,j,j+1\}}$. Note that $p$ has no effect on other propagators for starting vertices in this range \hlfix{(since $p$ will be the final propagator encountered by the algorithm)}{Sian added this: it's not really a locally-the-same argument so I'm not sure it'll help to reference Remark 3.12} so if $r \neq p$, then $I_w^W(r) = I_w^V(r)$. Meanwhile, the value of $I^{W}_w(p)$ depends on which step in the algorithm the propagator $q$ contributes a vertex in the diagram $V$, i.e. on the value of $I^{V}_w(q)$. Thus, if $w\in J_q^{V}(i)$ then 
    \[
    I_w^{W}(r) =  \begin{cases}
...
\note{Sian rewrote Case 1 based on Susama's suggested edits}
...
\note{Sian made minor edits to this proof: mostly removing those two numbered observations because one is now a remark and the other is only used once}
...
Let $V$ and $W$ be as in Lemma~\ref{lem I}.
Then \hlfix{$I_n^{W}$}{Does $I_n^W$ actually yeild any of the $+$s in $I_n^V$?}\note{I don't remember, but the point is that this is a two way statement, $I_n^W$ nd $I_{n-1}^W$ yield \emph{exacly} the plusses yielded by $I_n^{V}$, and it doesn't really matter which come from $I_n^W$ and which from $I_{n-1}^W$} and $I_{n-1}^{W}$ together yield the same plusses as $I_n^{V}$ did, along with two extra plusses which appear in the leftmost two boxes of the bottom row of the Le diagram of $W$.
\end{lem}
...
where the final inclusion follows from the definition of Grassmann necklace\sanote{why only $\{n-1,n\}$? I'd believe an inclusion the other way}\note{Definition of GN: $I_1 = (I_n\setminus\{n\})\cup\{\text{something}\}$}. In particular, $n-1$ is minimal in this set, so Algorithm~\ref{alg:GN to Le} yields a plus in the $(n-3) \rightarrow (n-1)$ position (i.e. the second box in the bottom row of the Le diagram of $W$) \hlfix{along with any plusses yielded by $I_n^V$.}{display seems to imply that there is exactly one such other $+$. Is this correct?}\note{Display implies there's at most one, but if we apply no coloops I think that becomes exactly one.}
\end{proof} 
...
\begin{proof}
If $n-2\not\in I_{n-2}^{V}$ then $I_{n-1}^{V}=I_{n-2}^{V}$ by definition, and by Lemma~\ref{lem I} we have the equality ${I_{n-2}^{W} = I_{n-1}^{V} \cup \{n-2\}}$.  Note that $n-3\not\in I_{n-2}^{V}$ by Lemma~\ref{lem no fourth vertex}.  Therefore the \st{paths controlling the} plusses contributed by $I_{n-2}^{W}$ are exactly the \st{paths for}{\color{violet} plusses from}\sanote{isn't this from Alg.\ref{alg:GN to Le} which says nothing about paths?} \note{see bolded stuff before previous lemma} $I_{n-1}^{V}$ along with the $(n-3)\rightarrow (n-2)$ path.  This gives the statement of the lemma.
\end{proof}
