...
To calculate $R(W)$, write \bmls R_1 = c_{1,1} c_{1, 2}\quad R_2 = c_{2,2} c_{2,3} \quad R_4 = c_{2,5} (c_{2,4}c_{1,5} - c_{2,5}c_{1,4})c_{1,4} \\ R_5 = c_{4,6} (c_{4,5}c_{3,6} - c_{3,5}c_{4,6})c_{3,5} \quad R_7 = c_{3,7} c_{3,8} \quad R_8 = c_{4,8} c_{4,1} \;.\emls All other $R_e$ are $1$.  Finally, the integrand associated to this Wilson loop diagram is \bas \Omega(W) = \frac{\textrm{d}c_{1,1}\textrm{d}c_{1,2}\textrm{d}c_{1,4}\textrm{d}c_{1,5}\textrm{d}c_{2,2}\textrm{d}c_{2,3}\textrm{d}c_{2,4}\textrm{d}c_{2,5}\textrm{d}c_{3,5}\textrm{d}c_{3,6}\textrm{d}c_{3,7}\textrm{d}c_{3,8}\textrm{d}c_{4,5}\textrm{d}c_{4,6}\textrm{d}c_{4,8}\textrm{d}c_{4,1}}{c_{1,1} c_{1, 2}c_{2,2} c_{2,3}c_{2,5} (c_{2,4}c_{1,5} - c_{2,5}c_{1,4})c_{1,4}c_{4,6} (c_{4,5}c_{3,6} - c_{3,5}c_{4,6})c_{3,5}c_{3,7} c_{3,8}c_{4,8} c_{4,1}} \;. \eas
\end{eg}
...
The following observation about the local behavior of the algorithm will be very useful in many of the arguments below.
 ...
 \note{Remember to keep an eye out when proofreading: do we just need the vertices appearing in the GN element to be the same or actually the function}
...
\begin{lem}\label{lem good p}
  ...
  \end{itemize}
\end{lem}
Note that since the first condition tells us that $p$ has nothing inside it then in the second condition if $q$ exists then $j<_ji<_jn-1$. 
...
The irriducible quadratic factor corresponds to the fact that $q$ and $s$ share an edge {\em and} contribute both endpoints of that edge to $I_7$; see Proposition~\ref{prop alg gives rad} below.  
...
All that remains is to prove that $\Delta_{I_i}$ has no irreducible factors of degree 3 or more.  Suppose for a contradiction that $f$ is a factor of $\Delta_{I_i}$ of degree $\geq 3$. Note that by removing the propagators which come before those contributing to $f$ in the order given by the construction of $I_i$ and \hlfix{changing $i$ to be the first vertex}{does this generalize to other $i$?} which contributes to $f$, we obtain a \hlfix{different admissible diagram}{Is this just the diagram gotten by removing certain props?} for which $f$ still divides $\Delta_{I_i}$ but also $i\in I_i$ and
$i$ contributes to $f$. \note{The point of all the stuff you were objecting to is just to spell out ``WLOG we may assume $i\in I_i$ and $i$ contributes to $f$''.  How do you want that said?} Showing that this different admissible diagram gives a contradiction is sufficient, and so we may assume that $i\in I_i$ and $i$ contributes to $f$.  Finally, we can suppose that $W$ is minimal in number of propagators with an irreducible factor of degree $\geq 3$.
...
\]
where $A$ is the $|S|\times |S|$ matrix indexed by the propagators in $S$ and the vertices in $I_i(S)$. No other propagators can be supported on these vertices since all other propagators are outside of $p$, and $p$ is the first propagator supported at $i$; this explains the zero block.  Therefore $\Delta_{I_i} = \det A \det C$, and both factors are nontrivial since at least one of $m$ and $m+1$ appear in $I_i$.  Since $p\in \cP_{in}(p)$ we have $f|\det A$ and if we remove the propagator outside of $p$ that contributes $m$ or $m+1$, we get a diagram with fewer propagators for which $\det A | \Delta_{I_i}$.  This contradicts the minimality of our choices.

\textbf{Case 2}: Suppose $p$ has one end on the edge $(i, i+1)$.  If no other propagators are supported on $i$ then the column of $C(W)$ corresponding to vertex $i$ has only one nonzero entry in it, and so $\Delta_{I_i}$ has a linear factor contributed by $p$ and $i$; as above, this is a contradiction.  Thus there is at least one other propagator on edge $(i,i+1)$ since since $I_i(p)=i$, one of these other propagators contributes $i+1$ in $I_i$, sob we can take $q$ to be the propagator such that $I_i(q)=i+1$. {\color{violet} How do we know this exists? Why can't we solve this by having $r$ supported on the $i-1^{th}$ edge?}\note{I tried to clarify what $q$ is and why it exists, but I don't know what $r$ is in your question.} We know that $q$ has one end on the edge $(i, i+1)$ and is adjacent to $p$ on that edge in the counterclockwise direction (see Figure \ref{fig no big factors}).  Write $(i, i+1, j, j+1)$ for the support of $q$.  The situation for $q$ is very similar to case 1: in particular, we have $I_{i+1}(q) = j$ by Lemma~\ref{vertex cyclic int lem} and so if $j\in I_i$ then the propagator which contributes $j$ is outside of $q$.  

Similarly to Case 1, let \st{$S$ be the set of propagators inside $q$ along with $p$ and $q$ themselves.}{\color{violet}$S = \cP_{in}(q)$ in the $<_i$ order. Note that $p \in S$.}\note{No, $S=\cP_{in}(q)\cup \{p\}$ in the $<_i$ order because $q$ is not inside $p$ in that order}\hlfix{Then all propagators in $S$ are mapped by $I_i(\cdot)$ to vertices strictly before $j$ and no other propagators are supported on vertices strictly before $j$.}{???} \note{This is the usual argument...} Thus the matrix giving $\Delta_{I_i}$ has the form
\[
\begin{bmatrix} A & B \\ 0 & C\end{bmatrix}
\]
