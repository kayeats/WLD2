...
In the sequel, when we refer to orderings on propagators, we refer to the ordering $\cP^i$. Note that different elements of the Grassmann necklace impose different orderings on the propagators. Furthermore, Lemma \ref{no fourth vertex} shows that this ordering respects the ordering of vertices in $I_i$. That is, $I_i(p_j) <_i I_i(p_l)$ for all $j <l$.\sanote{Maybe move closer to reference lemma}
