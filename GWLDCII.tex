...
0  & \textrm{ if } q \not \in V(p)  \end{cases}
\;. \label{C(W) dfn}\end{equation} \todo{Change $c_{p,q}$ to $x_{p,q}$ to be consistent} 
...
We first calculate $I^{W}_w$ for a starting vertex ${w \in [n]\backslash \{l, l+1, \ldots,j,j+1\}}$. \hlfix{Note that $p$ has no effect on other propagators for starting vertices in this range,}{By rmk \ref{rmk algorithm locally same}?} so if $r \neq p$, then $I_w^W(r) = I_w^V(r)$. Meanwhile, the value of $I^{W}_w(p)$ depends on which step in the algorithm the propagator $q$ \st{is assigned to}{\color{violet}contributes}\sanote{has this changed emphasis too much?} a vertex, i.e. on the value of $I^{V}_w(q)$. Thus, if $w\in J_q^{V}(i)$ then 
    \[
    I_w^{W}(r) =  \begin{cases}
