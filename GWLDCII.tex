...
In this paper, we give a geometric characterization of the factors appearing in the denominator of these Wilson loop diagram integrals in terms of the Grassmann necklace of the associated positroid. Specifically, we show that the set of irreducible factors in the denominator, as defined by the physics \cite{HeslopStewart}, are exactly the irreducible factors of the minors defined by the Grassmann necklace evaluated on a physically significant realization of the associated positroid cell. In other words, the singularities arising from tree level particle interactions in SYM $N=4$ theory do not just come from the Feynman rules describing the theory: they can also be seen directly from the associated geometry, given the physically correct representation.  We hope these results will pave the way for a general study of these poles in future. \sanote{cannot find where Wilson loop needs to become Wilson loop diagram (M correction 2)}
...
Note that as defined, the propagators in a Wilson loop diagram are undirected, so $(i,j)$ and $(j,i)$ represent the same propagator. However, in this manuscript, we sometimes need to impose a direction on a propagator $p = (i,j)$, for example in order to consider the region ``inside'' or ``outside'' of $p$. In this case, we write $(p,<_i)$ to indicate that $p$ is directed from edge $i$ to edge $j$, and $(p,<_j)$ for the opposite direction. \sanote{M comment 3. Skipped the symmetric binary bit, as I don't think it helps}

We also impose the convention that all vertices should always be interpreted in terms of the cyclic order on $V$.  So, for any $i\in V$ we will use $i+1$ to denote the successor of $i$ in $V$.   In particular, if $V = [n]$ then $(n,n+3)$ means the propagator connecting edges $3$ and $n$. In particluar, many of our proofs rely on restricting our attention to a region of the diagram bounded by a given propagator, \hlfix{or to short propagators such as $(i,i+2)$}{is this parenthetical needed? M comment 4}. The regions thus defined behave the same way regardless of whether or not they contain the vertex labeled by $1$. However, we emphasize that this is different to the standard convention in the physics literature, where propagators are written $(i,j)$ with $1 \leq i<j \leq n$.
...
The first condition bounds the total number of propagators in the diagram, and comes from the fact that these diagrams represent $N^k$MHV interctions, where $k +4 <n$, see for instance the explanation in the introduction of \cite{adamo:2011pr} while the second limits how densely propagators can be fitted into any part of the diagram; in particular, the second condition prohibits propagators that connect two adjacent edges. This can be derived from the integrals associated to the diagrams, as in. Note that since $\cP$ is a set, we may not have two propagators starting and ending on the same pair of edges. \cite[Theorem 1.13]{wilsonloop}. The third condition simply requires that propagators are non-crossing. This follows from the fact that these Wilson loop diagrams are a representation of planar Feynman diagrams of $N=4$ SYM theory. For a description of the relationship between planar Feynman diagram and planar Wilson loops, see the discussion in \cite[section 3]{LipstenMason}
...
This in turn induces a total order on $\binom{[n]}{k}$, namely the lexicographic order with respect to $<_j$.  It also induces a separate partial order $\gale{j}$ on $\binom{[n]}{k}$ (the {\bf Gale order} \cite{Gale}) \sanote{Not citing a thm here either as we give the dfn}, which is defined as follows: for 
...
By relating the behavior of the propagators in $W$ to the matroidal properties of $M(W)$, Agarwala and Marin-Amat also show that $M(W)$ is in fact a positroid whenever $W$ is admissible \cite[Corollary 3.39]{wilsonloop}. \todo{check tenses throughout (M.24)}
...
\begin{lem}\label{lem sian}
  Let $W$ be a weakly admissible Wilson loop diagram with at least 5 vertices and in which each vertex supports at least one propagator.  Then at least one of the following two conditions hold:
  \vspace{-0.5em}
  \begin{enumerate}
...
The overall result follows by induction. \todo{M.32 find and drop unnecessary 'then's}
\end{proof}
...
\begin{dfn} \label{proporderingdfn} \todo{M.36 wants this to be a remark. I disagree? But also note that there's nothing being defined here.}
