...
Let $V$ and $W$ be as in Lemma~\ref{lem I}, and suppose that $n-2,n-1 \in I_{n-2}^{V}$. Then:
  \begin{enumerate}
  \item If $n-4 \not\in I_1^V\setminus I_n^V$, then $I_{n-2}^W$ contributes an $(n-3)\rightarrow (n-2)$ plus and an $(n-5)\rightarrow (n-1)$ plus, along with \hlfix{the one}{check wording: 'the one' claims that $I_n^V$ only contributes one plus while 'one' claims that $I_{n-2}^W$ contributes one of the $I_n^V$ pluses} plus contributed by $I_n^V$.
...
\caption{Included for reference while we figure out the details; Lemma \ref{lem n-2 bad part 1}}\label{fig new le diagrams}
  ...
  \item $I_{n-2}^W$ contributes an $(n-3) \rightarrow (n-2)$ plus, a $c \rightarrow (n-1)$ plus, and one other plus. If $c = n-5$ then this third plus is the one contributed by $I_n^V$, while if $c = n-4$ then the third plus is $(n-5) \rightarrow n$ (whereas $I_n^V$ contributed $(n-4) \rightarrow n$).
...
\item This is just the statement of the previous lemma folded back into this one; if we decide to move it back here then we can just copy the proof over.
...
In summary, $W$ is a diagram with one of the two configurations depicted in Figure~\ref{fig special p}, and if $n-2 \in I_{n-2}^V$ then $n-1 \in I_{n-1}^V$ as well. Let $V$ be $W$ with $p$ removed. 
  ...
  From Lemma~\ref{lem shape} we know how the shapes of the Le diagrams of $V$ and $W$ are related; let $\mathcal{A}$ and $\mathcal{B}$ be as described in that lemma and subsequent discussion.  Lemmas~\ref{lem n and n-1}, \ref{lem n-2 good}, and \ref{lem n-2 bad} tell us that the three boxes of the bottom row of the Le diagram of $W$ each have a plus.  We will use Lemmas~\ref{lem n and n-1} through \ref{lem other k} to construct a bijection between the plusses of the Le diagram of $V$ and the plusses of the Le diagram of $W$ that are not in the bottom row.  This bijection is defined as follows:
  \begin{itemize}
  ...
  \item If there is a plus in the $n-2$ column of $\mathcal{A}$ in $V$ then Lemma~\ref{lem n-2 bad} applies, so there is exactly one such plus.  If $n-4 \not\in I_1^V$ then map this plus to the $(n-5)\rightarrow (n-1)$ plus in $W$, whereas if $n-4 \in I_1^V$ then map it to the $(n-5) \rightarrow n$ plus in $W$.
  \item All plusses in the $n-3$ column for $V$ are shifted one square to the left in $\mathcal{A}$ for $W$, i.e. into the $n-2$ column.
  \end{itemize}
The majority of these points are clearly reversible and hence bijective; the only possible ambiguity is the third bullet point. In this case we are in the setting of Lemma~\ref{lem n-2 bad}. If $n-4 \not\in I_1^V$ there is no plus in the $(n-5) \rightarrow (n-1)$ position in $V$, whereas if $n-4 \in I_1^V$ then there is a plus in this position but no plus in the $(n-5)\rightarrow n$ position. {\color{red} what about the arrow $(n-4)\rightarrow (n-1)$?} In either case, per Lemma~\ref{lem n-2 bad} this is the only new plus in region $\mathcal{A}$ for $W${\color{red} what about the arrows $(n-3)\rightarrow (n-2)$?}, so all plusses are accounted for and paired up uniquely with plusses in $V$.
