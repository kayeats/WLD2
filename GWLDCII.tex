...
When $p$ contributes $I_i(p) = a$, it must be the clockwise-most propagator amongst those not yet assigned that is supported on $a$ i.e. any propagator assigned later that is supported on $a$ must be inside $(p, <_{a-1})$.
\end{rmk} 
...
We first calculate $I^{W}_w$ for a starting vertex ${w \in [n]\backslash \{l, l+1, \ldots,j,j+1\}}$. \hlfix{Note that $p$ has no effect on other propagators for starting vertices in this range,}{By rmk \ref{rmk algorithm locally same}? If so, the entire proof depends on this fact. We should state it prominently and move on.} so if $r \neq p$, then $I_w^W(r) = I_w^V(r)$. Meanwhile, the value of $I^{W}_w(p)$ depends on which step in the algorithm the propagator $q$ \hlfix{is assigned to}{was this the one place where we has agreed to keep assigned instead of contributes?} a vertex {\color{violet} in the diagram $V$}, i.e. on the value of $I^{V}_w(q)$. Thus, if $w\in J_q^{V}(i)$ then 
    \[
    I_w^{W}(r) =  \begin{cases}
...
\begin{lem}\label{lem good p}
  Let $W$ be an admissible Wilson loop diagram with \hlfix{$n\geq 1$}{$k \geq 1$? $n$ is the number of vertices?} propagators.  Then there is some dihedral transformation of $W$ such that the resulting diagram $W'$ has a propagator $p$ with the following properties:
  \begin{itemize}
...
Let $W$ be an admissible Wilson loop diagram with \hlfix{$k\geq 1$}{Here, as above, can we just say 'with more than one propagator?} propagators, and suppose that $W$ admits one of the configurations described in Lemma~\ref{lem good p}, with $p$ and $q$ (if $q$ exists) both of length 2. Let $V$ be $W$ with $p$ removed.  Then we can express $I_*^W$ in terms of $I_*^V$ as follows:
