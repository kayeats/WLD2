...
We summarize here only the portion of positroid theory that we require in this paper; the interested reader is referred to \cite{Postnikov} for more details. 
...
\begin{thm}\label{res alg gives correct GN} \todo{fix mixing words and notation (M.28, M.47)} 
...
Also by Lemma~\ref{lem I} we have $I_{n-1}^{W} = I_n^{V} \cup \{n-1\}$. Recall from the proof of Lemma~\ref{lem I} that \hlfix{$n-3,n-2,n-1,n \not\in I_1^V$}{M doesn't like this notation M.58}; therefore
\[
...
Each $r_i$ is the determinant of a minor determined by certian propagators of $W$: specifically, $p$ plays a role in defining $r_i$ if if contributs one set of values (columns) $i^{th}$ element of the Grassmann necklace and a different set to the $(i-1)^{th}$ element. \sanote{major rewrite of this sentence (due to M.62) Karen, look over}. 
...
Next we need to show that $R$ is square-free.  Suppose $f^2|R$.  If $f$ is a linear factor, say from associating propagator $p$ to vertex $a$, then there must be two distinct points in the Grassmann necklace algorithm where $p$ changes from not being associated to vertex $a$ to being associated to vertex $a$.  This contradicts Lemma~\ref{vertex cyclic int lem}.  Now suppose $f$ is a quadratic factor, say from propagators $p$ and $q$ supported on edge $a$ with $p$ clockwise to $q$ on the edge.  In this case it is not possible for any $I_i$ to associate $p$ to $a+1$ and $q$ to $a$.  Furthermore, we know by part (4) that $p$ changes from not being associated to $a$ to being associated to $a$ if and only if $q$ changes from not being associated to $a+1$ to being associated to $a+1$.  Thus $f^2|R$ implies that \hlfix{twice}{Not sure what to do M.72} in the Grassmann necklace $p$ must change from not being associated to vertex $a$ to being associated to vertex $a$. This is again a contradiction, and so $R$ is square-free.
