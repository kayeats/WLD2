...
Let $W = (\cP, [n])$ be a weakly admissible Wilson loop diagram, with $p \in \cP$ supported on edges $i$ and $j$. Write $(p, <_i)$ to be the same propagator with a direction induced by the linear ordering $<_i$. Then
\begin{gather*}\cP_{in}(p,<_i) = \{ q \in \cP \ |\ (q,<_i) = (k,l) \text{ and }i \leq_i k <_i l \leq_i j \} \\
\cP_{out}(p,<_i) = \{ q \in \cP \ | \ (q,<_i) = (k,l) \text{ and } j \leq_i k <_i l  \} \;\end{gather*}
are the sets of propagators inside (resp. outside) $p$. \note{by this definition, $p \not\in \cP_{out}$}
\end{dfn} 
