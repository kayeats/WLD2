...
\note{Following two paragraphs added to replace the sentence ``clearly $n-3 \not\in I_1^V$''}\note{No, the argument is much simpler, by cyclic intervals $I_{1}^W(p) = n-3$, but $p$ is the last thing that can contribute at $n-3$, so for this to happen we must have $n-3\not\in I_1^V$.}
