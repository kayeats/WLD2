...
\note{Following two paragraphs added to replace the sentence ``clearly $n-3 \not\in I_1^V$''}
We first show that $n-3 \not\in I_1^V$. In Case 1, the only way we could have $n-3 \in I_1^V$ is if $V$ admits some propagator $r$ with $I_1(r) = n-3$; from the figure, one end of $r$ would have to lie on edge $n-4$, and by Lemma~\ref{lem no fourth vertex} the other end would have to lie on edge $n-1$ or $n$. However, if $r = (n-4,n-1)$ then by [remark following cyclic intervals lemma] \hlfix{we would have $I_1^V(r) = n-4$}{because if it contributed $n-3$ it would never be able to contribute $n-4$?}, while if $r = (n-4,n)$ then $r$ is the clockwise-most propagator supported on vertex 1 and we would have $I_1^V(r) = 1$. Either way, we have $I_1^V(r) \neq n-3$.

In Case 2 the only propagator supported on vertex $n-3$ in $V$ is $q$, and by [cyclic intervals remark] we \hlfix{have $I_{n-1}^V(q) = n-5$}{Because $I_{n-2}^V(q) = n-2$ and Lemma \ref{lem no vourht vertex} prevents $I_{n-1}^V(q) = n-2$?}. Further, it must also be the case that $I_n^V(q) = n - 5$: if $n-1$ is non-supporting in $V$ then this is immediate, while if there exists a propagator $r$ in $V$ with $I_{n-1}^V(r) = n-1$ then $r$ lies on edge $n-1$ and is the clockwise-most propagator supported on vertex $n$, so $I_n^V(r) = n$ and the remainder of $I_n^V$ proceeds identically to $I_{n-1}^V$. Thus $I_n^V(q) = n-5$, so by [cyclic intervals lemma or remark] we have $I_1^V(q) \in \{n-5,n-4\}$; in particular, $I_1^V(q)\neq n-3$.

We note for future reference that the arguments of the previous two paragraphs also imply that $n-2,n-1,n \not\in I_1^V$.

We can now describe $I_1^W$ in terms of $I_1^V$: when we start at vertex 1 in $W$ and apply the Grassmann necklace algorithm, $p$ is the \st{only remaining unassigned propagator when we reach vertex $n-3$}{\color{violet}last propagator to contribute}, and so we have $ I_1^{W}(p) = n-3$ and $I_1^W = I_1^V\cup \{n-3\}$. 
...
where the final inclusion follows from the definition of Grassmann necklace.
\sanote{why only $\{n-1,n\}$? I'd believe an inclusion the other way}
\note{Definition of GN: $I_1 = (I_n\setminus\{n\})\cup\{\text{something}\}$}
In particular, $n-1$ is minimal in this set, so Algorithm~\ref{alg:GN to Le} yields a plus in the $(n-3) \rightarrow (n-1)$ position (i.e. the second box in the bottom row of the Le diagram of $W$) \hlfix{along with any plusses yielded by $I_n^V$.}{display seems to imply that there is exactly one such other $+$. Is this correct?}\note{If $n$ is non-supporting then $I_n^V \setminus I_1^V = \emptyset$ and we only get the $(n-3) \rightarrow (n-1)$ plus.}
\end{proof} 
...
\begin{proof}
  ...
  First note that a Wilson loop diagram $W$ with one propagator supported on vertices $i<i+1<j<j+1$ has a Le diagram consisting of a single row with $n-i$ boxes. The columns are labeled from left to right by $n, \ldots, i+1$. By Algorithm~\ref{alg:GN to Le} there are plusses in the $i+1$, $j$, and $j+1$ positions. \sanote{changed for grammar and notational consistency. check}
...
We make one further simplification in order to eliminate the possibility that $n-2 \in I_{n-2}^V$ but $n-1 \not\in I_{n-2}^V$, which can only occur if our $W$ is in Case 2 of Figure~\ref{fig special p} and $n-1$ supports no propagators other than $p$. In this situation, flip $W$ on the line perpendicular to the $(n-2){th}$ edge to obtain a diagram with the configuration of Case 1.

\note{Sian rewrote above paragraph because their original version was wrong in multiple ways. Better?}
