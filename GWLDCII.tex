...
The next several lemmas assume $V$ and $W$ to be as above. They discuss how the shape of the Le diagram changes from $V$ to $W$ and how the plusses in $V$ shift to become plusses in $W$. In particular, the Le diagram for $W$ has an extra row with three columns labeled by $n-3$ (see Figure \ref{fig Le} and Lemma \ref{lem shape}). This last row contains three (new) plusses. All other plusses in the Le diagram of $V$ shift in a predictable manner to become plusses in the Le diagram of $W$. (See Lemmas \ref{lem n and n-1} to Theorem \ref{thm dim}.

We proceed to address the cases laid out in \ref{lem I}, getting the shape of the Le diagram of $W$ from the relation $I_n^W= I_n^V \cup {n-3}$. 
...
In the next two lemmas, we consider the plusses contributed to the Le diagram of $W$ by $I_{n-2}^W$. 

\begin{rmk}\label{rem n-2 case} Note that we need not consider the case when $n-2 \in \in I_{n-2}^V$ but $n-1 \not\in I_{n-2}^V$ laid out in Lemma \ref{lem I}. This case can only occur if our $W$ is in Case 2 of Figure~\ref{fig special p} and $n-1$ supports no propagators other than $p$. In this situation, apply the dihedral transformation that reflects $W$ along the line perpendicular to the $(n-2)^{th}$ edge, so that $p$ is unchanged but $q = (n-1,1)$ in the new diagram. The new diagram is an example of Case 1, i.e. satisfying $n-2 \not \in I_{n-2}^V$. Therefore, for the rest of this section, we only consider the situation where if $n-2 \in I_{n-2}^V$ then $n-1 \in I_{n-1}^V$. 
\end{rmk}
...
Now suppose $c = v= n-4$; in this case $I_{n-2}^W$ contributes plusses defined by the noncrossing paths starting from $\{n-3, n-4, n-5\}$ and ending at $\{n, n-1, n-2\}$.  Thus $I_n^V$ contributed the plus from the path $(n-4)\rightarrow (n)$, while $I_{n-2}^W$ contributes the plusses $(n-3)\rightarrow (n-2)$, $(n-4)\rightarrow (n-1)$ and $(n-5)\rightarrow (n)$.
...
Recall that $I_1^W = I_1^V\cup \{n-3\}$ and that $n-2,n-1,n \not\in I_1^W$.
  ...
  If $n-3\in I_{k}^{V}$ then $I_k^V$ must contribute a plus in the $n-3$ column of the Le diagram of $V$, and by Lemma~\ref{lem I} we have $I_{k}^{W} = I_k^{V}\cup \{n-2\}$. If $n-2$ supports no propagators in $V$ then no plusses appear in the $n-2$ column of the Le diagram of $V$. If $n-2$ supports at least one propagator in $V$ then $n-2 \in I_{n-2}^V$ and so by hypothesis $n-1 \in I_{n-2}^V$ as well. By Lemma~\ref{lem n-2 bad}, the only necklace element of $V$ containing $n-2$ is $I_{n-2}^{V}$ and \hlfix{the corresponding plus is not contributed to the Le diagram of $W$ by $I_{n-2}^{W}$.}{Lemma {\ref{lem n-2 bad}} point 4 says that  $I_{n-2}^W$ contributes a + in the n-2 column and point 3 says that $I_{n-2}^W$ contributes a + in the n-2 column. Are we talking about $n-1$ column?}

  Since $n-3 \in I_k^V \setminus I_1^V$, we must have a path in the Le diagram of $V$ from some vertical edge $i$ to the bottom edge $n-3$.  In the Le diagram of $W$, the index $n-3$ labels a vertical edge with no path \hlfix{starting at it}{coming from $I_k^W$. There are 3 paths starting here in the Le diagram as a whole} (since $n-3$ belongs to both $I_1^W$ and $I_k^W$), and there must be a path leading to $n-2$ since $n-2 \in I_k^W \setminus I_1^W$.  By the previous paragraph no other path from $I_k^V$ could end at $n-2$, and since the paths cannot cross we conclude that the $i\rightarrow (n-3)$ path in the Le diagram of $V$ must become a $i\rightarrow (n-2)$ path in the Le diagram of $W$. All other paths are unchanged.
...
\end{proof} \todo{I am still very confused by this proof, possibly due to my misunderstanding of first highlighted comment}
...
We consider the two cases of propagator configurations $W$ depicted in Figure~\ref{fig special p}.  Note that by Remark \ref{rem n-2 case}, we need only consder the situation that if $n-2 \in I_{n-2}^V$ then $n-1 \in I_{n-1}^V$ as well. Let $V$ be $W$ with $p$ removed. 
 ...
 The pairing is defined as follows:
  ...
  \end{itemize}
The first three points follow immediately from Lemmas \ref{lem n and n-1} through \ref{lem other k}. If $n-2 \not\in I_{n-2}^V$ by Lemma~\ref{lem n-2 good} there are no plusses in column $n-2$ of the diagram of $V$, and all plusses have been accounted for. If $n-2 \in I_{n-2}^V$ then we are in the setting of Lemma~\ref{lem n-2 bad}, and there is exactly one plus in column $n-2$ of the diagram of $V$; it is paired with the one plus in $W$ yet unaccounted for: namely those in positions $(n-5) \rightarrow n$ or $(n-5) \rightarrow (n-1)$, as described in points 4 and 6 of that lemma.
