...
Finally in Section~\ref{sec poles} we characterize the denominator of the integrand of a Wilson loop diagram in terms the associated matrix and the Grassmann necklace. This characterization is given explicitly by Algorithm~\ref{alg WLD to denom via GN} and Theorem~\ref{thm denom}. Given the physically correct representation of the positroid cell assoicated to a Wilson loop diagram, the irreducible factors of the denominator are exactly the set of irreducible factors of the minors of said representation given by the Grassmann necklace.
...
This formulation for the denominator comes from the physics literature: see equation (2) in \cite{HeslopStewart}. In Section \ref{sec poles}, we give a geometric meaning to the factors, $\{R_e | e \in V\}$, of $R(W)$.  Given a Grassmann necklace representation of the positroid cell $\Sigma(W)$ ((see Definition~\ref{def:grassmann necklace} and Algorithm \ref{alg:put GN on WLD}), the $R_e$ are exactly the irreducible factors of the minors of $C(W)$ defined by the Grassman necklace. In particular, \emph{which} minors of $C(W)$ contribut to $R(W)$ is geometrically determined. 
