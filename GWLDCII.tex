...
%As described above, each integrand associated to a Wilson loop diagram gives the volume of a convex polytope.  These individual volumes are not finite (*or maybe it needs to say "not necessarily finite" or "not always finite" or whatever is true*) which corresponds to the fact that the Feynman integrals have singularities coming from factors in their denominators.  However SYM $N=4$ is a finite theory \cite{Adamo:2011pv,Adamo:2012xe,Arkani-Hamed:2013jha}, so the singularities must cancel out in the sum over all diagrams.  This should mean that when the polytopes for each Wilson loop diagram are put together into one object (*you said union, but I think it must be more complicated than that, a set theoretic union can't make the infinite finite*) the overall volume is finite.  This cancellation has been conjectured in general \cite{Hodges2013} and verified explicitly in a few cases \cite{casestudy, HeslopStewart}, but it is hard to prove in general due to the fact that the relationship between Wilson loop diagrams and $3k$-dimensional positroid cells of $\Gr(k,n)$ is neither one-to-one nor onto.

%In this paper, we give a geometric characterization of the factors appearing in the denominator of these Feynman integrals in terms of the Grassmann necklace of the associated positroid. Specifically, we show that the set of irreducible factors in the denominator, as defined in the physics literature \cite{HeslopStewart}, are exactly the irreducible factors of the minors defined by the Grassmann necklace evaluated on a physically significant realization of the associated positroid cell. In other words, the singularities arising from tree level particle interactions in SYM $N=4$ theory do not just come from the Feynman rules describing the theory: they can also be seen directly from the associated geometry, given the physically correct representation.  We hope these results will pave the way for a general study of these poles in future.
