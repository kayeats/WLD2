...
\item Vertices of $W$ that do not support any propagators are called {\bf non-supporting}. \st{The set of all non-supporting vertices in $W$ is denoted $F(\emptyset)$. }\sanote{tried to get rid of this notation. Check if I"ve missed any}
\end{enumerate}
\end{dfn}
...
\begin{lem}\label{lem sian} \sanote{should this be rephrased more similarly to the philosophy of Lemma 4.4, allowing non-supporting vertices to be added in or taken out?}
...
Note that different $I_*$ impose different orderings on the propagators. Furthermore, Corrollary \ref{GN alg well defined} shows that this ordering respects the ordering of vertices in $I_i$. That is, $I_i(p_j) <_i I_i(p_l)$ for all $j <l$.
...
%Notice that for each element in the Grassmann necklace, the propagators contribute in a particular order. It is sometimes useful to think of the propagators in this order. 
...
The following observation about the local behavior of the algorithm will be very useful in many of the arguments below.
 ...
 \note{Remember to keep an eye out when proofreading: do we just need the vertices appearing in the GN element to be the same, or do we actually need the functions to match up} \sanote{need functions to match in 4.4}\note{That's ok though because this remark does give the function matching in that region.}
...
With this setup, if we can find a vertex $\ell$ (with \hlfix{$\ell <_i i$ and $\ell <_m m$}{ $i <_i \ell$ and $m <_m \ell$? This is what is in the picture?}) such that the \emph{configuration} of propagators ending in the interval \st{$(j-1, \ldots, \ell+1)$} {\color{violet}$[j, \ell]$} is identical in both $W'$ and $V'$, then the vertices contributed to $I_i^W$ and $I_m^V$ are the same from step $j$ to step $\ell$ (inclusive). See Figure~\ref{fig:locally identical} for an illustration of this. 
...
\end{figure} \sanote{just checking, we are allowing the first and the last propagators (in the $I_i$ induced ordering) to be different, but not the internal ones?}\note{I think I see the problem here.  When I wrote $(j-1, \ldots, \ell+1)$ I meant the *list* from $j-1$ to $\ell+1$, so in particular $j-1$ and $\ell+1$ are included; it was not meant to be interval notation.  However, since we usually do use something more like interval notation we should write $[j-1, \ell+1]$.}

\end{rmk}

With that in mind, we are ready to begin proving properties of the algorithm. We first show that the algorithm for $I_i$ can never reach the final vertex (in the $<_i$ order) in a propagator's support without having already assigned that propagator; in particular, this guarantees that the construction of $I_i$ always terminates in fewer than $n$ steps, as shown in Corrollary \ref{GN alg well defined}.
...
\end{lem} \sanote{wanted to rewrite this lemma to emphasize the fact that we are using the same properties of Alg 3.7 over and over again, and have these principals set apart in a remark}
\begin{proof}
Suppose for contradiction that we have $i \in [n]$ and $p \in \cP$ such that $I_i(p)$ is $<_i$-maximal in $V(p)$. There are two possibilities, which are illustrated in Figure~\ref{fig:no fourth vertex}: \st{either $p = (i-1,b)$ for some $b$ (so $i$ and $I_i(p) = i-1$ are the two ends of a single edge), or $p = (a,b)$ with $i\leq_i a <_i b$ and $I_i(p) = b+1$.} {\color{violet}either $i-1 \in V(p)$, in which case it is the last vertex in the $<_i$ order and $I_i(p)= i-1$, or $i-1 \not \in V(p)$ in which case,  $p= a, b$, $i \in [b+2, a]$ and $I_i(p) = b+1$.}
...
\end{figure}
\note{Question: is the diagram clearer in this configuration (starting vertex $i$ matches up in both) or in the old configuration (propagator $p$ matches up)?} \sanote{I think it is fine as is}

In Case 1, \st{observe that we must have another propagator $q = (c,b)$ with $c<_i b$ and $I_i(q) = b+1$, because otherwise $I_i$ would assign $p$ to $b+1$.}{\color{violet} since $b+1<_i I_i(p)$ there must be some propagator, $q$ before $p$ in $\cP^i$ such that $I_i(q) = b+1$. By remark \ref{clockwise ordering rem}, $p$ is inside $q$ in the $<_b$ order. Specifically, $q$ must also end on the $b^{th}$ edge. By non-crossing and desnity arguements of admissibility $i-1$ cannot be in $V(q)$.} Now $q$ also contributes the $<_i$-maximal vertex in its support to $I_i$ and is an instance of Case 2. Therefore it suffices to show that no admissible Wilson loop diagram admits the configuration illustrated in Case 2 of Figure~\ref{fig:no fourth vertex}. 

So let $p = (a,b)$ with $a<_ib$ and $I_i(p) = b+1$, and choose $p$ such that $\big|[a+1,b]\big|$ is \hlfix{minimal amongst propagators of $W$ contributing their last vertices with this property.}{changed wording to be accurate, but feels clunky} 
...
Let $W = (\cP,[n])$ be an admissible Wilson loop diagram\st{, and}{\color{violet}. For every }$p = (i,j) \in \cP$\st{.  Then}{\color{violet}, the sets} $J_p^{W}(i)$, $J_p^{W}(i+1)$, $J_p^{W}(j)$, and $J_p^{W}(j+1)$ are each non-empty cyclic intervals which partition $[n]$ and occur in the given cyclic order.
\end{lem}
\sanote{is there a way to express the cyclic intervals more easily than the cases in this proof?}
\begin{proof}
We will prove the result by induction on the number of propagators.  If $W$ has one propagator then the result is immediate.  Now suppose $W$ has more than one propagator.  Since non-supporting vertices have no effect on Algorithm~\ref{alg:put GN on WLD}, it suffices to prove the result for $W$  (possibly weakly admissible) with \st{$F(\emptyset) = \emptyset$}{\color{violet} no non-supporting vertices}.  Then by Lemma~\ref{lem sian}, $W$ has at least one of the four situations illustrated in Figure~\ref{fig 3 cases}.
...
We first calculate $I^{W}_w$ for a starting vertex ${w \in [n]\backslash \{l, l+1, \ldots,j,j+1\}}$. \hlfix{Note that $p$ has no effect on other propagators for starting vertices in this range,}{By rmk \ref{rmk algorithm locally same}? If so, the entire proof depends on this fact. We should state it prominently and move on.} so if $r \neq p$, then $I_w^W(r) = I_w^V(r)$. Meanwhile, the value of $I^{W}_w(p)$ depends on which step in the algorithm the propagator $q$ \hlfix{is assigned to}{did the change from assign to contribute never happen?} a vertex {\color{violet} in the diagram $V$}, i.e. on the value of $I^{V}_w(q)$. Thus, if $w\in J_q^{V}(i)$ then 
    \[
    I_w^{W}(r) =  \begin{cases}
...
\sanote{skipping careful read of this proof and moving on}
...
Suppose for a contradiction that there exists an admissible diagram for which there exists an $i$ with $m\in I_i\setminus \{i\}$ and $m \not\in I_{i+1}$.  Fix $n$.  Let the triple $(W, i, m)$ be such a counterexample on $n$ vertices \hlfix{which is minimal with respect to the number of propagators}{minimal in what respect? $i$ first where this occurs? $m$ the smallest such number? what do propagators have to do with it?}. %, and among all those counterexamples on $n$ vertices with the same number of propagators, let $(W,i,k)$ be minimal with respect to the distance from $i$ to $k$ in the $\leq _i$ order.
...
\textbf{Case I}:  Suppose $p$ has one end on edge $i-1$.  If $p$ is not supported on $i+1$, then in building $I_{i+1}$ we will assign the same propagators as in the construction of $I_i$ from vertices $i+1$ up to $b-1$, that is \bas I_{i+1} \cap [i+1,b-1] = I_{i} \cap [i+1,b-1]\eas {\color{violet}by Remark \ref{rmk algorithm locally same}}.  If $p$ is supported on $i+1$, then $b= i+1$ and the interval $[i+1,b-1]$ is empty, so the above equality remains true, albeit vacuously.  Furthermore, by Lemma~\ref{vertex cyclic int lem} it must happen that $p$ contributes vertex $b$ to $I_{i+1}$, as otherwise $b$ would never be contributed by $p$.  Consequently, by Remark \ref{clockwise ordering rem}, in building $I_{i+1}$,  when the algorithm reaches vertex $b$ there cannot be any remaining unassigned propagators that are supported on $b$ and clockwise of $p$.  This is also true in when the algorithm constructing $I_i$ reaches $b$, \st{since the same propagators have been assigned beforehand.}\sanote{don't like same here}{\color{violet}since any propagators lying counterclockwise of $p$ on the edge $i-1$ must have their support in $[i-1, b]$. By Lemma \ref{no fourth vertex}, these propagators contribute before the algorithm reaches $b$. Furthermore, Remark  \ref{rmk algorithm locally same} implies that these propagators contribute the same vertices to both $I_i$ and $I_{i+1}$.} This also implies that $m\geq_i b+1$.

Let $W' = (\cP_{out}(p,<_i), [n])$, i.e. the diagram obtained from $W$ by removing both $p$ and all propagators inside of $p$.

\st{By the above observations, if we commence Algorithm {\ref{alg:put GN on WLD}} in $W'$ from vertex $b$, then we are in the same situation with respect to unassigned propagators as if we began at $i$ in $W$ and proceeded to $b$ following the algorithm (recall that $I^W_i(p) =i$).} {\color{violet} Note that the propagators in $\cP_{out}(p,<_i)$ come first in the ordering imposed by $I_i^W$ (Definition \ref{proporderingdfn}). Therefore, the ordering and contributions of propagators defined by $I_b^{W'}$ are the same as that of $I_i^W$ after the removal of these initial propagators.} \st{The propagators contributing to $I_{i}^W$ before the vertex $b$ are exactly the ones removed in the construction of $W'$.}  Similarly, starting at $i+1$ in $W$ and moving to $b+1$ leaves us in the same situation with respect to unassigned propagators as beginning at $b+1$ in $W'$ would.  This gives the equations
\begin{align*}
  I_i^{W} \cap [b,i-1] & = I_b^{W'} \\
  I_{i+1}^{W} \cap [b+1,i-1] & = I_{b+1}^{W'}
\end{align*} 
\st{where the diagram is indicated in the superscript.}
Thus we have $m\in I_b^{W'}\setminus\{b\}$ and $m\not\in I_{b+1}^{W'}$, contradicting the minimality of $(W, i, m)$.\sanote{see previous question about minimality. not sure why this is true, unless minimality in $|[i,m]|$?.} 
...
Thus we have shown that Algorithm \ref{alg:put GN on WLD} does in fact produce a Grassmann necklace. Next we check that it produces the correct one, i.e. the Grassmann Neclace for $M(W)$. 
...
\end{thm}
\sanote{skipping reading of this for now}
\begin{proof}
...
We have now shown that Algorithm~\ref{alg:put GN on WLD} assoiates the correct Grassmann necklace to a Wilson Loop diagam Thereofre, we refer to Algorithm~\ref{alg:put GN on WLD} as the Grassmann necklace algorithm. We close this section with a simple observation about the Grassmann necklaces of positroids associated to Wilson loop diagrams.
...
In continuing to characterise the poistroid cells associated to Wilson loop diagrams, our next goal is to show that the dimension of the positroid cell defined by a Wilson loop diagram $(\cP, [n])$ has dimension $3|\cP|$ (Theorem \ref{thm dim}).  The positroid cells associated to the Amplituhedron are of dimension $4|\cP|$. In \cite{non-orientable}, Agarwala and Marcott show that the dimension of the Deodhar component is the sum of $\|\cP\|$ and the dimension fo the associated positroid cell.
%\[|\cP| + \text{ the dimension of associated positroid cell}.\]
\note{What if instead of displaying this we wrote ``is the sum of $\|\cP\|$ and the dimension fo the associated positroid cell.''}\sanote{implemented, but don't like how it no longer stands out. Thoughts?}
...
\sanote{musing: would it be worth moving the LE diagram exposition here? Do we use it ealier? I like having exposition in front generally, but it feels a mile away.}
...
\begin{lem}\label{lem good p}
  ...
  \begin{itemize}
  \item $p = (i, n-1)$ for some $i$, and $p$ has no propagators inside it w.r.t the $<_i$ ordering \st{(that is $i+2, \ldots, n-2$ do not support any propagators in $W'$)}.
...
Combining Lemmas \ref{lem uncovered}, \ref{lem dihedral}, and \ref{lem good p}, it therefore suffices to study the Le diagrams of weakly admissible Wilson loop diagrams with \st{$F(\emptyset)  = \emptyset$}{\color{violet}no non-supporting vertices} and admitting one of the configurations described in Lemma \ref{lem good p}, with propagators $p = (n-3, n-1)$ and $q= (n-5, n-3)$ (if $q$ exists) both of length 2. See Figure~\ref{fig special p} for an illustration of the two possibilities. 

The next few lemmas describe how the Grassmann necklaces (Lemma \ref{lem I}) and the Le diagrams (Lemmas \ref{lem shape} to Lemma \ref{lem other k}) of Wilson loop diagrams of this type are related to the corresponding Wilson loop diagram with propagator $p$ removed. \sanote{rephrased, still clear?}
...
Let $W$ be an admissible Wilson loop diagram with $k\geq 1$ propagators, and suppose that $W$ admits one of the configurations described in Lemma~\ref{lem good p}, with $p$ and $q$ (if $q$ exists) both of length 2. Let $V$ be $W$ with $p$ removed.  Then we can express $I_*^W$ in terms of $I_*^V$ as follows:
  ...
  \begin{align*}
    ...
    \begin{cases}
      ...
      I_k^{V}\cup\{n-2\} & \qquad \qquad \qquad \qquad \text{if $n-3\in I_k^{V}$ (Case 2)}
    \end{cases} \\
    & \text{for $1<k<n-2$}
  \end{align*}
\end{lem}

\sanote{Add exposition in this section and in intro about how this allows one to talk about what happens to a postriod cell as one adds a propagator. These results aren't enough, but a starting point, and certainly not done in this paper}
...
\end{figure} \sanote{moved figure before theorem 4.4 so that we can just refer to generically}
\begin{proof}

We first consider $I_1^{W}$, i.e. the set obtained by applying the Grassmann necklace algorithm to $W$ with starting vertex 1\note{I don't find this i.e. helpful at this point, is it an old thing that can go or do others find it useful?} \sanote{agreed, i vote to cut}. Note that $n-3 \not\in I_1^{V}$: \hlfix{for Case 2 this is clear from the diagram (since Algorithm {\ref{alg:put GN on WLD}} would force $q$ to contribute no later than vertex $n-4$),}{why? isn't obvious to me without several cases, and even then} while for Case 1 it follows directly from Lemma \ref{lem no fourth vertex}. Therefore when we start at vertex 1 and apply the Grassmann necklace algorithm to $W$, $p$ is the \st{only remaining unassigned propagator when we reach vertex $n-3$}{\color{violet}last propagator to contribute}, and so we have $ I_1^{W}(p) = n-3$ and $I_1^W = I_1^V\cup \{n-3\}$.

Next consider $I_n^{W}$. From the figure we see that in both cases we have $I_n^{W}(p) = n$. \hlfix{Therefore, the second through $k^{th}$ propagators contributing to $I_n^W$ are the same as the ordering in $I_n^V$. Since $n$ is non-supporting in $V$, $I_1^V = I_n^V$, we have $I_n^{W} = I_1^{V} \cup \{n\}$}{Alt. argument.  Can ignore.} At vertex 1 of the algorithm $I_n$, the unassigned propagators are exactly those that appear in $V$. Therefore the algorithm continues as in $I_1^{V}$ (recall Remark~\ref{rmk algorithm locally same} about locally identical propagator configurations), i.e. we have $I_n^{W} = I_1^{V} \cup \{n\}$. By a similar argument, we must have $I_{n-1}^{W} = I_n^{V} \cup \{n-1\}$.

Now consider $I_{n-2}^{W}$. If $n-2\not\in I_{n-2}^{V}$ \hlfix{by Corrollary {\ref{no coloops}}  $n-2$ is non-supporting in $V$. Thus we are in Case 1, with ${I_{n-2}^W(p) = n-2}$. By the arguments above ${I_{n-2}^{W} = I_{n-2}^{V}\cup \{n-2\}}$}{Alt. Argument. Can ignore} (i.e. the vertex $n-2$ supports no propagators in $V$), then ${I_{n-2}^W(p) = n-2}$ and this assignment of $p$ does not affect the rest of the construction of $I_{n-2}^{V}$, so we obtain ${I_{n-2}^{W} = I_{n-2}^{V}\cup \{n-2\}}$ as above.

On the other hand, if $n-2\in I_{n-2}^{V}$ then we must be in Case 2 of Figure~\ref{fig special p} and $I_{n-2}^V(q) = I_{n-2}^W(q) = n-2$ \st{(since $q$ is always the clockwise-most propagator supported on vertex $n-2$; see Figure {\ref{fig special p}})}. If $n-1 \not\in I_{n-2}^{V}$, then $I_{n-2}^W(p) = n-1$ and then the algorithm proceeds identically to $I_{n-2}^{V}$ for the remainder of its steps; thus $I_{n-2}^{W} = I_{n-2}^{V}\cup \{n-1\}$ in this case.

Finally, if $n-2,n-1 \in I_{n-2}^{V}$ then $I_{n-2}^W(q) = n-2$ and $I_{n-2}^W(p) = n-1$ as above, but \st{this is different to what occured in $I_{n-2}^{V}$}{\color{violet}but some other propagator must contribute $n-1$ to $I_{n-2}^{V}$} so we cannot use the same argument as above. When the algorithm $I_{n-2}^W$ reaches vertex $n$, only the propagators $p$ and $q$ have contributed; thus we proceed as in the construction of $I_n^{V}$ but without propagator $q$ by Remark \ref{rmk algorithm locally same}. By Lemma~\ref{vertex cyclic int lem} we know that $I_n^V(q) = n-5$ \sanote{it would be nice if this were easier to read from lemma without digging through proof}, and from \st{the diagram }{\color{violet} Remark \ref{clockwise ordering rem} } we see that the only way this could occur is if $q$ was the last to contribute to $I_{n-2}^V$. %all other propagators in $V$ had already been assigned when we reached vertex $n-5$; thus $q$ was the final propagator contributing to $I_n^{V}$. 
...
Recall that $I_1$ determines the shape of the Le diagram: specifically, the elements of $I_1$ label the rows of the diagram, working from top right to bottom left. By the arguments in Lemma {\ref{lem I}} we know that \st{${n,n-1,n-2, n-3 \not\in I_1^{V}}$}\sanote{Only see $n-3$ in Lemma 4.4?} {\color{violet} $ n-3 \not\in I_1^{V}$. By Lemma \ref{lem no fourth vertex}, $n, n-2 \not \in I_1^V$. Since $n-1$ is non-supporting in $V$, by Corrollary {\ref{no coloops}}, $n \not\in I_1^{V}$.} These are the leftmost four columns of the diagram for $V$.  From Lemma \ref{lem I} we know that that $I_1^{W} = I_1^{V}\cup \{n-3\}$. Therefore, the right hand boundary of the shape of $V$ is the same as the right hand boundary of the shape of $W$ except that $W$ has one additional row of 3 boxes while $V$ has an additional column in the $n-3$ position; that is, an extra column fourth from the left. 
\end{proof}
\begin{figure}
...
As illustrated in Figure~\ref{fig Le}, the pieces of the Le diagrams of $V$ and $W$ will be called $\mathcal{A}$ and $\mathcal{B}$ in what follows. Explicitly, $\mathcal{B}$ is the set of columns with labels $v < n-3$ and $\mathcal{A}$ is the set of columns labeled by $n$, $n-1$ and $n-2$ in $V$. In $W$, $\mathcal{A}$ consists of these columns without the last row. Over the course of the next few lemmas we will prove that the plusses in the $\mathcal{B}$ parts of each diagram are identical, and that the relationship between the plusses in the two $\mathcal{A}$ regions can be described explicitly.
...
Let $V$ and $W$ be as in Lemma~\ref{lem I}.
Then \hlfix{$I_n^{W}$}{Does $I_n^W$ actually yeild any of the $+$s in $I_n^V$?} and $I_{n-1}^{W}$ together yield the same plusses as $I_n^{V}$ did, along with two extra plusses which appear in the leftmost two boxes of the bottom row of the Le diagram of $W$.
\end{lem}
...
Also by Lemma~\ref{lem I} we have $I_{n-1}^{W} = I_n^{V} \cup \{n-1\}$. Recall from the proof of Lemma \st{{\ref{lem I}}} {\color{violet} {\ref{lem shape}} }\sanote{change back if actually in 4.4} that $n-3,n-2,n-1,n \not\in I_1^V$; therefore $I_1^W\setminus I_{n-1}^W = (I_1^V \setminus I_n^V) \cup\{n-3\}$, and $n-3$ is maximal in this set. Similarly, 
\[I_{n-1}^W \setminus I_1^W = (I_n^V \setminus I_1^V) \cup \{n-1\} \subseteq \{n-1,n\},\]
where the final inclusion follows from the definition of Grassmann necklace\sanote{why only $\{n-1,n\}$? I'd believe an inclusion the other way}. In particular, $n-1$ is minimal in this set, so Algorithm~\ref{alg:GN to Le} yields a plus in the $(n-3) \rightarrow (n-1)$ position (i.e. the second box in the bottom row of the Le diagram of $W$) \hlfix{along with any plusses yielded by $I_n^V$.}{display seems to imply that there is exactly one such other $+$. Is this correct?}
\end{proof} 
...
\begin{proof}
If $n-2\not\in I_{n-2}^{V}$ then $I_{n-1}^{V}=I_{n-2}^{V}$ by definition, and by Lemma~\ref{lem I} we have the equality ${I_{n-2}^{W} = I_{n-1}^{V} \cup \{n-2\}}$.  Note that $n-3\not\in I_{n-2}^{V}$ by Lemma~\ref{lem no fourth vertex}.  Therefore the \st{paths controlling the} plusses contributed by $I_{n-2}^{W}$ are exactly the \st{paths for}{\color{violet} plusses from}\sanote{isn't this from Alg.\ref{alg:GN to Le} which says nothing about paths?}  $I_{n-1}^{V}$ along with the $(n-3)\rightarrow (n-2)$ path.  This gives the statement of the lemma.
\end{proof}
...
\begin{proof}
  By Lemma~\ref{lem I} we have $I_{n-2}^{W}= (I_{n}^{V} \setminus \{n-5\})\cup \{n-1,n-2\}$ and $I_1^W = I_1^V\cup \{n-3\}$. Furthermore, we are necessarily case 2 of Figure~\ref{fig special p}, with a further propagator $r$ ending on the edge $n-1$, satisfying $I_{n-2}^V(r) = n-1$ (that is, $r$ is the clockwise most on this edge). \st{We can see from the location of $q$ that $n-2\not\in I_{n-3}^{V}$} and that $n-5$ is the label of the last row in $\mathcal{B}$.\sanote{is there an easy way to see these? Why can't $I_1^V(q) = n-4$, which would make this the last row of $\mathcal{B}$} {\color{violet}From the figure we see that $I_{n-3}^V = n-3$. In particular, this means that $n-2 \not \in I_{n-3}^V$. } \st{Also note that by Lemma {\ref{vertex cyclic int lem}} and Corollary {\ref{no coloops}} we must have } {\color{violet} Since $r$ is the clockwise most propagator on edge $n-1$, $I_n^V(q) = n$, i.e.} $n\in I_n^V$.Thus this section of the Grassmann necklace of $V$ looks like
  ...
  \end{figure} \sanote{can we put the diagrams labels in bold or set them apart somehow so that it is easier to see? Esp since they are located in a different part of the diagram for $I_n^V$?}
  
  Using this information we can determine the symmetric difference of $I_{n-2}^{V}$ and $I_1^{V}$.  Note that $I_{n-2}^V\setminus I_1^V$ and $I_1^V\setminus I_{n-2}^V$ each have size $\leq 3$ by the definition of a Grassmann necklace. We have $n-1, n-2 \in I_{n-2}^{V} \setminus I_1^V$ for certain, and the third element (if it exists) must be $n$. On the other hand we have $n-5 \in I_1^{V} \setminus I_{n-2}^V$, along with at least one and at most two other elements.  Call the larger (possibly only) of these other elements $a$ with $n-5 > a$. If there are two such elements, call the smaller $b$ such that $n-5>a>b$.  
...
\hlfix{Recall that $n-5$ is the largest row index}{why not $n-4$? (also asked above)} in the Le diagram of $V$, and $n-5 \in I_1^V \setminus I_{n-4}^V$ so $I_{n-4}^V$ must contribute a path starting at $n-5$. Since $n-4 \in I_{n-4}^V$ by the location of the propagator $q$, we must also have a path ending at $n-4$. Since the paths cannot cross, this implies that $I_{n-4}^V$ must contribute a $(n-5) \rightarrow (n-4)$ plus, and hence cannot contribute a $(n-5)\rightarrow (n-1)$ plus. This proves point (5) and completes the proof of the lemma.
\end{proof}
\sanote{may want a chance to reread this lemma after questions have been answered} 
...
\begin{proof}
  ...
  First note that a Wilson loop diagram $W$ with one propagator supported on vertices $i<i+1<j<j+1$ has a Le diagram consisting of a single row with $n-i$ boxes. The columns are labeled from left to right by $n, \ldots, i+1$. By Algorithm~\ref{alg:GN to Le} there are plusses in the $i+1$, $j$, and $j_1$ positions. \sanote{changed for grammar and notational consistency. check}

  Now consider Wilson loop diagrams with $k>1$ propagators.  By Lemma~\ref{lem uncovered} it suffices to prove the result for weakly admissible Wilson loop diagrams with $k$ propagators and no non-supporting vertices.  By Lemma~\ref{lem dihedral} it suffices to prove the result for at least one Wilson loop diagram from each dihedral orbit. In particular, consider $W$, which after possible dihedral transformation, has a propagator $p$ with the properties in Lemma~\ref{lem good p}.  

  We make one further simplification: if our $W$ is in Case 2 of Figure~\ref{fig special p} and $n-1$ supports only $p$ but $n-2$ supports at least one other propagator\sanote{?? see para below}, then flip $W$ on the line perpendicular to the $(n-2){th}$ edge to obtain a diagram with the configuration of Case 1. This eliminates the possibility that we could have $n-2 \in I_{n-2}^W$ but $n-1 \not\in I_{n-2}^W$. 

{\color{violet} I'm very confused about the previous paragraph. I thought that by constuction, in case 2, there can be exactly 2 propagators ending on edge $n-3$, $p$ and $q$. I also thought the distinguising feature between the two subcases of Figure 10 case 2 in Lemma 4.4 $I_{n-2}^W$ was the presence or absense of a propagator $r$ ending on edge $n-1$. If $r$ exists, $n-1 \in I_{n-2}^V$ and if  $r$ doesn't exist, $n-1 \not \in I_{n-2}^V$.}
...
In this section, we investigate the poles of the integrand for $I(W)$, that is $R(W)$ as defined in Definition~\ref{def R(W)}. Since SYM $N=4$ is a finite theory \cite{Alday:2007hr, ParkeTaylorids}, the scattering amplitudes (and thus the holomorphic Wilson loops $\,\mathfrak{W}_{k,n}$) are finite. That is, for a fixed set of external particles $\cZ$, \bas \sum_{W \text{ s.t. } |\cP| = k} I(W)(\cZ) <\infty \;.\eas In otherwords, all poles of the intergrands must cancel. As a step towards this, in this section, we prove some results about the structure of the denominator of the integrand, $R(W)$.

The results of Section~\ref{sec GN algorithm} allow us to relate the \hlfix{position of propagators}{ending edges?} in a Wilson loop diagram $W$ to minors of $C(W)$, $R(W)$. \sanote{Do we need to fix an ordering of rows of $C(W)$, or does that come out in the wash?}

The main result of this section is Theorem~\ref{thm denom}, which expresses the denominator $R(W)$ in terms of the Grassmann necklace of $W$. \sanote{Put something about this in the motivation for why WLD $\rightarrow$ GN algorithm is useful}This simplifies the computation of $R(W)$ and allows us to directly relate the poles of the integral to the combinatorics of the diagram.
...
\begin{itemize}
  \item For each $i \in [n]$, we construct a factor $r_i$ as follows:
    \begin{itemize}
      ...
      \item Let $r_i$ be the determinant of the $|S_i|\times |S_i|$ minor of $C(W)$ with rows indexed by $S_i$ and columns indexed by $I_i(S_i)$.  {\color{violet} If $S_i = \emptyset$, set $r_i = 1$}\sanote{a version of this was commented out, put back in}%$\Delta_{I_i}$ with all variables from rows associated to $p\not\in S_i$ set to $1$.
    \end{itemize}
...
As we discuss the algorithm it will also be useful to have the notation $\Delta_{I_i}$ for the determinant of the $k\times k$ minor of $C(W)$ with columns indexed by $I_i$. {\color{violet} In particular, one summand of this determinant is given by $\prod_{p \in \cP}x_{p, I_i(p)}$. The other summands are (up to sign) given by other bijections between the propagator set $\cP$ and the vertex set $I_i$.}
...
Finally, we can suppose that $W$ is minimal in number of propagators with an irreducible factor of degree $\geq 3$. \sanote{can we break footnote inot a couple sentences?}
...
\begin{scope}
	...
	\draw[propassignment,gray] (\zero + 1.6*\step:\radius) to[bend left = \arrowangle] (\zero + 1*\step:\radius); %p
...
\begin{tikzpicture}[baseline=(current bounding box.east),rotate=-20]
	...
	\draw[propassignment,gray] (\zero + 1.45*\step:\radius) to[bend left = \arrowangle] (\zero + 1*\step:\radius); %p
	\draw[propassignment,gray] (\zero + 1.61*\step:\radius) to[bend right = \arrowangle] (\zero + 2*\step:\radius); %q 
	\draw[propassignment,gray] (\zero + 7.7*\step:\radius) to[bend left = \arrowangle] (\zero + 7*\step:\radius); %blue
	\begin{scope}
		\clipcenterarc(0,0)(\startpoint-10:\endpoint+10:\radius)
...
\end{tikzpicture}
\]
  \caption{The two cases in the proof that no factors of $\Delta_{I_i}$ have degree 3 or more.{\color{violet}First picture: I thought $I_i(p) = i$. Doesn't the arrow say that $I_i(p) = i-1$?}}\label{fig no big factors}
\end{figure}

\textbf{Case 1}: Suppose $p = (i-1, m)$. Then $p$ has one end on $i-1^{th}$ edge and $I_{i+1}(p) = m$ by Lemma~\ref{vertex cyclic int lem}.  \sanote{again, it would be nice if this could be read off from a statement of a result somehwere}.

Let $S = \cP_{in}(p)$ be the set of propagators inside $p$ in the $<_i$ order. {\color{violet} By Remark \ref{rmk algorithm locally same},} $I_i$ and $I_{i+1}$ can only differ once $p$ contributes to $I_{i+1}$, so $I_i(q) = I_{i+1}(q)$ for each $q \in S \backslash \{p\}$. Thus if a propagator contributes $m$ in $I_i$ then it must lie outside $p$.

\hlfix{By Remark {\ref{clockwise ordering rem}}, if $I_i(q) = m$, then it must be the clockwise most propagator supported on $m$ that has not yet contributed to $I_i$, i.e. is lies in $\cP_{out}(p, <_i)$.}{more direct than above para. Alt. only. Don't need to adopt. (keep dfn of $S$.)}

If neither $m$ nor $m+1$ appear in $I_i$ then by Corollary~\ref{no coloops} $V(p) \cap I_i = \{i\}$, and so the row of $p$ in the matrix of $\Delta_{I_i}$ has only one nonzero entry; hence $\Delta_{I_i}$ has a linear factor contributed by $p$ and $i$, which is a contradiction to the fact that $i$ contributes to $f$.  So we must have at least one of $m$ and $m+1$ in $I_i$.  However, all propagators in $S$ contribute to $I_{i}$ strictly before $m$ {\color{violet} by Corrollary \ref{GN alg well defined}}, so the matrix giving $\Delta_{I_i}$ {\color{violet} where the rows appear in the order defined by $I_i$?} has the form
\[
...
On the other hand, if no vertex $\geq_i j$ is in $I_i$ then $\Delta_{I_i} = \det A$.  Looking in more detail into $A$, note that the only vertices supporting both $p$ and $q$ which belong to $I_i$ are $i$ and $i+1$. Hence
\[
...
\item \sanote{changed $b$ to $a+1$ here.} Suppose propagators $p$ and $q$ are supported on a common edge  $(a, a+1)$, with $I_i(p)=a$ and $I_i(q)=a+1$.  Let $x_{p,a},x_{p,a+1},x_{q,a},x_{q,a+1}$ be the associated variables in $C(W)$. For any fixed bijection $\sigma$ from $\cP-\{p,q\}$ to $I_i -\{a,a+1\}$ for which each propagator is supported on its image under the bijection, we can extend $\sigma$ to a bijection from propagators to $I_i$ in two ways: either $p\mapsto a$ and $q\mapsto a+1$ or $p\mapsto a+1$ and $q\mapsto a$.  The sum of the contributions of all these bijections to $\Delta_{I_i}$ is therefore the product of $x_{p,a}x_{q,a+1}-x_{p,a+1}x_{q,a}$ {\color{violet} By the arguments in the proof of the claim in part 2 of this theorem} \note{Could this use a ref back to where we discuss this kind of arugment in more detail.}\sanote{see violet?} with the minor coming from $\cP-\{p, q\}$ and $I_i - \{a,a+1\}$.  Since there is no cancellation of terms in the expansion of $\Delta_{I_i}$, if any other terms appear then they will cause a factor which is not in the form described in the previous part.  Therefore no such terms exist and $x_{p,a}x_{q,a+1}-x_{p,a+1}x_{q,a}$ is a factor of $\Delta_{I_i}$. 

Now let $f$ be a quadratic factor of $\Delta_{I_i}$.  By part (2) we know that $f$ is a $2\times 2$ minor coming from two propagators, call them $p$ and $q$, and two vertices, call them $a <_i b$.  It remains to show that \st{$a$ and $b$ are adjacent} {\color{violet}$b = a+1$}.  From this we can conclude that $p$ and $q$ each have one end on $(a,a+1)$. \st{, as any other way for both $p$ and $q$ to be supported on two consecutive vertices would contradict noncrossing or the density requirement of admissibility.}
...
Now consider the case where \st{$a<_i b$}{\color{violet}$I_i(q) = a+1$}, and suppose for contradiction that $q \not\in S_i$, i.e. that $I_{i-1}(q) = a+1$. Since $I_{i-1}(p) \neq a$, there must be some other propagator $s$ with $I_{i-1}(s) = a$ (else $q$ contributes $a$ to $I_{i-1}$). This propagator cannot lie on edge $(a,a+1)$ since, by Lemma~\ref{vertex cyclic int lem}, \hlfix{we must have $I_i(s) \in \{a, a+1\}$}{is this from examing the cyclic intervals burried in the proof?}, contradicting the fact that $I_i(p) = a$ and $I_i(q) = a+1$; thus $s$ has an end on $(a-1,a)$ and is inside $p$ in the $<_k$ order where $p=(k,a)$.
...
Next we need to show that $R$ is squarefree.  Suppose $f^2|R$.  If $f$ is a linear factor, say from associating propagator $p$ to vertex $a$, then there must be two distinct points in the Grassmann necklace algorithm where $p$ changes from not being associated to vertex $a$ to being associated to vertex $a$.  This contradicts Lemma~\ref{vertex cyclic int lem}.  Now suppose $f$ is a quadratic factor, say from propagators $p$ and $q$ supported on the edge $(a, a+1)$ with $p$ clockwise to $q$ on the edge.  In this case it is not possible for any $I_i$ to associate $p$ to $a+1$ and $q$ to $a$.  Furthermore, we know by part (4) that $p$ changes from not being associated to $a$ to being associated to $a$ if and only if $q$ changes from not being associated to $a+1$ to being associated to $a+1$.  Thus $f^2|R$ implies that twice in the Grassmann necklace $p$ must change from not being associated to vertex $a$ to being associated to vertex $a$. This is again a contradiction, and so $R$ is squarefree.
...
We are now ready to prove the main theorem of this section. We use the facts about $\Delta_{I_i}$ and $R$ above to show that the denominator of the integrand of $I(W)$ is given by Algorithm \ref{alg WLD to denom via GN}.
