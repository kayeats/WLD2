...
From $C(W)$ we obtain the matroid $M(W)$ associated to $W$, which is simply the matroid represented by $C(W)$: its ground set is the set $V$ of vertices of $W$, and the independent sets are exactly the sets $U \subseteq V$ such that the columns of $C(W)$ indexed by $U$ are linearly independent. 

It was shown in \cite{wilsonloop} that $M(W)$ is actually a positroid whenever $W$ is admissible; the matrix $C(W)$ therefore parametrizes a subspace $\Sigma(W)$ of the totally nonnegative real Grassmannian $\Gr(k, n)$. 

Many geometric and combinatorial techniques exist to study the behavior of positroids in $\Gr(k,n)$; more detail on this is given in Section~\ref{sec:positroid background} below. Our perspective in this paper is to study the Wilson loop diagrams by studying the positroids they define.
