...
Note that by Remark \ref{rem n-2 case}, we only need to consider when $W$ is a diagram with one of the two configurations depicted in Figure~\ref{fig special p}, and if $n-2 \in I_{n-2}^V$ then $n-1 \in I_{n-1}^V$ as well. Let $V$ be $W$ with $p$ removed. 
 ...
 The pairing is defined as follows:
  ...
  \item Every square containing in a plus in the leftmost two columns (the $n$ and the $n-1$ columns) of $\mathcal{A}$ for $V$ also contains a plus in $W$, so these plusses are also paired with themselves.
