...
Specifically with $V$ and $W$ as in Lemma~\ref{lem I} and with $n-2, n-1\in I_{n-2}^V$ (so we are necessarily in Case 2 and $n-1$ supports at least one additional propagator not shown in the figure) we have
  ...
  where the first two transitions follow by Remark~\ref{rmk cyclic} along with the fact that $n-3$ is a supporting vertex in $V$.  The third transition follows from the definition of Grassmann necklaces and the fact that $n-1\in I_{n-2}^V$ by hypothesis.  The ``something'' could be $n$ or some element of $I_1^V$.  

  

  {\note{converge following two fragments into a coherent thought} \color{blue} However, this ``something'' cannot be $n-1$, $n-2$, or $n-3$, because as noted in Lemma~\ref{lem I} these elements do not belong to $I_1^V$, and it also cannot be $n-4$: we saw that $I_{n-1}(q) = n-5$ by Remark~\ref{rmk cyclic}, and

   because the only thing that could contribute $n-4$ is $q$ but if $I_n^V(q)=n-4$ then the equation for $I_{n-2}^W$ from Lemma~\ref{lem I} would tell us that $n-4\in I_{n-2}^W$ but this is impossible as $I_{n-2}^W(q)=n-2$ but no other propagator than $q$ can contribute $n-4$. }

  We cannot have $I_1^V = I_n^V$, since the $n-2,n-1 \in I_{n-2}^V$ hypothesis ensures that the $(n-1)$th edge supports at least one propagator even in $V$.  The last transition of \eqref{eq necklace} is then simply a consequence of the general definition of Grassman necklaces, but it will be useful later to observe that the second ``something'' could be $n-4$.
