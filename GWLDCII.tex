...
Thus $W$ has one of the two configurations depicted in Figure~\ref{fig special p}, and by Remark~\ref{rem n-2 case} we can further assume that if $n-2 \in I_{n-2}^V$ then $n-1 \in I_{n-1}^V$ as well. Let $V$ be $W$ with $p$ removed. 
 ...
 The pairing is defined as follows:
  ...
  \item Every square containing a plus in the leftmost two columns (the $n^{th}$ and the $(n-1)^{th}$ columns) \todo{Put note in paper and response to referee --- generally turn $n$ to $n^{th}$ throughout M.60 -- I disagree and for reasons I can articulate} of $\mathcal{A}$ for $V$ also contains a plus in $W$, so these plusses are also paired with themselves.
