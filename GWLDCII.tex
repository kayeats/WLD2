...
Generally speaking the propagators are undirected, so $(i,j)$ and $(j,i)$ represent the same propagator. As a general convention, we write $p = (i,j)$ with  $i < j$ relative to the first vertex. However, in Section~\ref{sec:propagator configs} we will need to give the propagators a direction (in order to refer to vertices ``inside'' and ``outside'' the propagator); in this setting, we impose the convention that $p= (i,j)$ denotes a propagator flowing from $i$ to $j$, and $p= (j,i)$ denotes the \emph{same} propagator flowing in the opposite direction.
...
\begin{enumerate}
\item For $p = (i,j) \in \cP$, let $V(p) = \{i, i+1, j, j+1\}$ denote the set of vertices supporting $p$. For a set of propagators $P \subseteq \cP$, define 
\[V(P) = \bigcup_{p \in P} V(p)\]
to be the vertex support of $P$.
\item For $U \subseteq V$, the set of propagators supported on $U$ is denoted by
\[\Prop(U) = \{ p \in \cP\ |\ V(p) \cap U \neq \emptyset \}.\]
\item Vertices of $W$ that do not support any propagators are called non-supporting. The set of all non-supporting vertices in $W$ is denoted $F(\emptyset)$. 
\end{enumerate}
\end{dfn}
...
Let $W = (\cP, V)$ be an admissible Wilson loop diagram. Then $W' = (Q,U)$ is a subdiagram of $W$ if 
\[ Q \subseteq \cP \quad \text{ and }\quad V(Q) \subseteq U \subseteq V\;.\]
\end{dfn}
...
The first condition bounds the total number of propagators in the diagram, while the second limits how densely propagators can be fitted into any part of the diagram; in particular, it prohibits both propagators that connect two adjacent edges and parallel propagators that start and end on the same pair of edges. The third condition simply requires that propagators are non-crossing. 

These conditions are imposed by the physical interpretation of Wilson loop diagrams (see for example [ref]). We therefore restrict our attention in this paper to admissible Wilson loop diagrams and their subdiagrams only. 


Note that a subdiagram $(Q,U)$ of an admissible Wilson loop diagram need not be admissible itself: it inherits conditions 2 and 3 automatically, but we could have $|V(Q)| = |Q| + 3$. We call diagrams of this type {\bf weakly admissible}; in the context of this paper, they occur only when we take an admissible diagram and delete one or more non-supporting vertices.


Due to the physical motivation behind these diagrams, we can associate to each Wilson loop diagram $W$ a matrix $C(W)$, which is defined by

\begin{equation} C(W)_{p,q} = \begin{cases} c_{p,q} & \textrm{ if } q \in V(p) \\
0  & \textrm{ if } q \not \in V(p)  \end{cases}
\;. \label{C(W) dfn}\end{equation}

The entries $c_{p,q}$ are non-zero real variables. The columns of $C(W)$ correspond to the vertices of $W$ listed in their linear order, and the rows correspond to the propagators of $W$. $C(W)$ is therefore only defined up to rearrangement of the rows, which will not matter in this setting since we will only interpret it as carving out a family of elements of the $|\cP| \times |V|$ real Grassmannian.

\begin{eg}
Consider the admissible Wilson loop diagram 
\[W = (\{(1,4), (2,4), (5,7), (5,8)\},[8]),\]
which is represented graphically by the diagram
\[W = \begin{tikzpicture}[rotate=67.5,baseline=(current bounding box.east)]
    ...
    \drawprop{5}{1}{7}{0}
		\end{scope}
	\end{tikzpicture}\]
Ordering the propagators as listed above, we obtain the associated matrix
\[ C(W) = \left(
...
The matrix $C(W)$ parametrizes a subspace of $\Gr(k, n)$ as shown in \cite{wilsonloop}; we denote this subspace by $\Sigma(W)$. 

From $C(W)$ we obtain the matroid $M(W)$ associated to $W$, which is simply the matroid represented by $C(W)$: its ground set is the set $V$ of vertices of $W$, and the independent sets are exactly the sets $U \subseteq V$ such that the columns of $C(W)$ indexed by $U$ are linearly independent. More detail on this is given in Section~\ref{sec:positroid background} below.


The Wilson loop diagram also define a volume form on $\Sigma(W)$, namely 
\[ \Omega(W) = \frac{\prod_{r=1}^{|\cP|} \prod_{v \in V_{p_r}} \textrm{d}c_{p_r}}{R(W)} \;. \]
The interested reader is referred to [ref] for more information about the volume form. Of interest to us in this paper is the denominator $R(W)$, which we define next.

\begin{dfn}\label{def R(W)}
Let $W = (\cP,V)$ be an admissible Wilson loop diagram, and fix an edge $e \in V$. Let $\{p_1 \ldots p_r\}$ be the propagators with one end lying on edge $e$, ordered so that $p_1$ is closest to the vertex $e$, $p_r$ closest to $e+1$, and $p_i$ is closer to $e$ than $p_{i+1}$ is. Then we define 
\[ R_e =  c_{p_1,e+1} \prod_{j= 1}^{r-1} \left((c_{p_j,e} c_{p_{j+1},e+1} - c_{p_{j+1},e} c_{p_{j},e+1} ) \right) c_{p_r,e}\;.\]
Note that if $r = 1$ (i.e. there is exactly one propagator lying on edge $e$) then $R_e = c_{p,e} c_{p,e+1}$. The denominator $R(W)$ is defined to be the product of all of the $R_e$, i.e. 
\[R(W) = \prod_{e \in V} R_e.\]
\end{dfn}
It is this polynomial $R(W)$ that we characterize in Section~\ref{sec poles}, where we give a simple algorithm to read $R(W)$ directly from the Wilson loop diagram.
...
By \cite[Theorem 6.5]{Postnikov}, the set of all Le diagrams that fit within a $k\times(n-k)$ rectangle is in 1-1 correspondence with the positroid cells of $\Gr(k,n)$. The dimension of a positroid cell is equal to the number of $+$ squares in its Le diagram \cite[Theorem 6.5]{Postnikov}.
