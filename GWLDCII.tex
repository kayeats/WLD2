...
Below, we show that the polynomial $R$ associated to $W$ by Algorithm~\ref{alg WLD to denom via GN} is equal to the denominator $R(W)$ from Definition~\ref{def R(W)}, and is the radical\note{radical} of the product $\prod_{i=1}^n \Delta_{I_i}$. First, however, we give a worked example of Algorithm~\ref{alg WLD to denom via GN}.
