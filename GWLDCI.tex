...
An equivalent definition of $F(P)$ \note{Sian note to self: doublecheck that these {\em are} equiv definitions now} is \bas F(P)  = \big(V(P) \setminus V(P^c) \big) \cup F(\emptyset) \;,\eas 
...
\note{sian note to self: come back and package this as a remark, and make sure it follows from the previous result}
Note that this means that the set $F(\emptyset)$ of non-supporting vertices is the maximal subset of vertices of $W$ of rank $0$. That is, it is the unique flat of rank $0$ in $M(W)$. This also implies that every element of $F(\emptyset)$ is a circuit, and all other circuits of $M(W)$ contain at least two elements.
...
% \end{enumerate}
% \end{lem} \note{I'd like to attach this to the "generic facts about matroids" in lemma 2.2?}
% \begin{proof}
...
via equation \eqref{eq:equality in lem decompose flat}. Therefore $B\cup\{w\}$ is a dependent set, which implies that $B\cup \{w\}$ contains a circuit $D$. We must have $w \in D$ (since $B$ was an independent set), and \hlfix{$D \subseteq F(\Prop(C))$}{I know we've talked about this before and I can't remember why this is true} $\subseteq F$. Since $C$ was the union of all circuits in $F$, we finally obtain $w \in C$, which is a contradiction. This completes the proof of (1).
...
\note{sian note to self: editing has reached this point}
...
It turns out that this defines a class of combinatorially equivalent polytopes, which can be proved by giving a realization.  There are many interesting realizations of the associahedron \cite{CSZinequivalent}; the following will be the one which we will use.  See Figure~\ref{associahedron5gon} for examples.

\begin{dfn} \label{Andfn}
...
Let $D_1$ and $D_2$ be two dissections of a convex $n$-gon defined by the vertices $x_1, \ldots, x_n$.  Suppose $V_{>3}(D_1) = V_{>3}(D_2)$, but that the $D_1$ and $D_2$ do not have the same set of non-triangle faces.  Then there are four vertices $x_i$, $x_j$, $x_k$, $x_l$ satisfying the following conditions:\begin{enumerate} \item in either $D_1$ or $D_2$ all four vertices are on the same non-triangle face \item in the other disection, there is a diagonal of the dissection such that exactly one of $\{x_i, x_j, x_k, x_l\}$ lies on one side (so the others are either end points of this diagonal or strictly on the other side). \end{enumerate}
\end{lem} 
...
Assume for a contradiction that no quadrilateral as in the statement exists, and let $d$ and $f$ be as above.  Then $f$ must have two vertices strictly on each side of $d$, say $x_i$ and $x_j$ on one side and $x_k$, $x_l$ on the other side (and in that order).  Consider the face in $D_1$ incident to $d$ on the $x_i, x_j$ side of $d$. Call this $g$. Suppose this face is a triangle, then its other vertex cannot be in \st{$[x_i, x_j]$} the cyclic interval bounded by $x_i$ and $x_j$ (i.e. $[x_i, x_j]$) as in this case taking one of the other edges of the triangle $g$ would give a configuration as in the statement.  Suppose then that this face, $g$, is a quadrilateral. Again no other vertex of this face can be in $(x_i, x_j)$ as otherwise again replacing $d$ with one of the other edges of $g$ gives a quadrilateral as in the statement. The same holds if one of the other vertices of $g$ is $x_i$ or $x_j$ but the other is not.  If both the other vertices of $g$ are $x_i$ and $x_j$, then we also get a configuration as in the statement but now with the four vertices being those defining $g$ and the diagonal is one of the edges of $f$. (For instance $d$ can now be the edge connecting $x_l$ to $x_i$ or $x_j$ to $x_k$. Thus we can conclude that none of the vertices of this face are in $[x_i, x_j]$.  

Suppose the face $g$ has one vertex in $(x_j, ,x_k)$ and the rest in $(x_l, x_i)$ (or vice versa). If $g$ is a quadrilateral, its four vertices satisfy the condition with the edge between $x_k$ and $x_j$ in $f$ serving as an example we get a quadrilateral as in the statement again with this face giving the four vertices and the diagonal is a side of $f$.  If $g$ a triangle, let $d'$ be the other diagonal across $f$. These are the first two cases illustrated in Figure~\ref{inductive_step}.

Finally, if the face $g$ has two vertices in $(x_j, x_k)$ and two in $(x_l, x_i)$, let $d'$ be the the other edge of $g$ that forms a diagonal across $f$. In these last two cases (those represented in Figure~\ref{inductive_step}), replace $d$ with $d'$, which also separates $f$ into $x_i, x_j$ and $x_k, x_l$ \hlfix{but is strictly closer to $[x_i, x_j]$ than $d$ was}{why is this the case?}.  The same argument holds for $d'$, but the polygon is finite, so this process cannot go on indefinitely, contradicting that no quadtilateral as in the statement exists.
\end{proof}
