...
Propagators are undirected, so $p = (i,j) = (j,i)$. In previous work, we have used the convention, we write $p = (i,j)$ with  $i +1 \leq j$ relative to the first vertex. \sanote{Susama stopped here}
...
It is sometimes useful to discuss propagators in terms of the edges supporting them (rather than the vertices) so we make the following definition.
...
We now define the notion of an admissible diagram, which imposes certain density and non-crossing conditions on the configuration of the propagators. The motivation for these conditions comes from the link to SYM $N=4$ theory, where the admissible Wilson loop diagrams are exactly those that correspond to $N^kMHV$ diagrams; see for example \hlfix{[ref].}{I don't think we should go into any detail here, but I wanted to emphasise that there is a reason for choosing these particular conditions.}
...
Let $W = (\cP,[n])$ and $W' = (\cP',[n])$, and say that $W$ and $W'$ are related if
\begin{enumerate}
\item There exist exact subdiagrams $(P, V(P)) \subseteq W$ and $(P', V(P')) \subseteq W'$ with $V(P) =  V(P')$.
\item The complementary subdiagrams are identical, i.e. $(\cP \setminus P, V(P)^c) = (\cP' \setminus P', V(P')^c)$.\note{I don't want to change it in case I've just misunderstood, but it seems to me that the vertex sets for the complementary subdiagrams should be $[n]$ not $V(P)^c$ and $V(P')^c$.}
\end{enumerate}
...
\[(\{(5,8), (5,7)\}, \{5,6,7,8,1\}) \text{ and }(\{(5,8), (6,8)\}, \{5,6,7,8,1\})\] respectively. We also have $W_2 \sim W_3$ due to the exact subdiagrams 
\[(\{(1,4), (2,4)\}, \{1,2,3,4,5\}) \text{ and }(\{(1,4), (1,3)\}, \{1,2,3,4,5\}).\] 
However, the difference between $W_1$ and $W_3$ is the union of two exact subdiagrams, which is not itself exact. \end{eg}

As described in the introduction, Wilson loop diagrams come from the study of SYM $N = 4$ in the physics literature and therefore each diagram $W$ is associated to a matrix of data $C(W)$, which we now describe.
