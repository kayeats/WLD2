...
{\color{violet}Note that by Definition \ref{admisdfn}, any single propagator and its support defines an exact subdiagram. These are called trivial sub diagrams.}
...
In this section, we take this analysis further, showing the converse of Theroem [ref] from loc. cit. (see Theorem \ref{same matroid iff equiv}. We also enumerate the number of diagrams that map to an particular matroid, Corollary \ref{number of equiv diagrams}. To do this, we establish a connectection between Wilson loop diagrams and chord diagrams, subsection \ref{sec: polygon partitions}, and prove some matroidal properties of exact subdiagrams in subsection \ref{sec: exact diagram matroidal props}  including that they correspond to uniform matroids (Theorem \ref{exactuniformthm}), and the associated matroid can be written as as a contraction by the complementary propagator flat (Theorem \ref{exact diagrams contractions}).\sanote{stopped here}\sanote{need to discuss positroids, and whether positroids should appear earlier so that they can be discussed here.}
...
\begin{dfn}\label{WLDtriangulationdfn}
  ...
  \item Each propagator of $W$ defines a chord \note{We were originally thinking in terms of chord diagrams hence the language ``chord'' but I think ``diagonal'' would be more natural now.  Any thoughts against before I change it?} edge of $\tau(W)$; specifically,  a propagator $(i,j) \in \cP$ defines a chord connecting the vertices $i$ and $j$ in $\tau(W)$.
  \end{itemize}
\end{dfn}
...
The previous definition makes sense for any two polygon dissections of the same polygon, not just for those in the image of $\tau$.
