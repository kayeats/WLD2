...
Matroid theory relates to the study of Wilson loop diagrams as follows. In \cite{wilsonloop}, Agarwala and Amat show that every weakly admissible Wilson loop diagram with $k$ propagators defines a positroid of rank $k$ on a base set of size $n$, called $M(W)$. In other words, by \cite{Postnikov}, each admissible Wilson loop diagram corresponds to a convex subset of $\Gr(k,n)$, thus connecting the geometry of Wilson loop diagrams with the geometry underlying the Amplituhedron. The independent sets of $M(W)$ can be read directly from the diagram:
...
\ba \rk\big(F(P^c)\big) = \rk(C) + \rk(S)  = |\Prop(C)| + |S| \leq |\Prop(F(P^c))|\;,\label{rank of comp flat} \ea % this equation doesn't really need to be displayed, but it was causing a badbox  
while the inequality comes from the bound on ranks of vertex sets. 

Furthermore, from Remark \ref{alt F(P) rmk}, we see that $\Prop (F(P^c)) \subseteq P^c$. Therefore equation \eqref{rank of comp flat} gives \bas |\Prop (C)| + |S| \leq |P^c|  \;.\eas For ease of future notation, write $Q = \Prop(C)^c$. 
Rearranging and adding $|P|$ to both sides gives \ba |P| + |S| \leq |P| + |P^c| - |\Prop(C)| = |Q| \;.\label{P+Sbound}\ea Taking complements of and rearranging equation \eqref{eq decompose F} we have
...
Since \bas \Prop(C) \subseteq \Prop(F(P^c)) \subseteq P^c \eas taking complements gives $P \subseteq \Prop(C)^c = Q$. In other words, $(P, V(P))$ is an exact subdiagram of $(Q, V(Q))$. Since $(P, V(P))$ is a maximal exact subdiagram, $P=Q$, or $\Prop (C) = P^c$, and thus $S = \emptyset$, forcing $F(P^c)$ to be a cyclic flat. \sanote{the previous proof of (2) (commented out) was making my head spin. I think this is more straightforward, butI might just be rearranging deckchairs.}
...
In \cite{wilsonloop}, the authors show that each weakly admissible Wilson loop diagrams correspond to a postiroid. That is, each diagram correspond to matroids that can be represented by elements of the positive Grassmannians $\Gr(|\cP|, n)$. Any positroid of rank $k$ on $n$ elements defines a subspace of the positive Grassmannians $\Gr(k, n)$, namely the points which represent it \cite{Postnikov}. These subspaces give a CW structure on $\Gr(k,n)$, with each positroid defining a cell.  In other words, there is a map from weakly admissible Wilson loop diagrams, $W$, to matroids, $M(W)$, to positroid cells in the CW complex on $\Gr(|\cP|, n)$.
\note{How is this a def as it doesn't define the cells?  Also, should it be cell singular} \sanote{got rid of dfn}
...
Two faces are \emph{parallel} if one can be mapped to the other by a translation in $\mathbb{R}^d$.  In particular parallel faces must be of the same dimension. \hlfix{Any two vertices are trivially parallel to each other.}{changed. Correct?}
...
This proof is by a symmetrical construction, where we give a proof with respect to a single edge in the face $f$, leaving the rest of the argument as read due to the symmetry.\note{Actually, since it is by contradiction we don't need to do both directions; one direction already gives a contradiction and that's enough} \sanote{agreed. Withdraw previous questions}
