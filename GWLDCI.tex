...
***Some general intro to WLDs perhaps similar to the end of p2 and top of p3 of \cite{wilsonloop}.  For this paper we don't need the integral, so for the moment they are not defined below, but for motivation may want to be glossed a bit here ***

Furthermore, in \cite{wilsonloop}, Agarwala and Amat show that Wilson loop diagrams can be interpreted as positroids, a certain well behaved class of realizable matroids.  This opens up the study of Wilson loop diagrams to techniques from geometry and combinatorics.  We pursue this direction here and in the companion paper ***cite the other paper***.  

In the present paper we focus on the combinatorics and geometry of an equivalence relation on Wilson loop diagrams.  The main results are that Wilson loop diagrams are equivalent if and only if they define the same matroid, that the equivalence can be rephrased in terms of retriangulations in polygon dissections, and from there that the sizes of the equivalence classes can be enumerated, and the set of equivalence classes is related to directions in associahedra.

\subsection{Roadmap}

We proceed in Section~\ref{section overall background} with the required definitions for Wilson loop diagrams (subsection~\ref{section WLD background}) and matroids (subsection~\ref{sec matroid background}.  Wilson loop diagrams will be said to be admissible when they correspond to the physical situation of ***something***, or mathematically according to Definition~\ref{admisdfn}.  With this, the key definition is Definition~\ref{equivdfn} giving the equivalence relation on admissible Wilson loop diagrams.  The connection to matroids is stated precisely in Theorem \ref{thm WLD defines matroid}.
...
The connection between admissible Wilson loop diagrams and polytope dissections suggests a connection to associahedra. First we need some background.
...
It turns out that this defines a class of combinatorially equivalent polytopes, which can be proved by giving a realization.  There are many interesting realizations of the associahedron \cite{}; the following will be the one which we will use.
...
With the choice of $n$-gon and hence of $A_n$ fixed, view the $\tau$ map as taking values in the set of dissections of this particular convex $n$-gon, and so each weakly admissible Wilson loop diagram is associated via $\tau$ with a face (in some dimension) of $A_n$.
...
\begin{proof}
  Let $W$ and $X$ be equivalent.  From results in the paper we're working on we know that $\tau(W)$ and $\tau(X)$ differ from each other only by retriangulations of their triangulated pieces.  
...
\begin{proof}
  ...
  Suppose this never occurs.  Then all the non-triangle faces of $D_2$ which are crossed by a diagonal of $D_1$ must be quadrilaterals and likewise for $D_1$.  This means that any non-triangle and non-quadrilateral faces must be common between $D_1$ and $D_2$.  These common faces are irrelevant for the statement, and so by triangulating any common non-triangle faces, we may assume that $D_1$ and $D_2$ have only quadrilateral and triangle faces.
