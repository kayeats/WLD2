...
Throughout, we take the {\em matroid defined by $W$} to be the matroid $M(W)$ of Theorem \ref{thm WLD defines matroid}. Note from Definition~\ref{def:CWmatrix} that the vertices of the diagram $W$ correspond to columns of the associated matrix $C(W)$. Therefore, for any subset of vertices $V \subseteq [n]$, we can consider the restriction $C(W)|_V$ of $C(W)$ consisting of the columns corresponding to $V$. Since the non-zero entries of $C(W)$ are independent real variables, if $C(W)|_V$ has fewer than $|V|$ rows of non-zero entries then it not have full rank. This is equivalent to saying that if $V$ is a set of columns of $C(W)$ with $|\Prop(V)| < |V|$, then $V$ is a dependent set. Any set of columns containing such a $V$ cannot be independent. Comparing this to Theorem \ref{thm:WLDmatroid}, we see that $M(W)$ is also the matroid realized by $C(W)$. 
...
The key point to note is that the subgraph $t$ depends on how the propagators of $P$ sit inside $[n]$, not on how they sit in \hlfix{the subdiagram $(P,V(P))$, which would have an outer edges between vertices 8 and 1, between 2 and 4, and between 4 and 8, but would not include vertex 3}{Sian replaced $W_P$ with $(P,V(P))$ here and now isn't sure if that was right because this sentence doesn't make sense?}.  In particular, $t$ is not the subgraph of $\tau(W)$ consisting only of edges corresponding to propagators of $P$ (as it may also include some of the outer edges of $\tau(W)$), nor is it the polygon dissection of $(P,V(P))$ viewed as a Wilson loop diagram in its own right (which would have vertex set equal to $V(P)$).
\end{eg}
...
In this section we have characterized the correspondence between Wilson loop diagrams and positroids, showing that Wilson Loop diagrams define the same matroid if and only if they are equivalent (Theorem \ref{same matroid iff equiv}). Corollary \ref{number of equiv diagrams} enumerates the elements of each fiber of the map from Wilson Loop diagrams to positroids.  The obvious next question would be to enumerate the number of equivalence classes.  

We know from examples (e.g. \cite{casestudy}) that \hlfix{even after taking the dimension of the positroid cell into consideration}{Sian: I'm not quite sure what ``taking the dimension into consideration'' means here: do we mean ``even after restricting our attention to cells of dimension $3k$''?}\footnote{it is proven in \cite{generalcombinatoricsII} and \cite{Marcott} that Wilson loop diagrams with $k$ propagators map to $3k$ dimensional positroid cells}, many positroid cells do not appear as admissible Wilson loop diagrams with a fixed number of propagators and vertices. That is, the map from admissible Wilson loop diagrams to positroid cells of the correct dimension in the correct Grassmannian neither injective nor surjective. We do not obtain a formula for the number of cells, but do obtain an alternate characterization of the equivalence classes which is the subject of the next section.

\section{Associahedra and inequivalent Wilson loop diagrams}\label{sec associahedron}

The correspondence between (weakly) admissible Wilson loop diagrams and polygon dissections shown in Section \ref{sec: polygon partitions} suggests a connection to associahedra. In order to explore this connection further, we first need some background, which is given in Section~\ref{sec polytope background}.

%Associahedra can be defined in terms of polygon partitions.
In Section~\ref{sec associahedron results}, we show that the number of equivalence classes of Wilson loop diagrams correspond to the number of non-parallel faces of the appropriate associahedron (Theorem \ref{thm:count inequiv diagrams}). While this section does not give an explicit formula for the number of these faces, we hope that this linkage with a well-studied combinatorial object will shed light on the issue in the future.
...
This proof is by a symmetrical construction, where we give a proof with respect to a single edge in $f$, leaving the rest of the argument as read due to the symmetry.\note{Actually, since it is by contradiction we don't need to do both directions; one direction already gives a contradiction and that's enough}
