...
In recent years, there has been a an active program researching the geometric and combinatorics underlying SYM $N=4$ theory \cite{wilsonloops, amplituhedron, correlahedron, karpwilliams, galashinlam}. This body of work started with the observation that the on shell amplitudes of this theory correspond to the volume of a subspace of a positive Grasmannian, called an Amplituhedron \cite{amplituhedron}. Since then, there has been significant work to study the structure of the amplituhedron, geometrically and combinatorially, for instance \cite{amplituhedron2, karpwilliams, galashinlam}. 

Meanwhile, there has been a different body of work studying SYM $N=4$ theory from the point of view of Feynman integrals \cite{BullimoreSkinner, AldayMaldecena, others}. The first author and Amat uncovered a connection between the Feynman integrals developed in this literature and the positive Grassmannians \cite{wilsonloops}. Others have tried to define a geometric space associated to these integrals in a manner similar to the Amplituhedron \cite{correlahedron, non-orientable}. 

In this paper, we expand on map between Wilson loop diagrams and positroids, first defined in \cite{wilsonloops}. Each Wilson loop diagram, $W$, has associated to it a real-variable valued matrix $C(W)$, which defines positroid, i.e. a matroid that can be realized as an element of $\Gr(k,n)$. The subspace of $\Gr(k,n)$ parametrized by each $C(W)$ is the positroid cell defined by $W$, called $\Sigma(W)$. Lemma ?? of \cite{wilsonloops} identifies that certain Wilson loop diagrams define the same positroid cells. In particular, the authors define a subset of Wilson loop diagrams, called exact diagrams, and show that if two Wilson loop diagrams only differ by an exact subdiagram, they define the same positroid cell. 

In the current work we study exact subdiagrams in greater detail, identifying several matroidal properties. Furthermore, we enumerate how many different Wilson loop diagrams map to any positroid cell, $\Sigma(W)$. Furthermore, we show that counting the number of distinct $\Sigma(W)$, positroid cells that correspond to Wilson loop diagrams, corresponds to counting the number of directions of an associahedron.
