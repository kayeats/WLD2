...
We depict this data as a circle with marked points, called vertices. The vertices are labeled by $V$ (preserving the cyclic ordering). The arc between consecutive vertices are called edges. There are $k$ wavy lines in the interior of the diagram, depicting the proagators, with endpoints on the edges. A propagator, $p =(i,j)$ has one endpoint on the edge between the vertex labeled $i$ and $i+1$ and another endpoint on the edge defined by $j$ and $j+1$. Then we write \bas W = (\cP, V) \;.\eas 
...
The first conditions states that there are at least four more vertices than propagators in an admissible Wilson loop diagram. The second imposes an upper bound on how densely the propagators can be fitted in the diagram. The third ensures that ensures that no propagators cross in the interior of the diagram. \note{fixed? earlier, we had $i + 1 < j$ in the definition. When that was fixed, I didn't propagate forward. Or am I missing something else?}In other words, a Wilson loop diagram, $(\cP, n)$ is admissible if and only if $n \geq \cP +4$, and has no crossing propagators, no propagators of the form $p = (i, i+1)$, and no pairs of propagators that start and end on the same pair of non-adjacent edges.
...
\begin{eg}
The Wilson loop diagram $W$ shown above is exact. There is a single non-supporting vertex in $W$, $F(\emptyset) = \{7\}$. The subdiagram $(\{(2, 4), (1, 4)\}, \{1, 2, 3, 4, 5, \})$ is an exact subdiagram of $W$. For $p = (5, 8)$ $F(p) = \{6, 7,8\}$. 
\end{eg}
...
The set of propagators $P= \{(1,4), (4, 8), (1,8) \}$, define an exact subdiagram of $W$. The vertex set of $t$ are the vertices labeled $\{1, 2,3, 4, 5, 8\}$. The subgraph $t$ is then 
