...
\item For $P \subseteq \cP$, define $F(P) = V(P^c)^c$ to be\footnote{Here, as throughout, we are using the superscript $c$ for the set theoretic complement.} the set of vertices in $[n]$ that do not support any propagators outside the set $P$.
...
\begin{cor} \label{classifyflats}
  If $F$ is a flat of an admissible Wilson loop diagram, it can be written as the disjoint union of a cyclic propagator flat and an independent set: \bas F =C \sqcup S \eas where $C$ the union of all circuits in S. \end{cor}
...
Since $R \subset P$, $V(P) = V(R) \sqcup S$, where $S = V(P) \setminus V(R)$. Since $P$ and $R$ both define exact subdiagrams, we may write $|V(P)| = |V(R)| + |P \setminus R|$, and we have that $S$ is a vertex set of size $|P \setminus R|$. Furthermore, $F(R^c) = F(P^c) \sqcup S$. We will show that $S$ is an independent set in the subdiagram $(P, V(P))$, and thus in $W$. Supposing for the moment that $S$ is independent in $(P, V(P)$ this implies that for any maximal independent set $\mathfrak{B} \subset F(P^c)$, the set $\mathfrak{B} \sqcup S$ is independent in $F(R^c)$. Therefore, \bas \rk (F(R^c)) \geq |\mathcal{B}| + |S| = \rk (F(P^c)) + |S| = |P^c| + |P\setminus R| = |R^c|  \;.\eas By Lemma \ref{lem facts about WLD matroids}, $\rk (F(R^c)) \leq |R^c|$. Thus $\rk (F(R^c)) = |R^c|$ as desired.

It remains to show that $S$ is an independent set in the subdiagram $(P, V(P))$. Suppose not. Then, by Theorem \ref{thm WLD defines matroid} there is a subset $U \subseteq S$ such that $|\Prop(U)| < |U|$. Then the remaining propagators, $P \setminus \Prop(U)$ are supported entirely on $V(P) \setminus U$. That is \bas  V(P \setminus \Prop(U)) = V(P) \setminus U\; .\eas Comparing sizes of these sets, we get \bas |V(P \setminus \Prop(U))| = |V(P)| - |U| = |P| +3 -|U| > |P| +3 -|\Prop(U)| = |P \setminus \Prop(U)| +3\; ,\eas which violates admissibility. Therefore, $S$ is independent in $(P, V(P))$. 
...
The map $\tau$ defined in Definition \ref{WLDtriangulationdfn} assigns a dissection of an $n$-gon to a Wilson loop diagram $W = (\cP, [n])$. Recall that if $|\cP| = k$, then $\tau(W)$ is a dissection with $k$ chords. Therefore, the map $\tau$ associates to each weakly admissible Wilson loop diagram an $n-3 - k$ dimensional face of the associahedron $A_n$. This is illustrated for $n=5$ in Figure~\ref{associahedron5gon}.

\begin{figure}\label{associahedron5gon}
\includegraphics{associahedron5gon}
\caption{An associahedron labelled by polygon dissections.}
\end{figure}
