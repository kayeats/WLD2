...
\begin{dfn}
Given a Wilson loop diagram $W = (\cP, [n])$ and its associated positroid $M(W)$, let $\Sigma(W)$ denote the corresponding positroid cell in the CW complex on $\Gr(|\cP|, n)$.
\end{dfn}
...
\begin{proof}
  Let $W$ and $X$ be two equivalent Wilson loop diagrams.  From Corrollary \ref{equialentretriangulation} we know that $\tau(W)$ and $\tau(X)$ differ from each other only by retriangulations of their triangulated pieces. 
