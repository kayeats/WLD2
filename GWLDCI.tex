...
\begin{thm} \label{thm WLD defines matroid} \cite[Theorem 3.6]{wilsonloop} Any admissible Wilson loop diagram $W =(\cP, [n])$ defines a matroid $M(W)$ with ground set $[n]$. The independent sets are exactly those subsets $V \subseteq [n]$ such that for every subset of vertices, $U\subseteq V$, supports at least as many propagators as vertices in the set, $|\Prop(U)| \geq |U|$. \label{thm:WLDmatroid}\end{thm}
