...
\end{lem}
\begin{proof}
The first part of item 1 is \cite[Equation (9)]{wilsonloop} and surrounding discussion, and the second part is standard matroid theory. Item 2 is \cite[Lemma 3.27]{wilsonloop}. Item 3 is a direct consequence of \cite[Lemma 3.20]{wilsonloop} and the fact that $F(P_1)^c = V(P_1^c)$, and item 5 is given in \cite[Lemma 3.28]{wilsonloop}.
...
Let $W = (\cP, [n])$ be a Wilson loop diagram and $P \subseteq \cP$. If the set $V(P)^c$ has rank $|P^c|$, then the matroid defined by the subdiagram $(P, V(P))$ is equal to the contraction $M(W)/V(P)^c$.
\end{lem}
