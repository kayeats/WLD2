...
Meanwhile, there is a separate body of work studying SYM $N=4$ theory from the point of view of Feynman integrals in twistor space \cite{Adamo:2011pv, Britto:2005fq, Cachazo:2004kj}. These integrals are calculated via Wilson loops. As noted above, the \hlfix{first author and Amat}{At other points, we say "some of us" instead of specifying like this. Choose a convention}\note{Response by Karen: I am the cause of the ``some of us'' and similar; it is the kind of phrasing I usually use, and I like it, but I am not strongly attached to it.} uncovered a connection between the Feynman integrals developed in this literature and positroids, defined  as matroids that are realized as an element of the positive Grassmannian $\Gr(k,n)$ \cite{wilsonloop}. In (separate) papers with the second author and Marcott, this work was extended to study these Feynman integrals in terms of positroid cells and the positive Grassmannians \cite{casestudy, non-orientable}. Others have tried different approaches to define a geometric space associated to these integrals in a manner similar to the Amplituhedron \cite{Amplituhedronsquared, HeslopStewart}. 
...
The main result of Section~\ref{sec equiv} is Theorem~\ref{same matroid iff equiv}, which says that two Wilson loop diagrams define the same matroid if and only if they are equivalent.  In subsection~\ref{sec: polygon partitions} we relate exact subdiagrams of Wilson loop diagrams to triangulations of polygons, and hence show that every Wilson loop diagram can be uniquely decomposed as a collection of exact subdiagrams (Corollary~\ref{uniqueproppartitioncor}). In subsection~\ref{sec: exact diagram matroidal props} we examine the matroidal properties of the exact subdiagrams, and prove that a subdiagram of a Wilson loop diagram defines a uniform matroid if and only if the subdiagram is exact (Theorem \ref{exactuniformthm}). This uniform matroid is in fact equal to the dual matroid restricted to the exact subdiagram (Remark \ref{remark exact dual restiction}).  Finally, subsection~\ref{sec: matroids and equivalence} is devoted to the proof of Theorem~\ref{same matroid iff equiv}, along with an easy corollary which gives a formula for the size of each equivalence class (Corollary~\ref{number of equiv diagrams}).
...
Wherever possible, we drop the subscrips on propagators and their identifying vertices. Propagators are undirected, so $p = (i,j) = (j,i)$. In previous work, we have used the convention, we write $p = (i,j)$ with  $i +1 \leq j$ relative to the first vertex. 
...
and thus speak of a propagator $p = (i, j)$ as being supported by the $i$th and $j$th edges. \sanote{moved/altered dfn and sandwiching paragrahs to improve flow}
...
i.e. $F(P)$ can be thought of as the set of vertices that either only support propagators in $P$ or do not support any propagators at all. Furthermore, note that by construction we have $\Prop(F(P)) \subseteq P$.  This containment is strict if there is any propagator $p$ in $P$ with the property that every vertex in the support of $p$ also supports a propagator not in $P$. \sanote{whoever added this last sentence, thank you. It is an oft overlooked important point.}
\end{rmk}
...
Note that by Definition \ref{admisdfn}, any single propagator and its support defines an exact subdiagram. These are called \emph{trivial} subdiagrams.
...
\begin{enumerate}
\item There exist exact subdiagrams $(P, V(P)) \subseteq W$ and $(P', V(P')) \subseteq W'$ with the same number of propagators and suppored on the same vertices; $|P| = |P'|$ and  $V(P) =  V(P')$.
\item The complementary subdiagrams (i.e. all other propagators and vertices) are identical, i.e. $(\cP \setminus P, [n]) = (\cP' \setminus P', [n])$. \sanote{made some changes} \note{I don't want to change it in case I've just misunderstood, but it seems to me that the vertex sets for the complementary subdiagrams should be $[n]$ not $V(P)^c$ and $V(P')^c$.} \sanote{good call. I had misread it to $V(P^c)$ and $V(P{'c}$, which is didn't say, and added some text. Fixed now.}
\end{enumerate}
...
W_1 = \begin{tikzpicture}[rotate=67.5,baseline=(current bounding box.east)]
    ...
    \drawlabeledprop{5}{0}{8}{0}{\footnotesize \quad $r$}
...
W_2 = \begin{tikzpicture}[rotate=67.5,baseline=(current bounding box.east)]
	...
	\drawlabeledprop{1}{0}{4}{0}{\footnotesize $q$}
	\drawlabeledprop{2}{0}{4}{-1}{\footnotesize $p$}
    \drawlabeledprop{5}{0}{8}{0}{\footnotesize \quad $u$}
...
W_3 = \begin{tikzpicture}[rotate=67.5,baseline=(current bounding box.east)]
	...
	\drawWLD{8}{1.5}
	\drawnumbers
	\drawlabeledprop{1}{0}{4}{0}{\footnotesize $a$}
	\drawlabeledprop{1}{1}{3}{0}{\footnotesize \quad $b$}
    \drawlabeledprop{5}{0}{8}{0}{\footnotesize \quad $u$}
    \drawlabeledprop{6}{0}{8}{-1}{\footnotesize $v$}
		\end{scope}
	\end{tikzpicture} .
...
We have $W_1 \sim W_2$ because they are identical except on the exact subdiagrams 
\[(\{r, s\}, \{5,6,7,8,1\}) \text{ and }(\{u, v\}, \{5,6,7,8,1\})\] respectively. We also have $W_2 \sim W_3$ due to the exact subdiagrams 
\[(\{p, q\}, \{1,2,3,4,5\}) \text{ and }(\{a, b\}, \{1,2,3,4,5\}).\] 
However, the difference between $W_1$ and $W_3$ is the union of two exact subdiagrams, which is not itself exact. \sanote{labeled propagators to make easier to read} \end{eg}

As described in the introduction, Wilson loop diagrams come from the study of SYM $N = 4$ in the physics literature \st{and therefore} \hlfix{where each propagator corresponds to a section of  a $k$ dimesional vector bundle over twistor space. Thus}{I feel the strike out was too vague. The highlighted may be too technical. Can someone find baby bear's text?} each diagram $W$ is associated to a matrix of data $C(W)$, which we now describe. 
...
\sanote{Everything from here applies to weakly admissible. In fact, I think the dfn of admissible in my paper with Amat is what we call weakly admissible here. Should we change to weakly?}
...
Throughout, we take the {\em matroid defined by $W$} to be the matroid $M(W)$ of Theorem \ref{thm WLD defines matroid}. Note from Definition~\ref{def:CWmatrix} that the vertices of the diagram $W$ correspond to columns of the associated matrix $C(W)$. Therefore, for any subset of vertices $V \subseteq [n]$, we can consider the restriction $C(W)|_V$ of $C(W)$ consisting of the columns corresponding to $V$. Since the non-zero entries of $C(W)$ are independent real variables, any subset $V$ that has fewer than $|V|$ rows of non-zero entries does not have full rank. This is equivalent to saying that if $V$ is a set of columns of $C(W)$ with $|\Prop(V)| < |V|$, $V$ is a dependent set. Any set subset of columns containing such a $V$ cannot be independent. Comparing this to Theorem \ref{thm:WLDmatroid}, we see that $M(W)$ is also the matroid realized by $C(W)$. \sanote{fixed dependent set argument}
...
To prove (4), we need to show that $F(P)$ is maximally dependent. If $F(P) = [n]$ then this is automatic, so suppose not and let $v \in [n] \setminus F(P)$. In other words, $v \in V(P^c)$ and so $v$ supports some propagator $q \not\in P$.  Let $S \subseteq F(P)$ be an independent set of maximal size. Then $\Prop(S) \subseteq P$ (from the definition of $F(P)$) and no subset of $S$ supports fewer propagators than the number of vertices it contains (by Theorem~\ref{thm WLD defines matroid}) \st{this is one characterization of an independent set in $M(W)$)}. Since $v$ supports a new propagator $q \not\in P$, the set $S \cup  \subseteq F(P) \cup v$ also satisfies this independence condition. Furthermore, since $S \cup v$ is independent, by the first point in this lemma, $|S| = \rk(S) = \rk F(P)$. Similarly $|S \cup v| = |S|+1 = \rk (S \cup v) \leq \rk(F(P) \cup v)$.  Thus $\rk(F(P) \cup\{v\}) = \rk(F(P)) + 1$, as required. \sanote{tweaked}
\end{proof}
...
This section explores the correspondence between Wilson loop diagrams and positroids, established in \cite{wilsonloop}. In that paper, the authors show that admissible Wilson loop diagrams define matroids that are also positroids, Theorem \ref{put it back in then ref}. Furthermore, they show that if two Wilson loop diagrams are equivalent, they define the same matroid [ref thm].

In this section, we take this analysis further, showing the converse of Theroem [ref] from loc. cit. (see Theorem \ref{same matroid iff equiv}. We also enumerate the number of diagrams that map to an particular matroid, Corollary \ref{number of equiv diagrams}. To do this, we establish a connectection between Wilson loop diagrams and chord diagrams, subsection \ref{sec: polygon partitions}, and prove some matroidal properties of exact subdiagrams in subsection \ref{sec: exact diagram matroidal props}  including that they correspond to uniform matroids (Theorem \ref{exactuniformthm}), and the associated matroid can be written as as a contraction by the complementary propagator flat (Theorem \ref{exact diagrams contractions}).\sanote{need to discuss positroids, and whether positroids should appear earlier so that they can be discussed here.}
...
\begin{dfn}\label{WLDtriangulationdfn}
  ...
  \item Each propagator of $W$ defines a chord \note{We were originally thinking in terms of chord diagrams hence the language ``chord'' but I think ``diagonal'' would be more natural now.  Any thoughts against before I change it?}\sanote{go for it.} edge of $\tau(W)$; specifically,  a propagator $(i,j) \in \cP$ defines a chord connecting the vertices $i$ and $j$ in $\tau(W)$.
  \end{itemize}
\end{dfn}
...
Split every vertex of $T(W)$ which has degree $>3$ (see Figure~\ref{fig:vertex splitting in the dual graph} for an example of this). That is, split $T(W)$ at every vertex corresponding to a non-triangle face of $\tau(W)$. 
...
\draw (-.3,1.) node {\footnotesize $p$};
\draw (-.2,.65) node {\footnotesize $q$};
...
The set of propagators $P= \{p,q,r,s \}$ defines an exact subdiagram of $W$, with $V(P) = \{1,2,3,4,5,8,9\}$.  The corresponding subgraph $t$ of $\tau(W)$ has vertex set $\{1, 2,3, 4, 8\}$. Note that we include vertex 3 of $\tau(W)$ because the two endpoints of edge 3 in $W$ (namely the vertices 3 and 4) are both in $V(P)$. However, we don't include vertex 5 of $\tau(W)$ because only of its endpoints in $W$ (5 and 6) are in $V(P)$, namely 5. The same argument applies to vertices 7, 9 and 10. Vertex 6 is excluded because none of its endpoints lie in $V(P)$. 
...
The key point to note is that the subgraph $t$ depends on how the propagators of $P$ sit inside $[n]$, not on how they sit in $W_P$, which would have an outer edge between vertices 8 and 1, one between 2 and 4, and another between 4 and 8, but would not include vertex 3.  In particular, $t$ is not the subgraph of $\tau(W)$ consisting only of edges corresponding to propagators of $P$ (as it may also include some of the outer edges of $\tau(W)$), nor is it the polygon dissection of $(P,V(P))$ viewed as a Wilson loop diagram in its own right (which would have vertex set equal to $V(P)$).
\end{eg}
...
suppose that the set $V(P)$ defines $c$ disjoint cyclic intervals of $[n]$. Then $t$ has $m-c$ vertices.  If $t$ were a triangulation, $t$ would have $j -c$ \hlfix{internal}{here, internal means that it doesn't define the outer face of $t$ in the induced embedding. Different use of the word than above?} edges.

The graph $t$ has $j$ edges that come from propagators, and $m - 2c$ edges that come from the boundary polygon of $\tau(W)$. Since $t$ has $m-c$ vertices, it has $m-c$ \hlfix{external}{means defines outer face, not just and outer edge of $\tau(W)$} edges, of which $c$ come from propagators. Therefore, of the $j$ edges of $t$ that come from propagators, $j-c$ are internal to the connected component. Therefore, $t$ is a triangulated piece.
...
We begin by proving some useful facts about propagator flats, and flats of matroids associated to admissible Wilson loop diagrams more generally. Recall from Definition~\ref{VPropdfn} that the propagator flat $F(P)$ of $P \subseteq \cP$ denotes the set $V(P^c)^c$ of vertices that \st{do not support any propagators not in $P$.}\hlfix{only support propagators in $P$ (or no propagators)}{too many negatives. Can we use this instead?}
...
\item If $(P,V(P))$ is an exact subdiagram of $W$, then $F(P^c)$ is a dependent flat in $W$. \note{Should we emphasize that all these $P^c$ are wrt $W$ not the subdiagram?} \sanote{yes. done}
...
\end{dfn}\note{How is this a def as it doesn't define the cells?  Also, should it be cell singular}
...
For the converse, \note{I don't want to mess with this because it was one of the delicate proofs, but why can't we just say $M(W)=M(W')$ so the set of uniform submatroids is the same for $M(W)$ and $M(W')$ so by the previous result the set of exact subdiagrams of $W$ and $W'$ is the same, and all this was inclusion preserving so the set of maximal exact subdiagrams is also the same.} suppose that $M(W) = M(W') = M$. This immediately implies that $|\cP| = |\cP'|$, so to demonstrate the equivalence of $W$ and $W'$ it suffices to show that for any maximal exact subdiagram $(P,V(P))$ in $W$, there is a corresponding maximal exact subdiagram $(P',V(P'))$ in $W'$ with $V(P) = V(P')$.
...
In this section we have characterized the correspondence between Wilson loop diagrams and positroids, showing that Wilson Loop diagrams define the same matroid if and only if they are equivalent (Theorem \ref{same matroid iff equiv}). Corollary \ref{number of equiv diagrams} enumerates the fibres\note{Is ``enumerates the fibres'' correct?} of the map from Wilson Loop diagrams to positroids.  The obvious next question would be to enumerate the number of equivlence classes.  
...
The correspondence between (weakly) admissible Wilson loop diagrams and polygon dissections shown in section \ref{sec: polygon partitions} suggests a connection to associahedra. First we need some background. Associahedra can be defined in terms of polygon partitions. In this section, we show that the number of equivalence classes of Wilson loop diagrams correspond to the number of non-parallel faces of the appropriate associahedron (Theorem \ref{thm:count inequiv diagrams}). While this section does not give an explicit formula for the number of these faces, we hope that this linkage with a combinatorially well studied object will shed light on the issue in the future.
