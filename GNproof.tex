...
We now proceed to prove $Q\subseteq \mathcal{P}_{m-1}$ by inductively proving $Q_l \subseteq \mathcal{P}_{m-1}$ for all $l \geq 0$.  If $Q_0 = \Prop(j_m) = \emptyset$ then $Q = \emptyset$, and $I_i(Q) \cup \{j_m\}= \{j_m\}$ is a single vertex supporting no propagators, and thus a dependent set. Otherwise, $Q_0 \subseteq \cP_{m-1}$ follows immediately from the first observation above that $\Prop(j_m) \subseteq \cP_{m-1}$
