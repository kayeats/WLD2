...
Note that we cannot have $a_r+ 1 = j_m$ when $q$ is counterclockwise of $p_r$, since $q \not\in \Prop(j_m)$. If $V(p_r) \subseteq [I_i(p_r)-1,j_m]$, then since $q$ is inside $p_r$ we also have $V(q) \subseteq [a_r-1,j_m]$ and hence $q \in \cP_{m-1}$ by Lemma [no fourth vertex]. In particular, if $l=1$ then $p_r \in Q_{0} = \Prop(j_m)$ and we must be in this case. If not, then there must be some propagator $p_s \in Q_{l-2}$ such that $p_r$ and $p_s$ are both supported on $I_i(p_s)$; further, we can assume that $q$ is not supported on $I_i(p_s)$ since $q \not\in Q_{l-1}$. This yields one of the configurations in Figure~\ref{fig:awkward induction step}.

Next we prove the base case of $l=1$. Else, let $q \in Q_1 \setminus Q_0$. By construction $q$ shares support with a propagators in $\Q_{0}$. Call this propagator $p_r$. By the above, $p_r \in \cP_{m-1}$. Suppose that $q \not \in \cP_{m-1}$. The violet note above implies that $q$ must be inside $p_r$ in the $a_{r}-1$ ordering.  If $a_r = r_m-1$, such a $q$ cannot exist (it would force $I_i(q) = j_m$). Else, since $q$ cannot contribute its fourth vertex, by Lemma \ref{insert reference}, $I_i(q) < j_m$. That is, $q \in \cP_{m-1}$, giving a contradiction. There are two types of propagators in $Q_1 \setminus Q_0$, the $q$ such that $j_m$ is inside $q$ in the $a_q-1$ ordering, and the $q$ where $j_m$ is on the outside.

Continuing by induction, Since all of $Q_{l-1} \subseteq \cP_{m-1}$ by the induction hypothesis, consider $Q_l \setminus Q_{l-1}$. If this set is empty, we are done. Otherwise, let $q \in Q_l \setminus Q_{l-1}$. By construction $q$ shares support with a propagator in $Q_{l-1}$. Call this propagator $p_r$. Let $v_r$ be the fourth vertex of $p_r$ in the $a_{r}-1$ ordering. Since $I_i(p_r) = a_j$, $v_j$ is not assigned to $I_i$ by $p_j$. We have two cases: there exists a $p_s \in Q_{l-2}$ such that both $p_r$ is on the inside in the $a_s -1$ ordering, and $j_m$ is on the outside, or $p_r$ is on the outside of any $p_s \in Q_{l-2}$ that has $j_m+1$ on the outside in the $a_s-1$ ordering. In the first case, following the same argument as the base case, if $q \not \in \cP_{m-1}$, $q$ must be inside $p_s$ in the $a_{s}-1$ ordering.  Since $q$ cannot contribute its fourth vertex, $I_i(q) < v_s < j_m$. That is, $q \in \cP_{m-1}$, giving a contradiction. \sanote{I think this takes care of the picture you sent last night. However, it doesn't address your original complaint about the second case.}

In the second case, if $p_r$ is on the outside of all $p_s \in Q_{l-2}$ that has $j_m+1$ on the outside in the $a_s-1$ ordering. {\color{violet}I worry that as soon as we have propagators ``escaping'' $j_m$, i.e. having an endpoint in the cyclic intervale $[j_m+1, i-1]$, we start having to include propagators on ``the wrong side'' of the diagram. Which is why I don't like this case.}
...
\end{figure} \sanote{in both these pictures, you would have to have some chain of propagators coming out from $j_m$ to meet $p_s$ and then you have a crossing of propagators issue. $p_r$ must be on the outside of $p_s$}
