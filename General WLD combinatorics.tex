...
other after $k_{r} - 2$).  Then since $p_{r+1}$ has length 2, it must be that $(i_r+3)+1 \geq k_r-2$ and so $q_r$ has length $\leq 6$.  By the minimality assumption on $q_r$ no propagator in $W_r$ has $q_r$ on one sid eand $p_{r+1}$ on the other side, so there is any propagator in $W_r$ other than $p_{r+1}$ itself must be of the form $(i,j)$ with $i_r\leq i, j\leq i_r+2$ or $k_r-1 \leq i,j\leq k_r$ and hence be of length 2, and so we have configuration \ref{item pair of 2s} which we have already assumed does not occur.  Consequently, $\cP_{in}(q_r)$ contains only $p_{r+1}$ and so we have \ref{item big and 2} which again we have assumed does not occur.
Therefore the first case cannot occur.

The second case is that one of the following three things happen
\begin{itemize}
\item $i_r+3 \leq j_{r+1}\leq k_r-3$ or $i_r+3 \leq j_{r+1}+2\leq k_r-3$,
\item $i_r = j_{r+1}$, or 
\item $k_r-2 = j_{r+1}$
\end{itemize}
(Phrased more causually, this is that $p_{r+1}$ has either at least one end on an edge in the interval $[i_r+3, k_r-2]$ or both ends lie in $[i_r, i_r+3]$ or both ends lie in $[k_r-2, k_r+1]$.)
