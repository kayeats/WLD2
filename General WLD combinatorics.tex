...
Our goal is to compute the dimension of the positroid associated to an arbitrary admissible Wilson loop diagram. By Lemmas \ref{lem uncovered}, \ref{lem dihedral}, and \ref{lem good p}, it suffices to study the Le diagrams of \hlfix{weakly}{check} admissible Wilson loop diagrams admitting one of the configurations described in Lemma~\ref{lem good p} with propagators $p$ and $q$ (if it exists) both of length 2. Diagrams of this type are the subject of the next few lemmas, and are illustrated in Figure~\ref{fig special p}.

\begin{lem}\label{lem I}
Let $W$ be an admissible Wilson loop diagram with $n\geq 1$ propagators, and suppose that $W$ admits one of the configurations described in Lemma~\ref{lem good p} with $p$ and $q$ (if it exists) both of length 2. Let $V$ be $W$ with $p$ removed.  Then
  \begin{align*}
    I_1^{W} & = I_1^{V} \cup \{n-3\} \\
    I_n^{W} & = I_1^{V} \cup \{n\} \\
...
Next, consider $I_n^{(W)}$, i.e. we apply the Grassmann necklace algorithm with starting vertex $n$. In both cases$I_n^{(W)}(p) = n$. We are now at vertex 1, and the unassigned propagators are exactly those that appear in $V$. Therefore the algorithm continues as in $I_1^{(V)}$, i.e. we have $I_n^{W} = I_1^{V} \cup \{n\}$. Similarly, we must have $I_{n-1}^{W} = I_n^{V} \cup \{n-1\}$.
