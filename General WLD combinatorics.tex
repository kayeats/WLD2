...
Note that for any Wilson loop diagram, $W = (\cP, n)$, each propagator $P$ partions $n$ into two sets, $\{V_{in}(p), V_{out}(p)\}$, where $V_{in}(p)$ and $V_{out}(p)$ correspond to the two  cyclic subintervals of $[n]$ that lie on either side of $p$. This in turn defines three sets of propagators, $\{\cP_{in}, p, \cP_{out}\}$, where (for $\bullet \in \{in, \; out\}$) \bas \cP_{\bullet} = \{ q \in (\cP \setminus p) | V(q) \cap V_{\bullet}(p)  \neq \emptyset \} \;.\eas If $W$ has no propagators crossing $p$ and no other propagator supported on $V(p)$, then the set $\{\cP_{in}, p, \cP_{out}\}$ partitions $\cP$. The set $\cP_{in}$ defines the propagators with end points on one side of $p$, while $\cP_{out}$ defines the propagators with end points on the other side. In particular, for (weakly) admissible Wilson loop diagrams, the set $\{\cP_{in}, p, \cP_{out}\}$ is a partition for every propagators $p \in \cP$. 

It remains to determine with set of propagators/vertices are refered to as on the inside of $p$ and which are on the outside. To do this, we impose a direction on $p$.

 \sanote{can we use the notation directed propagator here? to emphasize the notational change?}
...
We may associate the propagators on the inside and outside of $p$ accordingly.
 
Note that under this definiton, $V_{in}(p, <_i) = V_{out}(p, <_j)$. That is, which set of propagators lie inside and outside of $p$ switch depending on the direction imposed $p$. 
...
\[\ell(p) = min\big\{|V_{in}(p)|,|V_{out}(p)|\big\} = min\big\{|[i+1,j]|,|[j+1,i]|\big\}.\]
\end{dfn}
In other words, $\ell(p)$ is the size of the smaller of the two sets $V_{in}(p)$ and $V_{out}(p)$ defined above.
