...
\item The vertex set of the subgraph $t$ is
  \begin{itemize}
  \item \hlfix{the vertices of $\tau(W)$ corresponding to edges of $W$ defined by cyclically consecutive elements of $V(P)$ as a subset of $[n]$}{changed to simplify with existing notation, but now it contradicts below}
  \end{itemize}
\item The edge set of the subgraph is
  ...
  \end{itemize}
\end{itemize}
Note that the subgraph $t$ only depends on $P$, \hlfix{not the vertex set of the subdiagram $W_P$}{really? why?}, but it is not the subgraph of $\tau(W)$ consisting only of edges corresponding to propagators of $P$. 
