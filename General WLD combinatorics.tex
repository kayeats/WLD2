...
Let $W = (\cP, n)$ be an admissible Wilson loop diagram, and $p = (i,j) \in \cP$ be a propagator. As always, we use the convention $i+1 <_1 j$. The region of $W$ bounded by $p$ and the edge of the Wilson loop diagram from $i$ to $j+1$ is refered to as the region {\bf inside} $p$, and the complement is the region {\bf outside }$p$. The set of propagators inside (resp. outside) $p$ are those lying in the region inside (resp. outside) $p$, excluding $p$ itself.
\end{dfn} \sanote{changed dfn to match notation} \sanote{do we really mean $i$ to $j+1$, or $i+1$ to $j$, as in length}

Note that this depends on the convention that $i+1 <_1 j$: if we work with the $<_j$ order on vertices then the regions inside and outside $p$ are swapped. \sanote{Is this remark needed? or does is obfuscate?}
...
\end{dfn}
In other words, given that $p$ partitions the vertices of $W$, $\ell(p)$ is the size of the smaller of the two partitions.
