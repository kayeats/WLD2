...
\item For the given $a$, either $a-3 <_{v+1} i_{s_1}$ or there exist $2$ propagators $s_2$ and $s_3 \in P_v$ such that $s_3 = (i_{s_3}, i_{s_1})$ and $s_2 = (i_{s_2}, i_{s_1})$. One may also have $s_2 = (i_{s_2}, i_{s_1}-1)$. Furthermore, $a-3 <_{v+1} s_m$.\end{enumerate}
\end{dfn}
...
Let $W$ be a Wilson loop diagram with $n$ vertices.  Let $C(W)$ be the matrix of $W$ as defined in \eqref{C(W) dfn}.
\begin{itemize}
...
For example consider the Wilson loop diagram in Figure~\ref{fig R eg}.  
\[
C(W) = \begin{bmatrix} a & b & 0 & 0 & 0 & c & d \\ e & f & 0 & 0 & g & h & 0 \\ i & j & 0 & k & l & 0 & 0 \end{bmatrix}
\]
The Grassmann necklace is $I_1 = \{1,2,4\}, I_2 = \{2,4,5\}, I_3 = \{4,5,6\}, I_4=\{4,5,6\}, I_5=\{5,6,7\}, I_6 = \{6,7,1\}, I_7=\{7,1,2\}$.  The bijection between propagators and vertices for each necklace element is also in Figure~\ref{fig R eg}.  Between $I_1$ and $I_2$, two propagators change which vertex they are assigned to and these are the propagators associated to rows 1 and 2 of $C(W)$ as given above.  Then
\[
\Delta_{I_2}=\det\begin{bmatrix} b & 0 & 0 \\ f & 0 & g \\ j & k & l \end{bmatrix} = kgb
\]
and so $r_2 = gb$.  From $I_2$ to $I_3$ only one propagator changes its vertex association and we get $r_3 = c$.  $I_3=I_4$ and so $r_4=1$.  Continuing likewise we get $r_5 = lhd$ and $r_6 = i$.  From $I_6$ to $I_7$ the propagators associated to the second and third rows change, thus from $\Delta_{I_7} = d(ej-fi)$ we get $r_7 = ej-fi$.  Note that as proved in the next proposition, this quadratic factor arises when the two edges whose vertex association changed are associated with the two ends of the edge on which they are both supported.  Finally, $r_1 = (af-be)k$.  Together we obtain
\[
R = (af-be)kgbclhdi(ej-fi)
\]
which is squarefree and contains all factors of $\prod_{i=1}^{n}\Delta_{I_i}$.
