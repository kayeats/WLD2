...
\begin{lem}\label{lem sian}
  ...
  \begin{enumerate}
    \item $W$ has a propagator of length $\leq 6$ with a propagator of length 2 inside it and nothing else inside it.\label{item big and 2} \sanote{doesn't this depend on which side you call inside? Should we say ``on one side''?  -- the point is that the inside should be the length 6 side.  Now there could be two length 6 sides in which case, yes there should exist one of the length 6 sides with the desired property.  I'm not sure how you want to phrase all this}
