...
\begin{lem}\label{lem sian}
  ...
  \begin{enumerate}
    \item $W$ has a propagator of length $\leq 6$ with a propagator of length 2 on one side it and nothing else on that side.\label{item big and 2} \sanote{tweaked}
...
We need a bit of notation: Let $W = (\cP, n)$ a weakly admissible Wilson loop diagram, and $Q \subseteq \cP$. Define the restriction $W|_Q$ to be the diagram $W$ with the propagators not in $Q$ removed. That is , \bas W|_Q = (Q, n). \eas  \sanote{tweaked this para and the next}
...
\begin{itemize}
\item $\ell(q_r) \geq 4$.
\item If we orient the $q_r = (i_1, k_1)$ such that the $\{p_1 \ldots p_r\} \subseteq \cP_{out}(q_r)$ are on the outside. Furthermore, we have have $\cP_{in}(q_r) \subsetneq \cP_{in}(q_{r-1})$. That is, the propagators on the inside of $q_r$ are strictly contained in the set of propagators inside of $q_{r-1}$. In particular, $W_{\cP_{in}(q_r) \cup q_r}$ does not contain any of the propagators $p_1, \dots, p_r$. \sanote{changed, have I broken it? In particular, $k_i +1$ now used to be called $k_i$ before}
\end{itemize}
...
Now suppose $q_{r} = (i_r, k_r) $ exists by the induction hypothesis, oriented $(q_r, <_{i_r})$ as above. For the rest of this proof, we assume this orientation as read, and drop the $<_{i_r}$ from the notation. Call the restriction to the propagators inside $q_r$ in this orientation (including itself) $W_r := W|_{\cP_{in} \cup q_r}$ does not contain any $p_i$ for $i\leq r$. By the original hypotheses on $W$ every vertex in $V_{in}(q_r)$ (which is a non-empty interval since $\ell(q_r) \geq 4$) must have support $\geq 1$, while the same is true for $V(q_r)$ because these vertices support $q_r$. By construction, the remaining vertices of $W|_{\cP_{in} \cup p}$, i.e. $V_{out}(q_r)$ are non-supporting.
...
Note we may replace $q_r$ by any other propagator, $q'_{r}$ of length $\geq 4$ in $W_r$ such that $p_{r+1}$ and $q_r$ are on opposite sides; such a new $q'_{r}$ still satisfies all the hypotheses, and so, without loss of generality we may assume that $q_r$ has minimal length among propagators of length $\geq 4$ such that $p_{r+1}\in \cP_{out}(q_r)$ is on the outside.  

There are two cases to consider.
In the first case define $p_{r+1} = (j_{r+1}, j_{r+1}+2)$  with $i_r \leq j_{r+1} \leq i_r+2$ and $k_r-2 \leq j_{r+1} \leq k_{r}$.  Then since $p_{r+1}$ has length 2, it must be that $(i_r+3)+1 \geq k_r-2$ and so $q_r$ has length $\leq 6$.  Let $s$ be a propagators in $\cP_{in}(q_r)$ such that $p_{r+1}$ is not an interior propagator in the $<_{i_r}$ ordering: $p_{r+1} \not \in \cP_{in}(s, <{i_r})$. If it exists, $s = (i_s, j_s)$ must satisfy $i_r \leq i_s \leq j_s \leq i_r+2$ or $k_r-2 \leq i_s \leq j_s \leq k_r$ and hence be of length 2.  Therefore, if $\cP_{in}(q_r)$ contains a propagator $t$ of length $\leq 4$, $p_{r+1}$ must lie on its inside in the $<i_r$ ordering ( $p_{r+1} \in \cP_{in}(q_r, <_{i_r})$) which is impossible my minimality.  Therefore, if $\cP_{in}(q_r)$ contains another propagator of length $2$ we would have configuration \ref{item pair of 2s} which we have already assumed does not occur.  Consequently, $\cP_{in}(q_r)$ contains only $p_{r+1}$ and so we have \ref{item big and 2} which again we have assumed does not occur.
Therefore the first case cannot occur.

In the second case, write $p_{r+1} = (j_{r+1}, j_{r+1}+2)$. Either at least $i_r+3 \leq j_{r+1} +2 \leq k_r-3$, or $i_r = j_{r+1}$ or $k_r-2 = j_{r+1}$; these situations all behave similarly.  By symmetry we consider only the first two cases. Not that since $j_{r+1}+4 \leq k_r-1$ in both cases, $j_{r+1}+4 \in V_{in}(q_r)$, and is thus supported. Since $p_{r+1}$ is not part of configuration \ref{item pair of 2s}, there is some propagator $t \in \Prop(j_{r+1}+4)$ with \hlfix{length $\geq 4$.  By noncrossingness $t$ is inside $q_r$, $t \in \cP_{in}(q_r)$, and $p_{r+1}$ is outside $t$, $p_{r+1} \not \in \cP_{in}(t, <_{i,r})$ by minimality. So we may set $q_{r+1} = t$ to continue the induction.  }{Hope I haven't broken things, but I don't follow why $\ell(t)\geq 4$ or $p_{r+1} \not \in \cP_{in}(t, <_{i,r})$}
...
Next we will prove that $j_k <_i j_0$ for all $k > 0$.  This gives a contradiction to the exisitence of $J$ and completes the proof of the theorem.  This proof will also be by induction.  We already observed that $j_1 <_i j_0$.  Suppose we have $j_{j} <_i j_0$ for $j<k$.  Then $p_{k-1}$ and $p_{k}$ are both supported at $j_{k-1}$, but $I_{i}(p_{k-1}) = j_{k-1}$.  Thus either $p_{k-1}$ appears before $p_k$ in the neighbourhood of $j_{k-1}$ or $p_k$ was taken earlier by $I_i$ so $I_i(p_k) <_i I_i(p_{k-1})$.  In the latter case we already have $j_k = I_i(p_k) <_i j_{k-1} <_i j_0$ which is what we want.  So we now assume that $p_{k-1}$ appears before $p_k$ in the neighbourhood of $j_{k-1}$.

There are two cases to consider.  First is if $I_i(p_{k-1}) = J(p_{k-1})-1$ and $p_{k-1}$ has an end on the edge $(I_i(p_{k-1}), J(p_{k-1}))$.  In this case consider the neighbourhood of $I_i(p_{k-2}) = J(p_{k-1})$.  As the $p_j$ are disjoint we know that at this vertex either $p_{k}$ was taken before $p_{k-2}$ (and so $I_i(p_k) <_i I_i(p_{k-2})$ and we are again done), or $p_k$ comes after $p_{k-2}$ around this vertex.

***Karen is here in this file (and her commute ended for the day)***
...
\begin{proof}
  ...
  By Lemma~\ref{lem dihedral} there is a dihedral transformation of $V$ which yields a diagram satisfying the statement of the lemma with $p$ and $q$ both length 2. Restoring the vertices which do not support any propagators, we obtain a dihedral transformation $W'$ of $W$ as desired (with potentially longer lengths for $p$ and $q$).
\end{proof}



Combining Lemmas \ref{lem uncovered}, \ref{lem dihedral}, and \ref{lem good p}, it therefore suffices to study the Le diagrams of \hlfix{weakly}{check} admissible Wilson loop diagrams admitting one of the configurations described in Lemma~\ref{lem good p} with propagators $p$ and $q$ (if $q$ exists) both of length 2. See Figure~\ref{fig special p} for an illustration of the two possibilities.
...
The two possible situations are illustrated in Figure~\ref{fig special p}. The arguments proceed in a similar manner to Lemma~\ref{vertex cyclic int lem}.

We first consider $I_1^{(W)}$. Note that $n-3 \not\in I_1^{(V)}$: for the right hand case this is clear from the diagram (since Algorithm~\ref{alg:put GN on WLD} would assign $q$ no later than vertex $n-4$), while for the left hand case it follows from Lemma \ref{lem no fourth vertex}. Therefore when we start at vertex 1 and apply the Grassmann necklace algorithm to $W$, $p$ is the only unassigned propagator when we reach vertex $n-3$ and we have $ I_1^{(W)}(p) = n-3$. 


Next, consider $I_n^{(W)}$, i.e. we apply the Grassmann necklace algorithm with starting vertex $n$. In both cases we have $I_n^{(W)}(p) = n$. We are now at vertex 1, and the unassigned propagators are exactly those that appear in $V$. Therefore the algorithm continues as in $I_1^{(V)}$, i.e. we have $I_n^{W} = I_1^{V} \cup \{n\}$. Similarly, we must have $I_{n-1}^{W} = I_n^{V} \cup \{n-1\}$.
...
On the other hand, if $n-2\in I_{n-2}^{V}$ then we must be in the right hand case of Figure~\ref{fig special p} and the algorithm assigns $q$ to vertex $n-2$ in both $V$ and $W$ (since $p$ is always the counterclockwise-most propagator supported on vertex $n-2$). If $n-1 \not\in I_{n-2}^{(V)}$, then in $W$ the algorithm assigns $p$ to $n-2$ and then proceeds identically to the remainder $I_{n-2}^{(V)}$; thus $I_{n-2}^{W} = I_{n-2}^{V}\cup \{n-1\}$ in this case.

Finally, if $n-2,n-1 \in I_{n-2}^{(V)}$ then the algorithm in $W$ assigns $q$ to $n-2$ and $p$ to $n-1$ as above, but this is different to what occurs in $I_{n-2}^{(V)}$ so we cannot argue as above. We are now at vertex $n$ and only propagators $p$ and $q$ have been assigned; thus we are proceeding as in the construction of $I_n^{(V)}$ but without propagator $q$. By Lemma~\ref{vertex cyclic int lem} we know that $q$ was assigned to $n-5$ by $I_n^{(V)}$, and the only way this could occur is if all other propagators in $V$ had already been assigned when we reached vertex $n-5$. Therefore $I_{n-2}^{W} = (I_{n}^{V} \setminus \{n-5\}) \cup \{n-1, n-2\}$; this completes all cases for $I_{n-2}^{(W)}$.
...
Recall that $I_1$ determines the shape of the Le diagram. By Lemma~\ref{lem I}, we know that $I_1^{W} = I_1^{V}\cup \{n-3\}$, and that $n,n-1,n-2, n-3 \not\in I_1^{(V)}$. This implies that the right hand boundary of the shape of $V$ is the same as the right hand boundary of the shape of $W$ except that $W$ has one additional row of 3 boxes while $V$ has an additional column in the $n-3$ position; that is, an extra column fourth from the left.
\end{proof}

The pieces of the Le diagrams of $V$ and $W$ will be called $\mathcal{A}$ and $\mathcal{B}$ in what follows, as in Figure~\ref{fig Le}. Over the course of the next few lemmas we will prove that the plusses in the $\mathcal{B}$ parts of each diagram are identical, and that the plusses in the $\mathcal{A}$ parts are very closely related.  

Note that when we speak of a plus in the Le diagram of $V$ being the same as in $W$ or vice versa, we mean that the position of the plus in $\mathcal{A}$ or $\mathcal{B}$ is the same; because of the column insertion the absolute indices may differ.
...
%Recall Algorithm~\ref{alg:GN to Le}, which constructs the Le diagram associated to a Grassmann necklace. As described in Section [ref], if Algorithm~\ref{alg:GN to Le} places a $+$ in the box with row index $i$ and column index $j$, we say that this plus is in the $i \rightarrow j$ position.


\begin{lem}\label{lem n and n-1}
Let $V$ and $W$ be as in Lemma~\ref{lem I}.
Then $I_n^{W}$ and $I_{n-1}^{W}$ together yield the same plusses as $I_n^{V}$ did, along with extra plusses in the leftmost two boxes of the bottom row of the Le diagram of $W$.
\end{lem}
...
\begin{lem}\label{lem n-2 good}
  Let $V$ and $W$ be as in Lemma~\ref{lem I}, with $n-2 \not\in I_{n-2}^{V}$. Then $I_{n-2}^{W}$ contributes an $(n-3)\rightarrow (n-2)$ plus, and all of the $I_{n-1}^{V}=I_{n-2}^{V}$ plusses.
\end{lem}

\begin{proof}
If $n-2\not\in I_{n-2}^{V}$ then $I_{n-1}^{V}=I_{n-2}^{V}$ and by Lemma~\ref{lem I} we have $I_{n-2}^{W} = I_{n-1}^{V} \cup \{n-2\}$.  Note that $n-3\not\in I_{n-2}^{V}$ by Lemma~\ref{lem no fourth vertex}.  Therefore the paths for $I_{n-2}^{W}$ are the paths for $I_{n-1}^{V}$ along with the $(n-3)\rightarrow (n-2)$ path.  This gives the statement of the lemma.
