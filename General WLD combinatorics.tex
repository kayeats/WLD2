...
\begin{lem}\label{lem sian}
  Let $W$ be an admissible WLD in which each vertex supports at least one propagator.  Then exactly one of the following things occurs.
  \begin{enumerate}
    \item $W$ has a propagator of length 3, say $p =  (i,i+3)$, with a propagator of length 2 inside it.
    \item $W$ admits no propagators of length 3, and there exists a pair of propagators with the property that the first propagator is $(i, i+1, i+2, i+3)$, the second is $(i+2, i+3, i+4, i+5)$, and no other propagator ends on the edge $(i+2, i+3)$.
  \end{enumerate}
\end{lem}

\note{This is the proof of the old result pasted without modification so we can identify where it needs editing. For some reason the WLD was named $D$ instead of $W$.}
\begin{proof}
...
We will inductively construct a sequence of pairs of propagators $(p_r,q_r)$ satisfying: $\ell(p_r) = 2$, and $p_r$ either forms part of a configuration \eqref{eq:2 prop config} in $D$ or there is a propagator $q_r$ satisfying
\begin{itemize}
\item $\ell(q_r) \geq 4$.
...
\end{itemize}
Since $D$ contains finitely many vertices, this must eventually terminate in a configuration of the form \eqref{eq:2 prop config}.

Start by choosing a propagator $p_1$ of length 2 in $D$ (which exists by Remark~\ref{rem:props of length 2 and 3}), and write $(j_1,j_1+1,j_1+2,j_1+3)$ for its support.  If it is part of a configuration \eqref{eq:2 prop config} we are done, so suppose otherwise: since $s_{j_1} \geq 2$, there must exist a second propagator $q_1$ of length $\geq 4$ which starts on edge $(j_1,j_1+1)$.  This bounds a region $[i_1,j_1+1]$ which does not contain $p_1$.

Now suppose $q_{r}$ exists by the induction hypothesis and bounds a region $[i_r,j_r+1]$.  Consider the restricted diagram $D_r:=D_{[i_r,j_r+1]}$; clearly we must have $s_l \geq 2$ for all $l \in [i_r+2,j_r-1]$ (which is a non-empty interval since $\ell(q_r) \geq 4$), while $s_l\geq 1$ for $l \in \{i_r,i_r+1,j_r,j_r+1\}$.  

By Remark~\ref{rem:props of length 2 and 3}, $D_r$ admits at least one propagator of length 2 and none of length 3.  Let $p_{r+1}$ be a propagator of length 2 in $D_r$; if it forms part of the configuration \eqref{eq:2 prop config} then we are done.  Otherwise, at least one endpoint of $p_{r+1}$ lies on an edge contained in the non-empty interval $[i_r+2,j_r-1]$, and so there is at least one other propagator connected to this edge.  Up to symmetry, there are three possibilities:
