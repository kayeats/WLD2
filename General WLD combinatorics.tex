...
\begin{itemize}
\item $\ell(q_r) \geq 4$.
\item If we orient the $q_r$ such that the $\{p_1 \ldots p_r\}$ are on the inside. The outer region of $W$ that is bounded by $q_r$ is strictly contained in the region of $W$ bounded by $q_{r-1}$ on the outside. Thus the restriction of $W$ to this region does not contain any of the propagators $p_1, \dots, p_r$. \sanote{changed, have I broken it?}
\end{itemize}
...
then we are done, so assume otherwise. \sanote{What if $p_{r+1}$ outside of $(q_r, <_{k_r-1})$?} 
