...
Note that for any Wilson loop diagram, $W = (\cP, n)$, each propagator $p$ partions $[n]$ into two cyclic subintervals of $[n]$, $\{[i+1, j], [j+1, i]\}$ that lie on either side of $p$. In the sequel we refer to these intervals as an interval inside and outside of $p$, depending on a ordering we associate to the propagator.
