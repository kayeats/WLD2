...
Let $W = (\cP, n)$ be an admissible Wilson loop diagram, and $\vec{p} = (i,j) \in \cP$ be a directional propagator and $\cev{p} = (j,i)$ be the same propagator with the opposite directionality. Call the vertices of $W$ \bas V_{in}(\vec{p}) = \{a \in [n] | i <_i a <_i j\} \cup V(p) \eas the region of $W$ {\bf inside} $\vec{p}$, and \bas V_{out}(\vec{p}) = \{a \in [n] | j <_j a <_j i\} \cup V(p) \;.\eas the region {\bf outside }$\vec{p}$. The set of propagators inside (resp. outside) $\vec{p}$ are those lying in the region inside (resp. outside) are the $q \neq p \in \cP$ such that  $V(q) \subset V_{in}(p)$ (resp. $V(q) \subset V_{out}(p)$) .
\end{dfn} 

Note that under this definiton, $V_{in}(\vec{p}) = V_{out}(\cev{p})$ and $V_{out}(\vec{p}) = V_{in}(\cev{p})$. That is, the regions inside and outside of a propagator switch depending on the direction imposed on the propagators $p$. 
...
\[\ell(p) = min\big\{|V_(in)(\vec{p})|,|V_(in)(\cev{p})|\big\}-2 = min\big\{|[i+1,j]|,|[j+1,i]|\big\}.\]
\end{dfn}
