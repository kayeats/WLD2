...
\item If $[n]$ can be partitioned into at least two non-empty sets, each of which support different sets of propagators, \bas [n] = \cup_i V(P_i) \quad \textrm{s.t.} \quad \cup P_i = \cP; \quad  P_i \cap P_j = \emptyset \;, \eas the the matroid $M(W)$ is seperable, \bas M(W) = \bigoplus_i M(P_i, V(P_i)) \;.\eas 
\end{enumerate}
\end{lem}
\begin{proof}
The first part of (1) is \cite[Equation (9)]{wilsonloop} and surrounding discussion, and the second part is standard matroid theory. (2) is \cite[Lemma 3.27]{wilsonloop}. (3) is also in \cite[Lemma FIND]{wilsonloop}.
\end{proof}
...
Let $v \in C$. Clearly $\Prop(v) \subseteq \Prop(C)$, and so by Lemma \ref{lem properties of prop flatfs} we have $F(\Prop(v)) \subseteq F(\Prop(C))$. Since $v \in F(\Prop(v))$ by the definition of propagator flat, we have $v \in F(\Prop(C))$ as required.
...
Note that by definition, since $F(P^c)$ has at $|P^c|$ propagators in it, this is an upper bound on the rank. However, showing equality on the first part is harder.
...
where $C$ is the largest cyclic flat contained in $F(P^c)$ and $S$ is an  independent set. Note that $C$ must be non-empty since $F(P^c)$ is dependent. We proceed by showing that $S$ must be empty, forcing $F(P^c)$ to be a cyclic flat.
...
Combining this with \eqref{Qsizebound} gives $|V(Q)| - 3 \leq |Q|$. That is, either $W$ was not admissible, or $(P,V(P))$ was not a maximal exact subdiagram. In other words, $S$ cannot be non-empty, proving that $F(P^c)$ is a cyclic flat, and therefore of maximal rank. 
