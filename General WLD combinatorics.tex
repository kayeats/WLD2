...
We depict this data as a convex polygon, with vertices labeled by $V$ (preserving the cyclic ordering), and $k$ wavy lines in the interior of the diagrams. We depict the choice of first vertex on $V$ by a $\bullet$. \note{If we look at the pictures we have later (at least some of them), we don't do that.  Instead all the vertices are $\bullet$.} Note that the polygon gives the verices of $W$ a cyclic ordering, which the choice of first vertex gives it a compatible linear order. Both the cyclic and the linear order become the correct perspective at various points in this paper. The pair $p_r$ defines a wavy line from the $i_r^{th}$ edge (between $i_r$ and $i_r+1$) and the $j_r^{th}$ edge. These are called propagators.  The condition on $i_r$ and $j_r$ means that the propagator does not go between adjacent edges. Let $\cP = \{p_r\}_{r=1}^k$ be the set of propagators. Then we write \bas W = (\cP, V) \;.\eas
...
Suppose $|P|=1$.  Then $(P, V(P))$ is a trivially exact subdiagram. In this case $t$ is an edge internal to $\tau(W)$.  The exactness condition on $(P, V(P))$ says that the four supporting vertices of the element of $P$ are distinct and no other propagators are supported on those vertices.  Translating this to $\tau(W)$ gives that the vertices which are the ends of $t$ are separated by at least two vertices along the cycle and no other internal edges connect to the ends of $t$.  This implies that $t$ is a trivial triangulation.

Now suppose $|P|>1$.  Let $j=|P|$, $m=|V(P)|$, and 
suppose that the set $V(P)$ defines $c$ disjoint cyclic intervals of $[n]$. Then $t$ has $m-c$ vertices.  If $t$ were a triangulated component, $t$ would have $j -c$ internal edges.
