...
\note{Are the next few definitions used outside the proof of Lemma~\ref{lem sian}?  If not, then perhaps they could go inside the proof so as not to clutter the document with temporary definitions?}
\begin{dfn}
...
\begin{lem}\label{lem sian}
  ...
  \begin{enumerate}
    ...
    \item $W$ admits no propagators of length 3, and one or both of the following two things occurs.
      \begin{enumerate}
        ...
        \item There exists a pair of propagators of length $2$ with the property that the first propagator is $(i, i+1, i+2, i+3)$, the second is $(i+2, i+3, i+4, i+5)$, and no other propagator ends on the edge $(i+2, i+3)$.\label{item pair of 2s}
      \end{enumerate}
  \end{enumerate}
\end{lem}

\begin{proof}
  Suppose first that $W$ has a propagator of length $3$, $p=(i, i+1, i+3, i+4)$.  By Remark~\ref{rm:props of length 2 and 3} along with the fact that every vertex of $W$ supports at least one propagator, we have that $i+1$ supports exactly one propagator and this propagator must have length 2 by noncrossingness.
...
%\end{equation}
  ...
  We will inductively construct a sequence of pairs of propagators $(p_r,q_r)$ satisfying: $\ell(p_r) = 2$, and $p_r$ either forms part of configuration \ref{item lonely 2} or \ref{item pair of 2s} from the statement
  %a configuration \eqref{eq:2 prop config} in $D$
  or there is a propagator $q_r$ satisfying
\begin{itemize}
\item $\ell(q_r) \geq 4$.
\item $q_r$ bounds a region of $D$ which contains strictly fewer vertices than that bounded by $q_{r-1}$ and the restriction of $D$ to this region does not contain any of the propagators $p_1, \dots, p_r$.
\end{itemize}
Since $D$ contains finitely many vertices, this must eventually terminate in one of the two desired configurations. %a configuration of the form \eqref{eq:2 prop config}.

Start by choosing a propagator $p_1$ of length 2 in $W$ (which exists by Remark~\ref{rem:props of length 2 and 3}), and write $(j_1,j_1+1,j_1+2,j_1+3)$ for its support.  If it is part of one of the two configurations we are looking for then we are done, 
%a configuration \eqref{eq:2 prop config} we are done,
so suppose otherwise: since $p_1$ is not in configuration \ref{item lonely 2} then there must be a second propagator starting either on edge $(j_1, j_1+1)$ or $(j_1+2, j_1+3)$.  Flipping the diagram if necessary, assume the former.  Since $p_1$ is not in configuration \ref{item pair of 2s} then there must exist a second propagator $q_1$ of length $\geq 4$ which starts on edge $(j_1,j_1+1)$.  This bounds a region $[i_1,j_1+1]$ which does not contain $p_1$.

Now suppose $q_{r}$ exists by the induction hypothesis and bounds a region $[i_r,j_r+1]$.  Consider the restricted diagram $W_r:=W_{[i_r,j_r+1]}$; clearly we must have $s_l \geq 2$ for all $l \in [i_r+2,j_r-1]$ (which is a non-empty interval since $\ell(q_r) \geq 4$), while $s_l\geq 1$ for $l \in \{i_r,i_r+1,j_r,j_r+1\}$.  

By Remark~\ref{rem:props of length 2 and 3}, $W_r$ admits at least one propagator of length 2 and none of length 3.  Let $p_{r+1}$ be a propagator of length 2 in $D_r$; if it forms part of either configuration \ref{item lonely 2} or \ref{item pair of 2s} 
%the configuration \eqref{eq:2 prop config}
then we are done.  Otherwise, at least one endpoint of $p_{r+1}$ lies on an edge contained in the non-empty interval $[i_r+2,j_r-1]$, and so there is at least one other propagator connected to this edge.  Up to symmetry, there are three possibilities:
