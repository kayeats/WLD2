...
Finally, if $n-2,n-1 \in I_{n-2}^{V}$ then the algorithm in $W$ assigns $q$ to $n-2$ and $p$ to $n-1$ as above, but this is different to what occurs in $I_{n-2}^{V}$ so we cannot use the same argument as above. We are now at vertex $n$ and only propagators $p$ and $q$ have been assigned; thus we are proceeding as in the construction of $I_n^{V}$ but without propagator $q$. By Lemma~\ref{vertex cyclic int lem} we know that $q$ was assigned to $n-5$ by $I_n^{V}$, and the only way this could occur is if all other propagators in $V$ had already been assigned when we reached vertex $n-5$ (see Figure~\ref{fig special p}); thus $q$ was the final propagator to be assigned in $I_n^{V}$. Therefore $I_{n-2}^{W} = (I_{n}^{V} \setminus \{n-5\}) \cup \{n-1, n-2\}$; this completes all cases for $I_{n-2}^{W}$.
...
Then $I_{n-2}^{W}$ and $I_{n-3}^{W}$ contribute the following plusses to the Le diagram of $W$:
  ...
  \item An $(n-3)\rightarrow (n-2)$ plus and a $(n-1)\rightarrow (n-1)$ plus.
  ...
  \item In the Le diagram of $V$, $I_{n-3}^{V}$ gives a plus in the $n-3$ column.  This $+$ is shifted one square to the left (into the $n-2$ column) in the Le diagram of $W$.
  \item All other plusses of $I_{n-3}^{V}$.
  \end{itemize}
Furthermore, no element of the Grassmann necklace of $V$ contributes a $(n-5)\rightarrow (n-1)$ plus.
\end{lem}

\begin{proof}
  By Lemma~\ref{lem I} we have $I_{n-2}^{W}= (I_{n}^{V} \setminus \{n-5\})\cup \{n-1,n-2\}$. We are necessarily in the case where $W$ admits a propagator $q$ (Case 2 of Figure~\ref{fig special p}) and we can see from the location of $q$ that $n-2\not\in I_{n-3}^{V}$ and that $n-5$ is the largest row index in $\mathcal{B}$.  Thus this section of the Gramann necklace of $V$ looks like
  ...
  This means that the plusses in the Le diagram of $V$ coming from $I_{n-2}^{V}$ are as in the first part of Figure~\ref{fig messy}.  Stepping to $I_{n-1}^{V}$ simply removes the $n-5\rightarrow n-2$ path (equation \eqref{eq necklace}); this is illustrated in the second part of Figure~\ref{fig messy}.

  \begin{figure}
    \includegraphics{messy}
    \caption{Plusses coming from $I_{n-2}^{V}$ (top left),  $I_{n-1}^{V}$ (top right) and $I_{n}^{V}$ (bottom), in the case where $n-1, n-2\in I_{n-2}^{V}$.  The blue lines are the non-intersecting paths defining the position of the plusses.  The dashed blue lines may or may not appear, but if one appears then they both do.}\label{fig messy}
  \end{figure}
  ...
  Now consider $I_{n-3}^{V}$.  By \eqref{eq necklace} $I_{n-3}^{V}$ contributes almost the same plusses as $I_{n_2}^{V}$: the only difference is that it contributes an $(n-5)\rightarrow (n-3)$ plus in place of the $(n-5)\rightarrow (n-2)$ plus.  Since we must have $n-3\in I_{n-3}^{V}$ (because propagator $q$ exists in $V$), it follows from Lemma\ref{lem I} that $I_{n-3}^{W} = I_{n-3}^{V}\cup\{n-2\}$.  Thus the paths for $I^{W}_{n-3}$ are the same as for $I^{V}_{n-3}$ except that the path that did go to $n-3$ now goes to $n-2$.  This cannot conflict with another path since \eqref{eq necklace} shows that $n-2$ only appears in $I_{n-2}^{V}$ among the necklace elements of $V$.

  Also note that $I_{n-3}^{V}, I_{n-2}^{V}$, and $I_{n-1}^{V}$ share their plusses outside of the $n-3$ and $n-2$ columns.  This proves the remaining bullet points of the lemma, leaving only the claim about the $(n-5)\rightarrow (n-1)$ plus.

  So suppose there were a $n-5\rightarrow n-1$ plus in the Le diagram of $V$.  By Algorithm~\ref{alg:GN to Le}, this plus could only be contributed by a necklace term that satisfies $n-5 \not\in I_*^{V}$.  By \eqref{eq necklace}, the only terms that could have this property are $I_{n-4}^{V}$, $I_{n-3}^{V}$, and $I_{n-2}^{V}$. The analysis above shows that it does not come from $I_{n-2}^V$ or $I_{n-3}^V$.

  Recall that $n-5$ is the largest row index in the Le diagram of $V$, and $n-5 \in I_1^V \setminus I_{n-4}^V$ so we do have a path starting at $n-5$. Since $n-4 \in I_{n-4}^V$ by the location of the propagator $q$, we must have a path ending at $n-4$. Since the paths cannot cross, this implies that $_{n-4}^V$ must contribute a $(n-5) \rightarrow (n-4)$ plus, and hence cannot contribute a $(n-5)\rightarrow (n-1)$ plus.

\end{proof}


\begin{lem}\label{lem other k}
  Let $V$ and $W$ be as in Lemma~\ref{lem I}, and suppose that if $n-2\in I_{n-2}^{V}$ then also $n-1\in I_{n-2}^{V}$. Fix a $k$ in the range $1<k<n-2$. Then $I_k^{W}$ contributes the same plusses as $I_{k}^{V}$, except that if $I_{k}^{V}$ contributed a plus in the $n-3$ column then this plus is shifted one square left into the $n-2$ column, and no plus was already in that location in the Le diagram of $W$.
\end{lem}
...
Recall that $I_1^W = I_1^V\cup \{n-3\}$ and that $n-2,n-1,n \not\in I_1^W$.
  ...
  Since $n-3 \in I_k^V \setminus I_1^V$, we must have a path from some vertical edge $i$ to the bottom edge $n-3$ in the Le diagram of $V$.  In the Le diagram of $W$, the index $n-3$ labels a vertical edge with no path starting at it (since $n-3$ belongs to both $I_1^W$ and $I_k^W$), and there must be a path leading to $n-2$ since $n-2 \in I_k^W \setminus I_1^W$.  By the previous paragraph no other path from $I_k^V$ could end at $n-2$, so since the paths cannot cross we conclude that the $i\rightarrow (n-3)$ path in the Le diagram of $V$ must become a $i\rightarrow (n-2)$ path in the Le diagram of $W$.
...
\begin{proof}
  ...
  By Lemmas~\ref{lem n and n-1}, \ref{lem n-2 good}, and \ref{lem n-2 bad}, the bottom row of the diagram of $W$ always contains 3 new plusses; thus $W$ admits $3k + 3$ plusses in total. Applying induction completes the proof.
