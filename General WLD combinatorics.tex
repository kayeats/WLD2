...
\item $F \setminus C$ is an independent \st{set. Further, if $W$ does not contain any vertices of rank $0$, i.e. all vertices support at least $1$ propagator, then $F \setminus C$ is an independent} flat. \sanote{$F$ and $C$ both contain all vertices of rank $0$ by definition} 
\end{enumerate}
\end{lem}
...
with $|\Prop(C)| = \rk(C)$ by Lemma \ref{lem facts about WLD matroids} and $|Q| = |B| = \rk(C)$ by the choice of $B$. Thus $Q = \Prop(C)$, and so $B$ is a basis in the matroid defined by $W' := (\Prop(C),V(\Prop(C)))$. All that remains is to note that $w \in F(\Prop(C)) \subseteq V(\Prop(C))$, i.e. $w$ is a vertex in $W'$; thus $B \cup\{w\}$ is dependent in $W'$, and hence also in $W$.  This also implies that $\rk(F\cup w) = \rk(F)$ since $B\subseteq F$ and so since $F$ is a flat we must have $w\in F$.

Returning our attention to $M(W)$, the dependent set $B \cup \{w\}$ must contain a circuit $C'$, with $w \in C'$ since $B$ was an independent set. Now $C \cup C' = C \cup \{w\} \supsetneq C$ is a cycle which, since $w\in F$, contradicts the maximality of $C$.


For part (2), first note that $F \setminus C$ is automatically independent as it contains no circuits. Now suppose that all vertices in $W$ have rank $\geq 1$. \sanote{can get rid of?}
...
{\color{violet} Note that by definition, since $F(P^c)$ has at $|P^c|$ propagators in it, this is an upper bound on the rank. However, showing equality on the first part is harder.}
...
where $C$ is the largest cyclic flat contained in $F(P^c)$ and $S$ is an  independent set. Note that $C$ must be non-empty since $F(P^c)$ is dependent. {\color{violet}We proceed by showing that $S$ must be empty, forcing $F(P^c)$ to be a cyclic flat.} \sanote{Does this fix confusion later?}

Since $C = F(\Prop(C))$ by Lemma \ref{lem decompose flat}, define $Q := \Prop(C)^c$ so that we may write $C = F(Q^c) = V(Q)^c$. {\color{violet}Note that since $C \subseteq F(P^c)$, $P \subseteq Q$. }Furthermore, we have 
...
{\color{violet}REPLACE BELOW WITH: Let $P$ be a maximal exact subdiagram as above, and let $(R, V(R))$ be an exact subdiagram that is not maximal. That is, $R \subsetneq P$. }

Let $(Q, V(Q))$ be an exact subdiagram that is not maximal, with \hlfix{$Q \subset P$ defining the maximal exact subdiagram above}{If I've understood this right, we would need to show that any exact subdiagram appears as $\Prop(C)^c$ for some maximal exact $P$ first} . \sanote{why? I don't understand your confusion}


{\color{violet}POSSIBLY REPLACE BELOW WITH: Since $R \subset P$, $V(P) = V(R) \sqcup S$. Since $P$ and $R$ both define exact subdiagrams, we may write $|V(P)| = |V(R)| + |P \setminus R|$, where $S$ is a vertex set of size $|P \setminus R|$ with $(P \setminus R \subset \Prop(S)$. Since $\rk V(P) =  \rk V(R) + |P \setminus R|$, $S$ is independent. Taking the complements, we may write \bas F(R^c ) = F(P^c) \sqcup S \;.\eas Since $S$ is an independent set of vertices of the correct size supporting the appropriate propagators, this gives $ \rk F(R^c ) = |R^c|$.}


Using the fact that $P$ and $Q$ are both exact diagrams, we have $|V(P)| = |V(Q)| + |P \setminus Q|$.  Then $V(Q)^c = F(P^c) \cup S$, where $S$ is an independent set of size $|P \setminus Q|$ and with $(P \setminus Q) \subset \Prop (S)$. Therefore, \hlfix{$\rk (F(Q^c)) = |Q^c|$.}{I'm lost here.} \sanote{better if I fix typo?}
\end{proof}
...
\begin{lem}\label{lem sian 1}
  Let $W$ be an admissible WLD in which each vertex supports at least one propagator.  Then exactly one of the following two things occurs.
  \begin{enumerate}
...
\begin{proof}
  Suppose first that $W$ has a propagator of length $3$, $p=(i, i+3)$.  By Remark~\ref{rem:props of length 2 and 3} along with the fact that every vertex of $W$ supports at least one propagator, we have that $i+2$ supports exactly one propagator and this propagator must have length 2 by noncrossingness.
...
With these observations in mind we can return to the proof.
  ...
  We will inductively construct a sequence of pairs of propagators $(p_r,q_r)$ satisfying: $\ell(p_r) = 2$, and $p_r$ either forms part of configuration \ref{item lonely 2}, \ref{item pair of 2s}, or \ref{item 4 and 2} from the statement,
...
\end{itemize}
Since $W$ contains finitely many vertices, this must eventually terminate in one of the three desired configurations. %a configuration of the form \eqref{eq:2 prop config}.
...
so suppose otherwise: since $p_1$ is not in configuration \ref{item lonely 2} then there must be a second propagator starting either on edge $j_1)$ or $j_1+2$.  Flipping the diagram if necessary, assume the former.  Since $p_1$ is not in configuration \ref{item pair of 2s} and every vertex is supported then there must exist a propagator $q_1$ of length $\geq 4$ with one end on edge $j_1$ or on edge $j_1-1$ or on edge $j_1-2$.  This bounds a region $[i_1,k_1]$ (where $k_1 \in \{j_1-1, j_1, j_1+1\}$) which does not contain $p_1$.
...
By Remark~\ref{rem:props of length 2 and 3}, $W_r$ admits at least one propagator of length 2.  $W_r$ has no propagators of length $3$ since $W$ has none.  Let $p_{r+1}$ be a propagator of length 2 in $W_r$; if it forms part of configuration \ref{item lonely 2}, \ref{item pair of 2s}, or \ref{item 4 and 2}
%the configuration \eqref{eq:2 prop config}
then we are done, so assume otherwise.

Note we may replace $q_r$ by any other propagator of length $\geq 4$ in $W_r$ which contains $p_{r+1}$ and such a new $q_r$ still satisfies all the hypotheses, and so, without loss of generality we may assume that $q_r$ has minimal length among propagators of length $\geq 4$ which contain $p_{r+1}$.  

There are two cases to consider.
The first case is that $p_{r+1}$ has one end before $i_r+3$ and the other after $k_r-3$.  Then since $p_{r+1}$ has length 2, it must be that $[i_r+3,k_r-3]$ is at most a single edge, and so $q_r$ has length $\leq 6$.  The only propagators inside $q_r$ that do not also contain $p_{r+1}$ must have both ends in $[i_r, i_r+3]$ or both ends in $[k_r-3, k_r]$ and hence be of length 2.  Therefore, if $q_r$ contains a propagator $t$ of length $\leq 4$, this propagator must also contain $p_{r+1}$ which is impossible my minimality.  Therefore, if $q_r$ contains another propagator of length $2$ we would have configuration \ref{item pair of 2s} which we have already assumed does not occur.  Consequently, $q_r$ contains only $p_{r+1}$; if $q_r$ is of length $4$ then we have \ref{item 4 and 2} and if $q_r$ is of length $5$ or $6$ then we have either \ref{item lonely 2} or an unsupported vertex, all of which are impossible.  Therefore the first case cannot occur.
...
\note{This is a mess for now; protecting sets commented out, and 1.5 versions of the induction copied in by not yet edited}

This is the lemma from the email with the proof copied from the email.
...
\begin{enumerate}
  ...
  \item Let $R$ be the set of propagators which are assigned to a vertex in $T_i$ by $f$. Let $S_{i+1}= S_i \cup R$.  Let $T_{i+1} = T_i \sup (\text{supp}(R) - \{n\})$.\label{item loop step}
  \item If there is a vertex $v_{i+1}$ in $T_{i+1}$ which has not yet been assigned then by construction there is a propagator $q_{i+1} \in S_{i+1}-S_i$ which can be assigned to $v_{i+1}$.  Do so, removing $q_{i+1}$'s old assignment.  This frees up a vertex $v_{i}$ in $T_i$ and so there is a propagator $q_{i} \in S_{i}$ which can be assigned to this vertex freeing up an earlier vertex and so on until $p$ has been assigned.  This new assignment contradicts the maximality of $f$.
  ...
  \item Increment $i$ and go to \label{itemp loop step}
\end{enumerate}
...
\note{This was commented out in the file: some (old?) version of the inductive argument?}
...
\note{end of the piece that was commented out in the file}
...
\begin{lem} \label{vertex cyclic int lem}
  ...
  \note{the above is the statement that was in this file, the below is the version of the statement from last july}
  
Let $p$ be a propagator of an admissible WLD $W$ on $n$ vertices.  Let $p = (i,j,k,l)$.  Let $J_p^{(W)}(i)$ be the indices $m$ where $I_m(p)=i$ and likewise for $J_p^{(W)}(j)$, $J_p^{(W)}(k)$, and $J_p^{(W)}(l)$.  Then $J_p^{(W)}(i)$, $J_p^{(W)}(j)$, $J_p^{(W)}(k)$, and $J_p^{(W)}(l)$ are each non-empty cyclic intervals of $\{1, \ldots, n\}$ which partition $\{1, \ldots, n\}$ and occur in the given cyclic order.
\end{lem}


\begin{proof}
    We will prove the result by induction on the number of propagators.  If $W$ has one propagator then the result is immediate.  Now suppose $W$ has more than one propagator.  Since unsupported vertices have no effect on the Grassmann necklace algorithm, it suffices to prove the result for $W$ with no unsupported vertices.  Then by Lemma~\ref{lem sian} $W$ has at least one of the three situations illustrated in figure~\ref{fig 3 cases}



  \begin{figure}
    \includegraphics[scale=0.8]{3cases}
    \caption{Three cases for admissible WLDs with no unsupported vertices.  The blue highlighted region cannot support other propagators.  The green half-propagators illustrate where propagators may occur, but are not required to exist.}\label{fig 3 cases}
  \end{figure}


In each of the three cases, when we remove the propagator labelled $p$ we obtain a diagram which satisfies the statement of the theorem by the induction hypothesis, and contains a propagator $q$ with no other propagators inside it (although this region may or may not contain an unsupported vertex $b$). Call this diagram $V$ (figure \ref{fig V diagram}).


\begin{figure}
\includegraphics[scale=0.8]{Vdiagram}
\caption{Diagram $V$ is $W$ with $p$ removed; there are no propagators inside $q$. Vertex $b$ is omitted if we are considering the right hand case.}
\label{fig V diagram}
\end{figure}

We first describe $J_q^{(V)}(*)$: since there are no propagators inside $q$, we see from the diagram that
\[b,c \in J^{(V)}_q(c) \text{ (if $b$ exists) and }  c+1 \in J^{(V)}_q(c+1).\]
Note that $c+2 \not\in J^{(V)}_q(c+1)$ by Lemma~\ref{lem last vert}, so $J^{(V)}_q(c+1) = \{c+1\}$ and $c+2 \in J^{(V)}_q(a-1)$ by the induction hypothesis. Thus there exist vertices $d,e \in [c+2,a]$ with $d <e$, such that
\[J^{(V)}_q(a-1) = [c+2,d-1], \quad J^{(V)}_q(a) = [d,e-1], \quad J^{(V)}_q(c) = [e,c], \quad J^{(V)}_q(c+1) = \{c+1\},\]
...
\textbf{Left and middle cases}: In both of these cases, $p$ has limited effect on the Grassmann necklace of $W$ due to the presence of the previously unsupported vertex $b$. We first calculate $I^{(W)}_w$ for a starting vertex ${w \in [n]\backslash \{b,c,c+1\}}$: if $w\in J_q^{(V)}(a-1)$ then
    \[
    I_w^{(W)}(r) =  \begin{cases}
        a & \text{if } r=p \\
        I_{w}^{(V)}(r) & \text{if } r\neq p
      \end{cases} 
    \]
    while if $w\in J_q^{(V)}(a)$ or $w\in J_q^{(V)}(c)$ then
    \[
    I_w^{(W)}(r) =  \begin{cases}
        b & \text{if } r=p \\
        I_{w}^{(V)}(r) & \text{if } r\neq p
      \end{cases} 
      \]
For the majority of the remaining vertices, we use the following observation: if $p$ is the first propagator to contribute to a Grassmann necklace term $I_w^{(W)}$, then the remainder of $I_w^{(W)}$ proceeds identically to the assignments of $I^{(V)}_{w+1}$.  Thus in both cases, we have
    \begin{align*}
       I_b^{(W)}(r) & = \begin{cases}
        b & \text{if } r=p\\
        I_b^{(V)}(r) & \text{if } r\neq p
       ...
       \end{cases}
    \end{align*}
The left and middle cases differ only at the starting vertex $c+1$. In the left hand case, we obtain
\[
       I_{c+1}^{(W)}(r)  = \begin{cases}
         c+1 & \text{if } r=p\\
         I_{c+2}^{(V)}(r) & \text{if } r\neq p
       \end{cases}
\]
by the same arguments as above; thus the intervals $J^{(W)}_r(*)$ are still cyclic for all $r \neq p$, and
\[J^{(W)}_p(a) = [c+2,d-1], \quad J^{(W)}_p(b) = [d,b], \quad J^{(W)}_p(c) = \{c\}, \quad J^{(W)}_p(c+1) = \{c+1\}\]
also satisfies the statement of the theorem.

In the middle case, if we start at $c+1$ we need to know whether any other propagators remain on edge $(a-1,a)$ when we reach vertex $a-1$. We already know that $J_q^{(V)}(c+1) = \{c+1\}$; in particular this means that $q$ contributes $a-1$ in $I_{c+2}^{(V)}$.  However the construction of $I^{(V)}_{c+1}$ first associates $q$ to $c+1$ and then proceeds identically to $I^{(V)}_{c+2}$.  In particular if $a-1$ was assigned in $I^{(V)}_{c+1}$, then it would not be available to assign to $q$ in $I^{(V)}_{c+2}$ as all other propagators supported at $a-1$ in $V$ come before $q$. 
    Therefore $p$ is the only unassigned propagator on edge $(a-1,a)$ when we reach $a-1$ in $W$, and
    \[
    I_{c+1}^{(W)}(r)  = \begin{cases}
      a-1 & \text{if } r=p\\
      I_{c+1}^{(V)}(r) & \text{if } r\neq p
    \end{cases}
    \]
Assembling the intervals, we have
\[J^{(W)}_p(a-1) = \{c+1\}, \quad J^{(W)}_p(a) = [c+2,d-1], \quad J^{(W)}_p(b) = [d,b], \quad J^{(W)}_p(c) = \{c\},\]
as required.


\textbf{Right hand case}: In this case there is no vertex $b$, and $p$ is supported on ${(c,c+1,c+2,c+3)}$ in $W$. For $w \in [n]\backslash\{c,c+1,c+2,c+3\}$: if $w\in J_q^{(V)}(a-1)$ or $w\in J_q^{(V)}(a)$ then
    \[
    I_w^{(W)}(r) =  \begin{cases}
        c & \text{if } r=p \\
        I_{w}^{(V)}(r) & \text{if } r\neq p
...
Finally, for the vertices in the support of $p$, we have
      ...
      I_{c+1}^{(W)}(r) & = \begin{cases}
        ...
        I_{c+3}^{(V)}(r) & \text{if } r\neq p,q
      \end{cases}\\
      I_{c+2}^{(W)}(r) & = \begin{cases}
        c+2 & \text{if } r=p \\
        I_{c+3}^{(V)}(r) & \text{if } r\neq p
      \end{cases}\\
      I_{c+3}^{(W)}(r) & = \begin{cases}
        c+3 & \text{if } r=p \\
        I_{c+4}^{(V)}(r) & \text{if } r\neq p
      \end{cases}
\end{align*}
Note that $I_{c+2}^{(V)}(r) = I_{c+3}^{(V)}(r)$ for all propagators $r$ in $V$. Once again we can see that the $J_r^{(W)}(*)$ are cyclic for all $r$ in $V$, and
\[J_p^{(W)}(c) = [c+4,e-1], \quad J_p^{(W)}(c+1) = [e,c], \quad J_p^{(W)}(c+2) = \{c+1,c+2\}, \quad J_p^{(W)}(c+3) = \{c+3\}.\]
