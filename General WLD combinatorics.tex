...
\begin{bmatrix} A & B \\ 0 & C\end{bmatrix}
\]
where $A$ is the matrix for the propagators in $S$ and the vertices in $I_i(S)$. Again two things can now happen.  If some vertex $j$ or larger in $>_i$ appears in $I_i$ then $B$ and $C$ have at least one column and so the block form of the matrix gives a nontrivial factorization of $\Delta_{I_i}$ which is a contradiction.  If no vertex $j$ or larger is in $I_i$ then the matrix giving $\Delta_{I_i}$ is just $A$.  Looking in more detail into $A$, note that of supports of $p$ and $q$, only $i$ and $i+1$ are in $I_i$ and hence
\[
A = \begin{bmatrix} D & 0 \\ E & F\end{bmatrix}
\]
where $D$ is the $2\times 2$ matrix indexed by the propagators $p$ and $q$ and the vertices $i$ and $i+1$.  By assumption $\Delta_{I_i}$ had degree 3 or more and so this matrix decomposition gives that $\Delta_{I_i}$ factors into nontrivial factors contradicting our assumptions.
