...
Since $\tau(W)$ is an embedded graph (with a fixed distinguished embedding) by Lemma \ref{tausimpleplanarlem}, $T(W)$ is an uniquely defined graph.\note{The fixed embedding is important, because otherwise you only have a unique planar dual if the graph is 3-connected which this one isn't.  I'm just noting that here so that everyone knows why I added the part about the embedding and then we can take this comment out.}
...
In \cite{wilsonloops}, the authors show that all {\color{violet}admissible} Wilson loop diagrams correspond to positroids. That is, they correspond to matroids that can be represented by elements of the positive Grassmannians $\Gr(|\cP|, |V|)$.\todo{Check: positroids or totally positive? Why specify exactness?}

\begin{dfn}
Given a Wilson loop diagram $W$, define the positroid cell associated to a Wilson loop diagram, $\Sigma(W)$, to be the cell in the CW complex on $\Gr(|\cP|, |V|)$ defined by the \hlfix{realizations}{we haven't specified what this means}  of $W$ that lie in $\Gr(|\cP|, |V|)$. {\color{violet} Comment: Realizable matroids are those that can be written as a matrix. A realization is one such matrix. I think this is standard nomenclature?}
\end{dfn}
