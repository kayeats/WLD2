...
\begin{dfn}
  Let $W$ be an admissible Wilson loop diagram and $\tau(W)$ its polygon partition. A \emph{triangulated piece} of $\tau(W)$ is a 2-connected subgraph of $\tau(W)$ which is a triangulation. We will take the convention that a subgraph consisting of {\color{red}two non-adjascent} vertices joined by an edge  is also a triangulated piece, called a \emph{trivial} triangulated piece.
\end{dfn}
...
\begin{lem} \label{decompositionlem}
  For $W$ an admissible Wilson loop diagram, the polygon partition $\tau(W)$ has a unique decomposition into maximal triangulated pieces, and boundary edges. 
\end{lem}

***draw an example***

\begin{proof}
We begin by giving an algorithm for the decomposition, then prove its uniqueness. 

Let $T(W)$ be a the dual graph of $\tau(W)$ with the vertex corresponding the to the infinite face removed. In other words, place a vertex on each finite face of $\tau(W)$. These vertices are connected if there is a chord edge separating the edges. Furthermore, if the face is bounded by an edge of the polygonal cycle of $\tau(W)$, the corresponding vertex gets a leaf edge for each such boundary. Since, by Lemma \ref{tausimpleplanarlem}, $\tau(W)$ is a simple planar graph, this \hlfix{$T(W)$ is an uniquely defined tree.}{Why do I know it is a tree? }

By construction, each face of $\tau(W)$ is a polygon. Therefore, each vertex of $T(W)$ is at least 3 valent (one for each edge of the polygon). If split each vertex of $T(W)$ if it is strictly greater than $3$ valent. (In other words, we fail to split exactly when the face is a triangle). The connected components of $T(W)$ correspond to the decomposition of $\tau(W)$ into maximal triangulated pieces. The the vertices of the forest thus obtained have valence 1, corresponding to root or leaf vertices, or 3, corresponding to triangulated faces. Subtrees with no trivalent vertices correspond to either edges of the boundary of $\tau(W)$, if they were originally leaves of $T(W)$, or to maximal trivial triangulations. \hlfix{Splitting at all the faces that are not triangles ensures maximality of the decomposition.}{more words?}

\hlfix{To see uniqueness, consider the uniqueness of $T(W)$. If there were two different decompositions of $\tau(W)$, this would imply that there are two different ways to split a vertex of $T(W)$, which implies the n0n-uniqueness of the valency of the vertices of $T(W)$, which contradicts the uniqueness of $T(W)$.}{Your proof was much more complicated. What am I missing?}
\end{proof}
...
\begin{proof}
  ...
  Let $t$ be a triangulated piece of the decomposition of $\tau(W)$ given in Lemma \ref{decomplem} and suppose $t$ has $n>2$ vertices.  We want to count how many edges of $t$ are not on the outer face of $\tau(W)$.  Consider the intersection of $t$ with the outer face of $\tau(W)$.  This is a possibly disconnected subgraph of the polygon of $\tau(W)$ with $n$ vertices, call it $S$.  Let $k$ be the number of connected components of $S$. \hlfix{Also $S$ is a spanning subgraph of the outer face of $t$ and $S$ has $n$ edges.}{Not sure what this means. There are 4 edges in the picture you sent.}  To join the components $S$ must have $k$ edges not in the outer face of $\tau(W)$.  Furthermore $t$ has $n-3$ edges not in its outer face and so also not in the outer face of $\tau(W)$.  Thus there are $n-3+k$ edges of $t$ not in the outer face of $\tau(W)$.
