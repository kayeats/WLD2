...
Let $p$ be a propagator of an admissible WLD $W$ on $n$ vertices.  Let $p = (i,j)$.  For $l\in \{i, i+1, j, j+1\}$, let $J_p^{(W)}(l)$ be the indices $m$ where $I_m(p)=l$.  Then $J_p^{(W)}(i)$, $J_p^{(W)}(i+1)$, $J_p^{(W)}(j)$, and $J_p^{(W)}(j+1)$ are each non-empty cyclic intervals of $\{1, \ldots, n\}$ which partition $\{1, \ldots, n\}$ and occur in the given cyclic order.
\end{lem}
...
Consider the fourth case of Figure~\ref{fig 3 cases} first, as it is the easiest.  In this case the support of $p$ is unsupported in $V$ so for every propagator $r$ in $V$ (including $q$) $J_r^{(V)} = J_r^{(W)}$.  Additionally, $J_p^{(W)}(l+t) = l+a$ for $a\in\{1,2,3\}$ and $J_p^{(W)}(l) = [n] \backslash \{l+1, l+2, l+3\}$ which proves the result in this case.
...
For the majority of the remaining vertices, we use the following observation: if $p$ is the first propagator to contribute to a Grassmann necklace term $I_w^{(W)}$, then the remainder of $I_w^{(W)}$ proceeds identically to the assignments of $I^{(V)}_{w+1}$.  Thus in both cases, we have for any $0\leq b <a$
    ...
    If $j$ is in the support of $p$ which occurs in all cases except the second case with two vertices in the right hand region then
     \[
       I_j^{(W)}(r) & = \begin{cases}
         j & \text{if } r=p \text{ and $j$ is in the suport of $p$} \\
         I_{j+1}^{(V)}(r) & \text{if } r\neq p
       \end{cases}
       \]
       If $j$ is not in the support of $p$,  then we must be in the second case with two vertices in the right hand region.  Then if we start at $j$ we need to know whether propagators other than $p$ will remain around $i$ and $i+1$ when we reach vertex $i$ in the Grassmann necklace algorithm.  ****this part needs work, and it affects the rest potentially too, since $p$ is not taken first***
       
For these two cases, it remains to consider the starting vertex $j+1$. If $j+1$ is in the support of $p$ then we can argue as above to get
\[
       I_{j+1}^{(W)}(r)  = \begin{cases}
         j+1 & \text{if } r=p \text{ and $j+1$ is in the support of $p$}\\
         I_{j+2}^{(V)}(r) & \text{if } r\neq p
       \end{cases}
\]
