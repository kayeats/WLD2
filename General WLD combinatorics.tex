...
We need a bit of notation: Let $W = (\cP, n)$ a weakly admissible Wilson loop diagram, and $Q \subseteq \cP$. Define the restriction $W|_Q$ to be the diagram $W$ with the propagators not in $Q$ removed. That is , \bas W|_Q = (Q, n). \eas  \footnote{If we denote $V \subset [n]$ and $Q = \Prop(V^c)$, then we can write \bas M(W|_{Q^c}) = M(W)/F(Q) \oplus M_{\emptyset, n-|i-j+1|} \eas where  $M_{\emptyset, k}$ is the matroid of rank $0$ on a set of $k$. That is $M_{\emptyset, n-|i-j+1|} =M((\emptyset, n-|i-j+1|))$. } \sanote{tweaked this para and the next}
...
\begin{itemize}
\item $\ell(q_r) \geq 4$.
\item If we orient the $q_r = (i_1, k_1)$ such that the $\{p_1 \ldots p_r\} \subseteq \cP_{out}(q_r)$ are on the outside. Furthermore, we have have $\cP_{in}(q_r) \subsetneq \cP_{in}(q_{r-1})$. That is, the propagators on the inside of $q_r$ are strictly contained in the set of propagators inside of $q_{r-1}$. In particular, $W_{\cP_{in}(q_r) \cup q_r}$ does not contain any of the propagators $p_1, \dots, p_r$. \sanote{changed, have I broken it?}
\end{itemize}
