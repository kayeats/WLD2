...
Let $W = (\cP, n)$ be an admissible Wilson loop diagram, with $p = (i,j) \in \cP$. Write $(p, <_i)$ be the same propagator with a direction induced by the linear ordering $<_i$. Then \bas V_{in}(p, <_i) = \{a \in [n] | i+1 <_i a <_i j \}\eas the region of $W$ {\bf inside} $p$, and \bas V_{out}(p, <_i) = \{a \in [n] | j+1 <_j a <_j i \} \;.\eas the region {\bf outside }$p$. 
\end{dfn} 

Similarly, we may associate the ordering $<_j$ to $p$, in which case $V_{in}(p, <_i) = V_{out}(p, <_j)$ and $V_{out}(p, <_i) = V_{in}(p, <_j)$.

This in turn defines two sets of propagators, $\{\cP_{in}, \cP_{out}\}$, where (for $\bullet \in \{in, \; out\}$) \bas \cP_{\bullet} = \{ q \in (\cP \setminus p) | V(q) \cap V_{\bullet}(p)  \neq \emptyset \} \;.\eas The set $\cP_{in}$ (resp. $\cP_{out}$) is the set of propagators, not equal to $p$ \hlfix{whose support contains points in $V_{in}(p, <_*)$ (resp. $V_{in}(p, <_*)$), where $<_*$ refers to the appropriate ordering.}{Note, this means that paralell propagators are neither on the inside and outside of $p$. Is that okay?} If $W$ has no propagators crossing $p$ and no other propagator supported entirely on $V(p)$, then the set $\{\cP_{in}, p, \cP_{out}\}$ partitions $\cP$. Under these circumstances, the set $\cP_{in}$ defines the propagators with end points on one side of $p$, while $\cP_{out}$ defines the propagators with end points on the other side. In particular, for (weakly) admissible Wilson loop diagrams, the set $\{\cP_{in}, p, \cP_{out}\}$ is a partition for every propagators $p \in \cP$. Recall from Definition~\ref{admisdfn} that a {\em weakly admissible} diagram is one that satisfies the density and non-crossing conditions but does not require that $|V(\cP)| \geq |\cP| + 4$.
...
\begin{rmk}\label{rem:props of length 2 and 3} \sanote{change remark to "Configurations"?}
The following observations about propagators of short length in an (weakly) admissibly Wilson loop diagram, $W$, are easily verified:
\begin{enumerate}
...
\begin{lem}\label{lem sian}
  ...
  \begin{enumerate}
    \item $W$ has a propagator of length $\leq 6$ with a propagator of length 2 inside it and nothing else inside it.\label{item big and 2} \sanote{doesn't this depend on which side you call inside? Should we say ``on one side''?}
...
\begin{proof} \sanote{fixed misuse of the word support in lemma}
...
Now suppose $W$ has no propagators of length $3$. \sanote{when do we deal with this restriction?}
...
Define the restriction of $W$ to a region $[i,j]$ to be the subdiagram obtained by forgetting all propagators not wholly supported on vertices in the cyclic interval $[i,j]$; the vertex set remains the same as in $W$.  Denote this diagram by $W_{[i,j]}$. That is, if $W = (\cP, n)$ is a Wilson loop diagram, \bas W_{[i,j]} = (\cP \setminus \Prop([i,j]^c), n)\eas

{\color{blue} If we denote $Q = \Prop([i,j]^c)$, then we can write \bas M(W_{[i,j]}) = M(W)/F(Q) \oplus M_{\emptyset, n-|i-j+1|} \eas where  $M_{\emptyset, k}$ is the matroid of rank $0$ on a set of $k$. That is $M_{\emptyset, n-|i-j+1|} =M((\emptyset, n-|i-j+1|))$. } \sanote{If we like this idea, I can incorporate the necessary nomenclature earlier.}

Note that if $p = (i, j)$, then we can consider two obvious restrictions of $W$, namely \bas W_{[i,j+1]} = W_{V_{in}(p, <_i) \cup V(p)} \quad \textrm{and} \quad  W_{[j,i+1]}  = W_{V_{out}(p, <_i) \cup V(p)} \eas corresponding to the regions inside and outside of $(p,<_i)$.  They contain the propagators on the inside and outside of $(p, <_i)$ respectfully, as well as $p$ itself.  We say that $p$ \textit{bounds the regions} $[i,j+1]$ and $[j,i+1]$ which we call outside and inside respectively. The ordering of the start and end points allow us to identify one or the other region unambiguously. 
...
\begin{itemize}
\item $\ell(q_r) \geq 4$.
\item If we orient the $q_r$ such that the $\{p_1 \ldots p_r\}$ are on the outside. The inner region of $W$ that is bounded by $q_r$ is strictly contained in the region of $W$ bounded by $q_{r-1}$ on the inside. Thus the restriction of $W$ to this region does not contain any of the propagators $p_1, \dots, p_r$. \sanote{changed, have I broken it?}
\end{itemize}
...
By assumption: $p_1$ is not in configuration \ref{item pair of 2s}, there are no propagators of length $3$, and every vertex is supported.  Therefore there must exist a propagator $q_1$ outside of $(p_1, <_j)$ of length $\geq 4$ with one end on edge $j_1$ or on edge $j_1-1$ or on edge $j_1-2$.  This bounds a region $[i_1,k_1] = V_{out}(q_1) \cup V(q_1)$ on the outside of $(q_1, <_{i_1})$ (where $k_1 \in \{j_1-1, j_1, j_1+1\}$) which does not contain $p_1$.

INSERT PICTURE HERE?

Now suppose $q_{r}$ exists by the induction hypothesis, oriented such that $(q_r, <_{i_r})$ and bounds a region $[i_r,k_r]$ on the outside. This region does not contain any $p_i$ for $i\leq r$. \sanote{Know what $p_1$ is. What are the others?}  Consider the restricted diagram $W_r:=W_{[i_r,k_r]}$; by the original hypotheses on $W$ every vertex $l \in [i_r+2,k_r-2]$ (which is a non-empty interval since $\ell(q_r) \geq 4$) must have support $\geq 1$, while the same is true for $l \in \{i_r,i_r+1,k_r-1,k_r\}$ because these vertices support $q_r$.  

By Remark~\ref{rem:props of length 2 and 3}, $W_r$ admits at least one propagator of length 2.  $W_r$ has no propagators of length $3$ since $W$ has none.  Let $p_{r+1}$ be a propagator of length 2 in $W_r$ (i.e. outside of $(q_r, <_{k_r-2})$; if it forms part of configuration \ref{item big and 2} or \ref{item pair of 2s}
%the configuration \eqref{eq:2 prop config}
then we are done, so assume otherwise. 

Note we may replace $q_r$ by any other propagator, $q'_{r}$ of length $\geq 4$ in $W_r$ such that $p_{r+1}$ and $q_r$ are on opposite sides; such a new $q'_{r}$ still satisfies all the hypotheses, and so, without loss of generality we may assume that $(q_r, <_{k_r-1})$ has minimal length among propagators of length $\geq 4$ which contain $p_{r+1}$ on the outside.  
